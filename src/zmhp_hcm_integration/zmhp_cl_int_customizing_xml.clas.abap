class ZMHP_CL_INT_CUSTOMIZING_XML definition
  public
  final
  create private .
...
public section.
  ...
  types:
    t_customizing_infotypes TYPE TABLE OF zmhp_int_infotyp WITH DEFAULT KEY .
  types:
    t_customizing_fields TYPE TABLE OF zmhp_int_fields WITH DEFAULT KEY .
  types:
    t_customizing_folders TYPE TABLE OF zmhp_int_folders WITH DEFAULT KEY .
  types S_CUSTOMIZING_FIELDS type ZMHP_INT_FIELDS .
  types S_CUSTOMIZING_FOLDERS type ZMHP_INT_FOLDERS .
  types:
    t_customizing_values TYPE TABLE OF zmhp_int_values WITH DEFAULT KEY .
  types:
    t_customizing_conversion TYPE TABLE OF zmhp_int_convers WITH DEFAULT KEY .
  types:
    t_customizing_mapping_fields TYPE TABLE OF zmhp_int_mapp_fi WITH DEFAULT KEY .
  types:
    t_customizing_mapping TYPE TABLE OF zmhp_int_mapping WITH DEFAULT KEY .
  ...
  class-methods GET_INSTANCE
    ...
    returning
      value(RO_INSTANCE) type ref to ZMHP_CL_INT_CUSTOMIZING_XML .
...
private section.
  ...
  class-data GO_INSTANCE type ref to ZMHP_CL_INT_CUSTOMIZING_XML .
...
CLASS ZMHP_CL_INT_CUSTOMIZING_XML IMPLEMENTATION.
...
*     Noting found? Check molga independent customizing
      READ TABLE me->custom_fields WITH KEY molga = zmhp_cl_int_constants=>star field = field INTO field_customizing.
    ENDIF.
...
ENDMETHOD.
  ...
  METHOD get_customizing_conversion.
    ...
    IF me->custom_fields IS NOT INITIAL AND me->custom_conversion IS INITIAL.
      ...
      SELECT * FROM zmhp_int_convers INTO TABLE me->custom_conversion WHERE molga = molga AND infty IN lr_infty.
...
METHOD get_customizing_fields.
  ...
  IF me->custom_folders IS NOT INITIAL AND me->custom_fields IS INITIAL.
    ...
    LOOP AT me->custom_folders ASSIGNING FIELD-SYMBOL(<folder>).
      SELECT * FROM zmhp_int_fields INTO TABLE @DATA(fields) WHERE object       = @<folder>-object       AND
      ...
      IF sy-subrc IS INITIAL.
        ...
        CLEAR fields.
      ELSE.
        SELECT * FROM zmhp_int_fields INTO TABLE fields WHERE object       = <folder>-object       AND
                                                              object_seqnr = <folder>-object_seqnr AND
  ...
  ENDIF.
      lr_infty = VALUE #( FOR infty IN me->customizing ( sign = 'I' option = 'EQ' low = infty-infty ) ).
*      SELECT * FROM zmhp_int_fields INTO TABLE me->custom_fields WHERE object = me->object_type AND object_seqnr = me->object_seqnr AND molga = molga AND infty IN lr_infty.
*      IF sy-subrc IS INITIAL.
*        customizing = me->custom_fields.
*        customizing_exists = abap_true.
*      ELSE.
*        SELECT * FROM zmhp_int_fields INTO TABLE me->custom_fields WHERE object = me->object_type AND object_seqnr = me->object_seqnr AND molga = '*' AND infty IN lr_infty.
*        IF sy-subrc IS INITIAL.
*          customizing = me->custom_fields.
*          customizing_exists = abap_true.
*        ELSE.
...
*   Get conversion customizing
  IF me->custom_conversion IS INITIAL.
    ...
    IF sy-subrc    IS NOT INITIAL OR
      ...
      SELECT * FROM zmhp_int_convers INTO TABLE @DATA(int_conv_tmp) WHERE molga = '*' AND infty IN @lr_infty.
...
*   Get mapping customizing
  IF me->custom_mapping_fields IS INITIAL.
    SELECT * FROM zmhp_int_mapp_fi INTO TABLE me->custom_mapping_fields WHERE molga = molga AND infty IN lr_infty.

    IF sy-subrc    IS NOT INITIAL OR
       add_default EQ abap_true. "JMB20210709 I
      SELECT * FROM zmhp_int_mapp_fi INTO TABLE @DATA(int_map_fi_tmp) WHERE molga = '*' AND infty IN @lr_infty.
  ...
  IF me->custom_mapping_values IS INITIAL.
    SELECT * FROM zmhp_int_mapping INTO TABLE me->custom_mapping_values WHERE molga = molga AND infty IN lr_infty.

    IF sy-subrc    IS NOT INITIAL OR
       add_default EQ abap_true. "JMB20210709 I
      SELECT * FROM zmhp_int_mapping INTO TABLE @DATA(int_map_tmp) WHERE molga = '*' AND infty IN @lr_infty.
...
ENDMETHOD.
  ...
  METHOD GET_CUSTOMIZING_FOLDERS.
    ...
    IF me->customizing IS NOT INITIAL AND me->custom_folders IS INITIAL.
      lr_infty = VALUE #( FOR infty IN me->customizing ( sign = 'I' option = 'EQ' low = infty-infty ) ).
      SELECT * FROM zmhp_int_folders INTO TABLE me->custom_folders WHERE object = me->object_type AND object_seqnr = me->object_seqnr AND molga = molga AND infty IN lr_infty.
      IF sy-subrc IS INITIAL.
        customizing = me->custom_folders.
        customizing_exists = abap_true.
      ELSE.
        SELECT * FROM zmhp_int_folders INTO TABLE me->custom_folders WHERE object = me->object_type AND object_seqnr = me->object_seqnr AND molga = '*' AND infty IN lr_infty.
        IF sy-subrc IS INITIAL.
          customizing = me->custom_folders.
          customizing_exists = abap_true.
        ELSE.
  ...
  METHOD get_customizing_infty.
    IF me->customizing IS INITIAL.
      customizing_exists = abap_true.
      SELECT * FROM zmhp_int_infotyp INTO TABLE @me->customizing WHERE object       = @me->object_type  AND
...
*      lr_infty = VALUE #( FOR infty IN me->customizing ( sign = 'I' option = 'EQ' low = infty-infty ) ).
      LOOP AT me->customizing ASSIGNING FIELD-SYMBOL(<customizing>).
        SELECT * FROM zmhp_int_mapping INTO TABLE me->custom_mapping_values WHERE molga = molga AND infty EQ <customizing>-infty AND seqnr = <customizing>-seqnr.
        IF sy-subrc IS INITIAL.
  ...
  METHOD get_customizing_values.
    ...
    IF me->customizing IS NOT INITIAL AND me->custom_values IS INITIAL.
      lr_infty = VALUE #( FOR infty IN me->customizing ( sign = 'I' option = 'EQ' low = infty-infty ) ).
      SELECT * FROM zmhp_int_values INTO TABLE me->custom_values WHERE object = me->object_type AND molga = molga AND infty IN lr_infty.
      IF sy-subrc IS INITIAL.
        customizing = me->custom_values.
        customizing_exists = abap_true.
      ELSE.
        SELECT * FROM zmhp_int_values INTO TABLE me->custom_values WHERE object = me->object_type AND molga = '*' AND infty IN lr_infty.
        IF sy-subrc IS INITIAL.
          customizing = me->custom_values.
          customizing_exists = abap_true.
        ENDIF.
  ...
  METHOD get_instance.
    IF zmhp_cl_int_customizing_xml=>go_instance IS BOUND                   AND
       zmhp_cl_int_customizing_xml=>go_instance->molga = molga             AND
       zmhp_cl_int_customizing_xml=>go_instance->object_type = object_type.
      ro_instance = zmhp_cl_int_customizing_xml=>go_instance.
    ELSE.
      CREATE OBJECT zmhp_cl_int_customizing_xml=>go_instance
        EXPORTING
