class ZMHP_CL_INT_ACTION_HANDLER definition
  public
  final
  create public .
...
CLASS ZMHP_CL_INT_ACTION_HANDLER IMPLEMENTATION.
...
*    classdescr ?= cl_abap_typedescr=>describe_by_name( 'ZMHP_CL_INT_ACTION_HANDLER' ).
    DATA(custom_actionhandler) = zmhp_cl_int_general_settings=>get_actionhandler( ).
*   Get exception class description
    ...
    DATA(actions) = VALUE zmhp_tt_it_aend( FOR action IN new_it_aend WHERE ( infty = zmhp_cl_int_constants=>it0000 ) ( action ) ).
...
*        IF sy-subrc IS INITIAL.
*          CALL METHOD zmhp_cl_int_action_handler=>(method)
*            EXPORTING
*              action      = <action>
*            IMPORTING
*              has_error   = has_error
...
*              new_it_aend = new_it_aend.
*        ELSE.
        zmhp_cl_int_action_handler=>process_general(
          EXPORTING
            action = <action>
          IMPORTING
            has_error   = has_error
