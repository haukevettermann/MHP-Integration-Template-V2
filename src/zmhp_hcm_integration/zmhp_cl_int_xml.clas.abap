class ZMHP_CL_INT_XML definition
  public
  final
  create public .
...
public section.
  ...
  types:
    tt_customizing TYPE TABLE OF zmhp_int_fields .
...
CLASS ZMHP_CL_INT_XML IMPLEMENTATION.
...
*       LAST_LINE    = 0
  ...
  METHOD create_xml_interface.
    ...
    CATCH CX_ECATT_APL_XML.
      ...
      message-number = zmhp_cl_int_constants=>msg_no-m21.
    ENDTRY.
  ENDMETHOD.
...
*   Check if folder is global object folder
    ...
    IF folder_object_name = folder_xpath_name.
      ...
      zmhp_cl_int_xml=>get_xpath_element(
        EXPORTING
          element_xpath = zmhp_cl_int_constants=>xpath_begda
          xml_node      = xml_document
        IMPORTING
          value         = DATA(value)
          message       = message ).
      IF value IS NOT INITIAL.
        DATA(value_out) = CONV zmhp_dd_value( '' ).
        ...
        CALL METHOD (conversion_handler)=>convert_date
          ...
          RECEIVING
            value_out = value_out.
        <node>-begda = value_out.
        CLEAR value_out.
*        <node>-begda = zmhp_cl_int_conversion=>convert_date( value_in = CONV #( value ) ).
      ELSE.
        <node>-begda = zmhp_cl_int_constants=>lowdate.
      ENDIF.
      zmhp_cl_int_xml=>get_xpath_element(
        EXPORTING
          element_xpath = zmhp_cl_int_general_settings=>get_xpath_endda( )
          xml_node      = xml_document
        IMPORTING
          value         = value
          message       = message ).
      IF value IS NOT INITIAL.
        conversion_handler = zmhp_cl_int_general_settings=>get_conversion_handler( ).
        CALL METHOD (conversion_handler)=>convert_date
          EXPORTING
            value_in  = CONV zmhp_dd_value( value )
          RECEIVING
            value_out = value_out.
        <node>-endda = value_out.
        CLEAR value_out.
*        <node>-endda = zmhp_cl_int_conversion=>convert_date( value_in = CONV #( value ) ).
      ELSE.
        <node>-endda = zmhp_cl_int_constants=>highdate.
      ENDIF.
    ELSE.
      lr_processor->set_expression( expression = CONV #( folder_xpath ) ).
      lr_processor->run( progname = space ).
...
*       In case of job info or employment data only process primary assignment.
          IF folder_xpath = zmhp_cl_int_constants=>folders-employmentdata OR folder_xpath = zmhp_cl_int_constants=>folders-jobinformation.
            zmhp_cl_int_xml=>get_xpath_element(
              EXPORTING
                element_xpath = COND #( WHEN folder_xpath = zmhp_cl_int_constants=>folders-employmentdata THEN zmhp_cl_int_constants=>primary
                                        WHEN folder_xpath = zmhp_cl_int_constants=>folders-jobinformation THEN zmhp_cl_int_constants=>primaryassignment )
                xml_node      = lo_node
              IMPORTING
                value         = value
                message       = message ).
          ...
          zmhp_cl_int_xml=>get_xpath_element(
            EXPORTING
              element_xpath = zmhp_cl_int_general_settings=>get_xpath_begda( )
              xml_node      = lo_node
            IMPORTING
              value         = value
              message       = message ).
