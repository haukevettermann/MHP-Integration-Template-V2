class ZMHP_CL_INT_OBJECT_HANDLER definition
  public
  final
  create public .
...
public section.
  ...
  types S_INFTY_INSTANCES type ref to ZMHP_CL_INT_INFTY_PROC_XML .
  types:
  ...
  data CLOUD_PERNR type ZMHP_DD_OBJECTNUMBER .
  ...
  data PRELP_TAB type ZMHP_TT_PRELP_TAB .
  data PRELP_TAB_ORIG type ZMHP_TT_PRELP_TAB .
  data PRELP_TAB_NOCHNG type ZMHP_TT_PRELP_TAB .
...
CLASS ZMHP_CL_INT_OBJECT_HANDLER IMPLEMENTATION.
  ...
  METHOD process_customzing.
    LOOP AT me->go_customizing->customizing ASSIGNING FIELD-SYMBOL(<infotype>).
      DATA(status_handler) = zmhp_cl_int_status_handler=>get_instance( sap_id      = CONV #( me->sap_id )
...
*      DATA(lo_infotype) = NEW zmhp_cl_int_infty_proc_xml( field_customizing = infty_customizing_folder xml_node = me->xml_node infty = <infotype>-infty ).

      DATA(go_infotype) = NEW zmhp_cl_int_infty_proc_xml( customizing = me->go_customizing
...
*     Make sure actions are assigned to correct infotypes
    zmhp_cl_int_action_handler=>process_action(
      IMPORTING
        has_error   = DATA(has_error)
      CHANGING
        new_it_aend = status_handler->new_it_aend ).
    zmhp_cl_int_statics=>ora_bal  = CORRESPONDING #( BASE ( zmhp_cl_int_statics=>ora_bal ) status_handler->ora_bal ). "JMB20211017 I
    zmhp_cl_int_statics=>it_aend  = CORRESPONDING #( BASE ( zmhp_cl_int_statics=>it_aend ) status_handler->new_it_aend ).
    zmhp_cl_int_statics=>om_aend  = CORRESPONDING #( BASE ( zmhp_cl_int_statics=>om_aend ) status_handler->new_om_aend ).
    zmhp_cl_int_statics=>messages = CORRESPONDING #( BASE ( zmhp_cl_int_statics=>messages ) status_handler->new_log_messages ).
*     persist data
    IF zmhp_cl_int_statics=>test_run = abap_false.
      status_handler->persist_data( source = zmhp_cl_int_constants=>serializer ).
    ENDIF.
  ...
  METHOD read_customizing.
    DATA(ok) = abap_true.
    me->go_customizing = zmhp_cl_int_customizing_xml=>get_instance(
      EXPORTING
    ...
    IF ok IS INITIAL.
      message-id = zmhp_cl_int_constants=>msg_class_int.
      message-type = zmhp_cl_int_constants=>error.
      message-number = 000."Kein Infotyp customizing
      message-message_v1 = me->molga.
      message-message_v2 = zmhp_cl_int_constants=>person.
    ELSE.
