...
*&
*&---------------------------------------------------------------------*
REPORT zmhp_int_clear_aend_tab.
...
IF p_rbpa = abap_true.
  SELECT * FROM zmhp_int_it_aend INTO TABLE @DATA(it_aend_orig) WHERE int_run = @p_intrun. "AND status = @p_stat.
ELSE.
  SELECT * FROM zmhp_int_om_aend INTO TABLE @DATA(om_aend_orig) WHERE int_run = @p_intrun. "AND status = @p_stat.
ENDIF.
WHILE lines( it_aend_orig ) > 0 OR lines( om_aend_orig ) > 0.
  ...
  IF p_test = abap_false.
    ...
    IF om_aend IS NOT INITIAL.
      DELETE zmhp_int_om_aend FROM TABLE om_aend.
    ENDIF.
    COMMIT WORK.
