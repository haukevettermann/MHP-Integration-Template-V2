...
* Get physical path
*IF p_rbpa = abap_true.
*  SELECT * FROM zmhp_int_it_aend INTO TABLE @DATA(it_aend) WHERE pernr = @p_sapid AND cloud_id = @p_cldid AND int_run = @p_intrun.
*ELSE.
*  SELECT * FROM zmhp_int_om_aend INTO TABLE @DATA(om_aend) WHERE sap_id = @p_sapid AND cloud_id = @p_cldid AND int_run = @p_intrun.
*ENDIF.
*
...
*SELECT * FROM zmhp_int_msg_p INTO TABLE @DATA(msg_p) WHERE aend_id IN @aend_id_range AND int_run = @p_intrun AND sap_id = @p_sapid AND cloud_id = @p_cldid.
