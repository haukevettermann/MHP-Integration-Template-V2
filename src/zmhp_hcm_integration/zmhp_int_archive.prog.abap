...
* Get physical path
*DATA(logical_filename) = COND filename-fileintern( WHEN sy-sysid = 'D02' AND p_rarch = abap_false THEN 'ZMHP_HCM_ORACLE_EXTRACTS_DEV'
*                                                   WHEN sy-sysid = 'D02' AND p_rarch = abap_true THEN 'ZMHP_HCM_ORACLE_EXTRACTS_ARCHIVE_DEV'
*                                                   WHEN sy-sysid = 'Q02' AND p_rarch = abap_false THEN 'ZMHP_HCM_ORACLE_EXTRACTS_QS'
*                                                   WHEN sy-sysid = 'Q02' AND p_rarch = abap_true THEN 'ZMHP_HCM_ORACLE_EXTRACTS_ARCHIVE_QS'
*                                                   WHEN sy-sysid = 'P02' AND p_rarch = abap_true THEN 'ZMHP_HCM_ORACLE_EXTRACTS_PROD'
*                                                   WHEN sy-sysid = 'P02' AND p_rarch = abap_true THEN 'ZMHP_HCM_ORACLE_EXTRACTS_ARCHIVE_PROD' ).
...
* get physical path
      DATA(logical_filename2) = COND filename-fileintern( WHEN sy-sysid = 'D02' AND p_rarch = abap_false THEN 'ZMHP_HCM_ORACLE_EXTRACTS_ARCHIVE_DEV'
                                                         WHEN sy-sysid = 'D02' AND p_rarch = abap_true THEN 'ZMHP_HCM_ORACLE_EXTRACTS_DEV'
                                                         WHEN sy-sysid = 'Q02' AND p_rarch = abap_false THEN 'ZMHP_HCM_ORACLE_EXTRACTS_ARCHIVE_QS'
                                                         WHEN sy-sysid = 'Q02' AND p_rarch = abap_true THEN 'ZMHP_HCM_ORACLE_EXTRACTS_QS'
                                                         WHEN sy-sysid = 'P02' AND p_rarch = abap_true THEN 'ZMHP_HCM_ORACLE_EXTRACTS_ARCHIVE_PROD'
                                                         WHEN sy-sysid = 'P02' AND p_rarch = abap_true THEN 'ZMHP_HCM_ORACLE_EXTRACTS_PROD' ).
      CALL FUNCTION 'FILE_GET_NAME'
        EXPORTING
