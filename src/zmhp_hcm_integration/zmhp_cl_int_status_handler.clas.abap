class ZMHP_CL_INT_STATUS_HANDLER definition
  public
  create public .

public section.

  types S_INSTANCES type ref to ZMHP_CL_INT_STATUS_HANDLER .
  types:
    t_fields TYPE TABLE OF zmhp_int_msg_f .
  types:
    t_instances TYPE TABLE OF s_instances WITH DEFAULT KEY .
  types:
    t_log TYPE TABLE OF zmhp_int_msg_l .
  types:
    t_proc TYPE TABLE OF zmhp_int_msg_p .
  types:
    BEGIN OF ty_field,
        infty     TYPE infty,
        subty     TYPE subty,
  ...
  data IT_AEND_ACTION type ZMHP_TT_IT_AEND .
  data OM_AEND type ZMHP_TT_OM_AEND .
  data LOG_MESSAGES type T_LOG .
  data CHANGED_VALUES type T_FIELDS .
  data SAP_ID type ZMHP_DD_VALUE .
  data CLOUD_ID type ZMHP_DD_VALUE .
  data CLOUD_PERNR type ZMHP_DD_VALUE .
  ...
  data NEW_IT_AEND type ZMHP_TT_IT_AEND .
  data NEW_OM_AEND type ZMHP_TT_OM_AEND .
  data SIMU_IT_AEND type ZMHP_TT_IT_AEND .
  data SIMU_OM_AEND type ZMHP_TT_OM_AEND .
  ...
  data DEL_IT_AEND type ZMHP_TT_IT_AEND .
  data DEL_OM_AEND type ZMHP_TT_OM_AEND .
  data OBJECT_SEQ type SEQNR .
  data ORA_BAL type ZMHP_TT_ORA_BAL .
  ...
  methods GET_PROCESSOR
    importing
      !BEGDA type DATS
      !ENDDA type DATS
      !LOG type ZMHP_INT_MSG_L
    exporting
      !MSG_P type T_PROC .
  methods GET_LATEST_STATUS
    returning
      value(STATUS) type ZMHP_DD_STATUS .
  ...
  methods SET_FIELD_CHANGE
    importing
      ...
      !VALUE_OLD type ZMHP_DD_VALUE
      !VALUE_NEW type ZMHP_DD_VALUE .
  methods SET_STATUS
    importing
      value(STATUS) type ZMHP_DD_STATUS
      !AEND_ID type GUID_32 .
  ...
  methods ENHANCE_ENTRIES
    ...
    changing
      !IT_AEND type ZMHP_INT_IT_AEND optional
      !OM_AEND type ZMHP_INT_OM_AEND optional
      !PROC_MSG type ZMHP_INT_MSG_P optional .
  methods ADD_CHANGED_FIELDS
    importing
      !AEND_ID type GUID_32
      !INFTY type INFTY
      !FIELD type ZMHP_DD_FIELD
      !SUBTY type SUBTY
      !VALUE_OLD type ZMHP_DD_VALUE
      !VALUE_NEW type ZMHP_DD_VALUE
      !VALUE_CHANGED type ZMHP_DD_VALUE optional
  ...
  methods CONSTRUCTOR
    importing
      !SAP_ID type ZMHP_DD_VALUE optional
      !CLOUD_ID type ZMHP_DD_VALUE optional
      !CLOUD_PERNR type ZMHP_DD_VALUE optional
  ...
  class-methods GET_INSTANCE
    importing
      ...
      !MOLGA type MOLGA
    returning
      value(INSTANCE) type ref to ZMHP_CL_INT_STATUS_HANDLER .
  class-methods GET_ALL_INSTANCES
    returning
      value(INSTANCES) type T_INSTANCES .
  methods ADD_NEW_AEND
    importing
      !ORA_BAL type ZMHP_INT_ORA_BAL optional
    changing
      !OM_AEND type ZMHP_INT_OM_AEND optional
      !IT_AEND type ZMHP_INT_IT_AEND optional .
  ...
  methods MAP_BAPIRET1
    ...
    changing
      !MESSAGE_LOG type ZMHP_INT_MSG_L .
...
CLASS ZMHP_CL_INT_STATUS_HANDLER IMPLEMENTATION.
  ...
  METHOD add_changed_fields.
    ...
    <changed_value>-field_id = zmhp_cl_int_status_handler=>generate_guid( ).
...
*     Map bapiret fields to log structure
        ...
        <message>-msg_guid = zmhp_cl_int_status_handler=>generate_guid( ).
*        IF simu = abap_true.
...
ENDMETHOD.
  ...
  METHOD check_for_error.
    IF type = zmhp_cl_int_constants=>person.
      SELECT SINGLE * FROM zmhp_int_it_aend INTO @DATA(it_aend) WHERE cloud_id = @cloud_id AND pernr = @sap_id
                                      AND status = @zmhp_cl_int_constants=>booking_status-error AND int_run NE @int_run.
      IF it_aend IS NOT INITIAL.
        has_error = abap_true.
      ENDIF.
      SELECT SINGLE * FROM zmhp_int_om_aend INTO @DATA(om_aend_check_pa) WHERE ( status = @zmhp_cl_int_constants=>booking_status-initial "OR status = @zmhp_cl_int_constants=>booking_status-error_des
                                    OR status = @zmhp_cl_int_constants=>booking_status-error ) AND ext_type = 'DT'.
      IF om_aend_check_pa IS NOT INITIAL.
        has_error = abap_true.
      ENDIF.
    ELSEIF type = zmhp_cl_int_constants=>position OR type = zmhp_cl_int_constants=>orgunit.
      IF ext_type = 'D'.
        SELECT SINGLE * FROM zmhp_int_om_aend INTO @DATA(om_aend) WHERE cloud_id = @cloud_id AND sap_id = @sap_id AND ext_type = @ext_type
                                        AND status = @zmhp_cl_int_constants=>booking_status-error AND int_run NE @int_run.
        IF om_aend IS NOT INITIAL.
          has_error = abap_true.
        ENDIF.
      ELSEIF ext_type = 'DT'.
        SELECT SINGLE * FROM zmhp_int_om_aend INTO @DATA(om_aend_dt) WHERE cloud_id = @cloud_id AND sap_id = @sap_id AND ext_type = @ext_type
                                        AND status = @zmhp_cl_int_constants=>booking_status-error AND int_run NE @int_run.
        IF om_aend_dt IS NOT INITIAL.
          has_error = abap_true.
        ENDIF.
        SELECT SINGLE * FROM zmhp_int_om_aend INTO @DATA(om_aend_check_dt) WHERE ( status = @zmhp_cl_int_constants=>booking_status-initial "OR status = @zmhp_cl_int_constants=>booking_status-error_des
                                      OR status = @zmhp_cl_int_constants=>booking_status-error ) AND ext_type = 'D' AND subty ne 'B012'.
...
* Serializer logic without ranges
  IF sap_id IS NOT INITIAL OR cloud_id IS NOT INITIAL OR int_run IS NOT INITIAL OR molga IS NOT INITIAL.
    ...
    SELECT * FROM zmhp_int_it_aend WHERE int_run = @int_run
                                     AND pernr = @sap_id
                                     AND cloud_id = @cloud_id
                                     AND molga = @molga
             INTO TABLE @me->it_aend.

    SELECT * FROM zmhp_int_om_aend WHERE int_run = @int_run
                                     AND sap_id = @sap_id
                                     AND cloud_id = @cloud_id
                                     AND molga = @molga
...
* Booking report logic with ranges
  ELSE.
    ...
    SELECT * FROM zmhp_int_it_aend WHERE pernr    IN @sap_id_rng
...
*             INTO TABLE @me->it_aend_action.
    ...
    IF it_aend_only = abap_false.
      SELECT * FROM zmhp_int_om_aend WHERE sap_id   IN @sap_id_rng
                                       AND status   IN @status_rng_int
                                       AND int_run  IN @int_run_rng
                                       AND cloud_id IN @cloud_id_rng
                                       AND ( begda  IN @dates_rng OR
...
* Select corresponding messages
  IF range_aend_id IS NOT INITIAL.
    SELECT * FROM zmhp_int_msg_l WHERE aend_id IN @range_aend_id INTO TABLE @me->log_messages.
  ENDIF.

* Select corresponding field values
  IF range_aend_id IS NOT INITIAL.
    SELECT * FROM zmhp_int_msg_f WHERE aend_id IN @range_aend_id INTO TABLE @me->changed_values.
  ENDIF.
...
ENDMETHOD.
  ...
  METHOD delete_old_records.
    ...
    IF me->it_aend IS NOT INITIAL.
      LOOP AT me->it_aend INTO DATA(record_pa). "WHERE status = zmhp_cl_int_constants=>booking_status-initial.
        CLEAR: aend_id.
    ...
    IF aend_id_range IS NOT INITIAL.
      ...
      SELECT * FROM zmhp_int_msg_p INTO TABLE @DATA(msg_p) WHERE aend_id IN @aend_id_range.
  ...
  METHOD enhance_entries.
    IF it_aend IS SUPPLIED.
*      DATA(data_transform) = zmhp_cl_int_data_transform=>get_instance( ).
...
*      it_aend-aend_id = zmhp_cl_int_utility=>create_guid( ).
      ...
      IF it_aend-status IS INITIAL.
        it_aend-status = zmhp_cl_int_constants=>booking_status-initial.
      ENDIF.
      IF it_aend-operation IS INITIAL.
        IF me->sap_id IS INITIAL.
          it_aend-operation = zmhp_cl_int_constants=>infty_operation-pa_insert.
        ENDIF.
        IF me->sap_id IS INITIAL OR me->sap_id = '00000000'.
          it_aend-operation = zmhp_cl_int_constants=>infty_operation-pa_insert.
        ELSEIF me->sap_id IS NOT INITIAL AND it_aend-endda = zmhp_cl_int_constants=>highdate.
          it_aend-operation = zmhp_cl_int_constants=>infty_operation-pa_insert.
        ENDIF.
        IF it_aend-endda NE zmhp_cl_int_constants=>highdate.
          IF it_aend-action = zmhp_cl_int_constants=>termination AND it_aend-infty = zmhp_cl_int_constants=>it0000.
            it_aend-operation = zmhp_cl_int_constants=>infty_operation-pa_insert.
          ELSE.
            IF it_aend-infty = zmhp_cl_int_constants=>it0002 OR it_aend-infty = '9402'.
    ...
    ENDIF.
    IF om_aend IS SUPPLIED.
*      data_transform = zmhp_cl_int_data_transform=>get_instance( ).
      om_aend-int_run = me->int_run.
      om_aend-sap_id = me->sap_id.
      om_aend-cloud_id = me->cloud_id.
*      om_aend-aend_id = zmhp_cl_int_utility=>create_guid( ).
      om_aend-molga = me->molga.
      om_aend-legal_entity = me->bukrs.
      om_aend-istat = '1'.
      om_aend-plvar = zmhp_cl_int_constants=>plvar.
      om_aend-uname = sy-uname.
      om_aend-aedtm = sy-datum.
      IF om_aend-otype = zmhp_cl_int_constants=>position.
        om_aend-ext_type = zmhp_cl_int_constants=>position.
      ELSE.
      ...
      IF om_aend-status IS INITIAL.
        om_aend-status = zmhp_cl_int_constants=>booking_status-initial.
      ENDIF.
      IF om_aend-operation IS INITIAL.
        IF me->sap_id IS INITIAL OR me->sap_id = '00000000'.
          om_aend-operation = zmhp_cl_int_constants=>infty_operation-om_insert.
        ELSEIF me->sap_id IS NOT INITIAL AND om_aend-endda = zmhp_cl_int_constants=>highdate.
          om_aend-operation = zmhp_cl_int_constants=>infty_operation-om_modify.
        ENDIF.
        IF om_aend-endda NE zmhp_cl_int_constants=>highdate.
          om_aend-operation = zmhp_cl_int_constants=>infty_operation-om_delimit.
        ENDIF.
      ENDIF.
    ENDIF.
...
*      proc_msg-molga = me->molga.
      ...
      proc_msg-processor_guid = zmhp_cl_int_utility=>create_guid( ).
...
*      proc_msg-status = '01'.
  ...
  METHOD get_all_instances.
    instances = zmhp_cl_int_status_handler=>instances.
  ENDMETHOD.


  METHOD get_changed_fields.
    DATA: msg_field      TYPE zmhp_int_msg_f,
          msg_fields     TYPE zmhp_tt_msg_f,
...
*    CREATE DATA lr_struc_new TYPE HANDLE lr_structdescr.
    ...
    LOOP AT fields ASSIGNING FIELD-SYMBOL(<fs_field>). "INTO field.
      me->add_changed_fields( aend_id = aend_id
                              ...
                              editable = zmhp_cl_int_customizing_xml=>get_instance(
                                           molga = me->molga
                                           object_type = otype
...
*    me->save_changed_fields( EXPORTING msg_fields = msg_fields IMPORTING is_ok = DATA(is_ok) ).
  ...
  METHOD get_instance.
    ...
    IF instance IS NOT BOUND.
      instance = NEW zmhp_cl_int_status_handler(
        sap_id   = sap_id
        cloud_id = cloud_id
...
*    select * from zmhp_int_om_aend where status = zmhp_cl_int_constants=>error.
  ENDMETHOD.
  ...
  METHOD get_processor.
    ...
    IF me->it_aend IS NOT INITIAL.
      IF me->sap_id IS NOT INITIAL.
        ...
        IF it0001_tab IS NOT INITIAL.
          ...
          IF no_sachx = abap_false.
            ...
            SELECT * FROM zmhp_int_superus INTO TABLE @DATA(superusers) WHERE sbgrp_code = @<msg_p>-sbgrp_code AND sb_code = @<msg_p>-sb_code .
        ...
        ELSE.
          bukrs_select = '*'.
        ENDIF.
        SELECT * FROM zmhp_int_superus INTO TABLE superusers WHERE bukrs = bukrs_select.
      ...
      IF superusers IS INITIAL.
        SELECT * FROM zmhp_int_superus INTO TABLE superusers WHERE bukrs = '*' AND sbgrp_code = '*' AND sb_code = '*'.
        LOOP AT superusers ASSIGNING <superuser>.
          APPEND INITIAL LINE TO msg_p ASSIGNING <msg_p>.
          <msg_p>-user_name = <superuser>-uname.
          me->enhance_entries( CHANGING proc_msg = <msg_p> ).
...
*   OM_AEND logic
  ...
  METHOD map_bapiret1.
    ...
    IF message_log-message IS INITIAL.
      zmhp_cl_int_utility=>get_message( EXPORTING msgid = message_log-id msgno = message_log-num msgv1 = message_log-message_v1 msgv2 = message_log-message_v2
  ...
  METHOD persist_data.
    CASE source.
        "Serializer
      WHEN zmhp_cl_int_constants=>serializer.
        is_ok = me->persist_data_serializer( ).
        "IT Poster
      WHEN zmhp_cl_int_constants=>poster.
        is_ok = me->persist_data_poster( ).
        "INT Cockpit
      WHEN zmhp_cl_int_constants=>cockpit.
    ...
    IF is_ok = abap_true.
      IF simu = abap_true.
        ...
        IF me->simu_it_aend IS NOT INITIAL.
          MODIFY zmhp_int_it_aend FROM TABLE me->simu_it_aend.
        ENDIF.
        IF me->simu_om_aend IS NOT INITIAL.
          MODIFY zmhp_int_om_aend FROM TABLE me->simu_om_aend.
        ENDIF.
      ENDIF.
      "If not a testrun we need to clear old values
      IF source = zmhp_cl_int_constants=>poster.
        me->delete_old_records( ).
      ENDIF.
      IF me->new_changed_values IS NOT INITIAL.
        MODIFY zmhp_int_msg_f FROM TABLE me->new_changed_values.
      ENDIF.
      IF me->new_log_messages IS NOT INITIAL.
        MODIFY zmhp_int_msg_l FROM TABLE me->new_log_messages.
      ENDIF.
      IF me->new_processor IS NOT INITIAL.
        MODIFY zmhp_int_msg_p FROM TABLE me->new_processor.
      ENDIF.
    ENDIF.
...
*    ENDIF.
  ...
  METHOD persist_data_cockpit.
    ...
    IF me->new_it_aend IS NOT INITIAL.
      MODIFY zmhp_int_it_aend FROM TABLE me->new_it_aend.
      IF sy-subrc IS INITIAL.
        is_ok = abap_true.
      ENDIF.
    ENDIF.
    IF me->it_aend IS NOT INITIAL.
      MODIFY zmhp_int_it_aend FROM TABLE me->it_aend.
      IF sy-subrc IS INITIAL.
        is_ok = abap_true.
      ENDIF.
    ENDIF.
    ...
    IF me->new_om_aend IS NOT INITIAL.
      MODIFY zmhp_int_om_aend FROM TABLE me->new_om_aend.
      IF sy-subrc IS INITIAL.
        is_ok = abap_true.
      ENDIF.
    ENDIF.
    IF me->om_aend IS NOT INITIAL.
      MODIFY zmhp_int_om_aend FROM TABLE me->om_aend.
      IF sy-subrc IS INITIAL.
        is_ok = abap_true.
      ENDIF.
    ENDIF.
  ...
  METHOD persist_data_poster.
    ...
    IF me->del_it_aend IS NOT INITIAL.
      DELETE zmhp_int_it_aend FROM TABLE me->del_it_aend.
      IF sy-subrc IS INITIAL.
        is_ok = abap_true.
      ENDIF.
    ENDIF.
    ...
    IF me->del_om_aend IS NOT INITIAL.
      DELETE zmhp_int_om_aend FROM TABLE me->del_om_aend.
      IF sy-subrc IS INITIAL.
        is_ok = abap_true.
      ENDIF.
    ENDIF.
...
*
  IF me->ora_bal IS NOT INITIAL.
    is_ok = NEW zmhp_cl_int_ora_bal( )->insert( me->ora_bal ).
  ENDIF.
*JMB20211017 insert end
...
*   Set status it_aend
    DATA(it_aend) = VALUE zmhp_int_it_aend( me->it_aend[ aend_id = aend_id ] OPTIONAL ).
    IF it_aend IS INITIAL.
      DATA(bapiret1) = VALUE bapiret1( type = 'E' id = zmhp_cl_int_constants=>msg_class_int number = 004 ).
      me->add_log_message(
        EXPORTING
          aend_id      = aend_id
          bapiret1     = bapiret1 ).
...
*       Transform to it_aend
        ...
        it_aend-status = zmhp_cl_int_constants=>booking_status-manual.
        APPEND it_aend TO me->new_it_aend.
*       Kein neuer Eintrag. Update des bestehenden.
        APPEND changed_field TO me->new_changed_values.
      ELSE.
        bapiret1 = VALUE bapiret1( type = 'E' id = zmhp_cl_int_constants=>msg_class_int number = 005 ).
        me->add_log_message(
          EXPORTING
            aend_id      = aend_id
            bapiret1     = bapiret1 ).
...
*   Set status it_aend
    ...
    ENDIF.
*   Set status om_aend
    DATA(om_aend) = VALUE zmhp_int_om_aend( me->om_aend[ aend_id = aend_id ] OPTIONAL ).
    IF om_aend IS NOT INITIAL.
