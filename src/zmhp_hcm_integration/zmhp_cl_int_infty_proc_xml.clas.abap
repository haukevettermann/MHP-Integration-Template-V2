class ZMHP_CL_INT_INFTY_PROC_XML definition
  public
  final
  create public .
...
public section.
  ...
  methods BUILD_INFOTYPE_OLD
    importing
      !SAP_ID type HROBJID
      !CLOUD_ID type ZMHP_DD_OBJECTID
  ...
  methods DETERMINE_TIME_SLICES_FOLDER
    importing
      !FOLDER type ZMHP_CL_INT_INFTY_PROC_XML=>S_FOLDER
    exporting
      !MESSAGE type BAPIRET1
      !TIME_SLICES type T_TIME_SLICES
    returning
  ...
  methods CONSTRUCTOR
    importing
      !CUSTOMIZING type ref to ZMHP_CL_INT_CUSTOMIZING_XML
  ...
  methods PROCESS_FOLDER_CUSTOMIZING
    importing
      !CUSTOMIZING type ZMHP_CL_INT_CUSTOMIZING_XML=>S_CUSTOMIZING_FOLDERS
      !PARENT_FOLDER_SEQNR type SEQNR
      !FOLDER_XML_NODE type ref to IF_IXML_NODE
      !PARENT_FOLDER type ZMHP_DD_FOLDER
    exporting
      !MESSAGE type BAPIRET1
    changing
      !SEQNR type SEQNR .
...
CLASS ZMHP_CL_INT_INFTY_PROC_XML IMPLEMENTATION.


  METHOD action_handling.
    DATA(status_handler) = zmhp_cl_int_status_handler=>get_instance( sap_id      = CONV #( sap_id )
...
*   Loop at actions. Could be more than one...
    ...
    ENDCASE.
*    LOOP AT status_handler->new_it_aend WHERE infty EQ zmhp_cl_int_constants=>it0000.
**     Assign infotypes
*      LOOP AT status_handler->new_it_aend WHERE infty NE zmhp_cl_int_constants=>it0000.
*
*      ENDLOOP.
*    ENDLOOP.
...
*       Aggregate folders that are not relevant for timeslice
  ...
  METHOD build_infotype.
    DATA:
      ...
      aend_id         TYPE zmhp_value,
      converted_value TYPE zmhp_dd_value,
    ...
    LOOP AT me->folders_aggregated ASSIGNING FIELD-SYMBOL(<folder_aggregated>).
      ...
      me->infotype_changes = NEW zmhp_cl_int_it_aend( ).
...
*--------------------------------------------------------------------*
      ...
      LOOP AT time_slices ASSIGNING FIELD-SYMBOL(<time_slice>).
        ...
        aend_id = zmhp_cl_int_status_handler=>generate_guid( ).
        APPEND INITIAL LINE TO <ft_table> ASSIGNING FIELD-SYMBOL(<structure>).

        "First read begda and endda as they can never be initial.
        ASSIGN COMPONENT zmhp_cl_int_constants=>begda OF STRUCTURE <structure> TO FIELD-SYMBOL(<value>).
        ...
        ENDIF.
*     Fill field infotype
        ASSIGN COMPONENT zmhp_cl_int_constants=>infty OF STRUCTURE <structure> TO <value>.
        <value> = me->infty.
...
*       Get customizing instance in order to process mappings and conversions
          DATA(customizing_instance) = zmhp_cl_int_customizing_xml=>get_instance( object_type = object_type molga = molga ).
...
*           Set status. If already error, do not overwrite.
            IF message-type = zmhp_cl_int_constants=>error.
              has_error = abap_true.
            ENDIF.
...
*         Convert Dates (begda and endda) to SAP Format
          ...
          IF sy-subrc = 0.
            zmhp_cl_int_conversion=>get_instance( molga = me->molga ).    "JMB20211016 I
            TRY.
                DATA(conversion_handler) = zmhp_cl_int_general_settings=>get_conversion_handler( ).
                CALL METHOD (conversion_handler)=>(<conversion>-conversion_method)
                  EXPORTING
                    value_in  = CONV zmhp_dd_value( value )
                  RECEIVING
                    value_out = value.
...
*--------------------------------------------------------------------*
        DATA(fallback_values) = VALUE zmhp_cl_int_customizing_xml=>t_customizing_values( FOR fallback_value IN customizing->custom_values WHERE ( infty = infty ) ( fallback_value ) ).
...
*                   <ft_table> TYPE STANDARD TABLE.
**   Instantiate Status Handler
*    DATA(status_handler) = zmhp_cl_int_status_handler=>get_instance(
*                             sap_id   = CONV #( sap_id )
*                             cloud_id = CONV #( cloud_id )
*                             int_run  = int_run
*                             molga    = molga ).
...
*                CHANGING
*                value       = value ).
*          DATA(has_error) = COND boole_d( WHEN message-type = zmhp_cl_int_constants=>error THEN abap_true ELSE abap_false ).
...
**            IF <infty_value>-infty = zmhp_cl_int_constants=>it0000
**         Log message
*            status_handler->add_log_message( EXPORTING aend_id = CONV #( aend_id ) bapiret1 = message ).
**            IF message-type = 'E'.
...
**                  subty      = VALUE #( infty_values[ field_sap = zmhp_cl_int_constants=>subty ]-value OPTIONAL )
...
*            customizing_exists = DATA(customizing_exists) ).
*
**        IF <infty_value>-field_sap = zmhp_cl_int_constants=>begda OR <infty_value>-field_sap = zmhp_cl_int_constants=>endda.
**         Convert Dates (begda and endda) to SAP Format
*        READ TABLE me->customizing->custom_conversion WITH KEY field_sap = <infty_value>-field_sap ASSIGNING FIELD-SYMBOL(<conversion>).
*        IF sy-subrc = 0.
*          CALL METHOD zmhp_cl_int_conversion=>(<conversion>-conversion_method)
*            EXPORTING
*              value_in  = CONV zmhp_dd_value( <infty_value>-value )
*            RECEIVING
...
**     Fill fallback values
**--------------------------------------------------------------------*
*      DATA(fallback_values) = VALUE zmhp_cl_int_customizing_xml=>t_customizing_values( FOR value IN customizing->custom_values WHERE ( infty = infty ) ( value ) ).
*      LOOP AT fallback_values ASSIGNING FIELD-SYMBOL(<fallback_value>).
*        ASSIGN COMPONENT <fallback_value>-field OF STRUCTURE <structure> TO <value>.
*        IF <value> IS ASSIGNED AND <value> IS INITIAL.
*          <value> = <fallback_value>-value.
...
*    ENDLOOP.
  ...
  METHOD determine_time_slices_folder.
    DATA(next_folders) = VALUE zmhp_cl_int_infty_proc_xml=>t_folders( FOR next_folder IN me->folders WHERE ( parent_folder = folder-folder AND parent_folder_seqnr = folder-seqnr ) ( next_folder ) ).
    IF sy-subrc IS INITIAL.
  ...
  METHOD determine_time_slices_old.
    ...
    IF infty NE zmhp_cl_int_constants=>it0105 AND infty NE zmhp_cl_int_constants=>it0001.
*        endloop.
...
*      me->time_slices = CORRESPONDING #( time_slices ).
    ELSEIF infty EQ zmhp_cl_int_constants=>it0001.
      LOOP AT me->folders ASSIGNING FIELD-SYMBOL(<folder>) WHERE parent_folder_seqnr = 000.
        DATA(new_folders) = VALUE zmhp_cl_int_infty_proc_xml=>t_folders( FOR folder IN me->folders WHERE (  seqnr NE 000 AND parent_folder_seqnr = <folder>-seqnr ) ( folder ) ).
        ...
        ENDLOOP.
      ENDLOOP.
    ELSEIF infty EQ zmhp_cl_int_constants=>it0105.
...
*          ENDIF.
          ...
          IF value IS INITIAL.
            SELECT * FROM zmhp_int_values INTO TABLE @DATA(fallback_values) WHERE object = @me->object_handler->object_type
                                                  AND molga = @me->molga
                                                  AND infty = @me->infty
                                                  AND field = @<field_customizing>-field.
            IF sy-subrc IS NOT INITIAL.
              SELECT * FROM zmhp_int_values INTO TABLE @fallback_values WHERE object = @me->object_handler->object_type
                                                                              AND molga = '*'
                                                                              AND infty = @me->infty
                                                                              AND field = @<field_customizing>-field.
  ...
  METHOD get_folders.
    me->customizing_folders =  VALUE zmhp_cl_int_customizing_xml=>t_customizing_folders( FOR folders IN me->customizing->custom_folders WHERE ( infty = me->infty AND seqnr = me->seqnr ) ( folders ) ).
*    me->customizing_folders =  VALUE zmhp_cl_int_customizing_xml=>t_customizing_fields( FOR field IN me->customizing->custom_fields WHERE ( infty = me->infty AND seqnr = me->seqnr ) ( field ) ).
...
*   Fill Dates for folders without date.
  ...
  METHOD get_sap_ids.
    ...
    IF otype = zmhp_cl_int_constants=>orgunit.
    ...
    ELSEIF otype = zmhp_cl_int_constants=>person.
    ...
    IF sy-subrc <> 0.
      message = VALUE bapiret1( type = zmhp_cl_int_constants=>warning
                          id = zmhp_cl_int_constants=>msg_class_int
                          number = zmhp_cl_int_constants=>msg_no-m14
                          message_v1 = cloud_id
                          message_v2 = otype ).
    ENDIF.
  ENDMETHOD.
...
*
  DATA(pt_obj) = VALUE rsdsselopt_t( ( sign = 'I' option = 'EQ' low = zmhp_cl_int_constants=>absence )
                                     ( sign = 'I' option = 'EQ' low = zmhp_cl_int_constants=>del_absence )
                                     ( sign = 'I' option = 'EQ' low = zmhp_cl_int_constants=>attendance )
                                     ( sign = 'I' option = 'EQ' low = zmhp_cl_int_constants=>del_attendance )
                                     ( sign = 'I' option = 'EQ' low = zmhp_cl_int_constants=>quota )
                                     ( sign = 'I' option = 'EQ' low = zmhp_cl_int_constants=>balance )
                                     ( sign = 'I' option = 'EQ' low = zmhp_cl_int_constants=>time_events ) ).
*JMB20211111 insert end

  IF object_type = zmhp_cl_int_constants=>person.
...
*     Check action and set instance attribute.
    IF me->infty = zmhp_cl_int_constants=>it0000.
...
*     In case of Hire position & position relation to orgunit need to be created
    ...
    ENDIF.
*     Assign pernr
    ASSIGN COMPONENT zmhp_cl_int_constants=>objid OF STRUCTURE <structure> TO <value>.
*    Read SAP ID to according Cloud ID from mapping IT
*    IF sap_id IS INITIAL OR sap_id = '00000000'.
*      me->get_sap_ids( EXPORTING otype = zmhp_cl_int_constants=>orgunit begda = begda endda = endda cloud_id = cloud_id
...
*  ENDIF.
*    <value> = sap_id_tmp.
    ASSIGN COMPONENT 'ORACLEID' OF STRUCTURE <structure> TO <value>.
    IF <value> IS INITIAL.
      <value> = me->object_handler->cloud_id.
    ENDIF.
...
*   Instantiate Status Handler
    DATA(om_aend) = CORRESPONDING zmhp_int_om_aend( wplog ).
    om_aend-aend_id = aend_id.
*   Set status of Department in case of inactivation.
    IF om_aend-infty = zmhp_cl_int_constants=>it1000.
      om_aend-department_status = VALUE #( me->fields[ field_sap = 'STATUS' ]-value OPTIONAL ).
    ENDIF.

    "Set related object
    IF om_aend-infty = zmhp_cl_int_constants=>it1001.
      om_aend-relat_obj = VALUE #( me->fields[ field_sap = 'RELAT_OBJ' ]-value OPTIONAL ).
    ENDIF.

    om_aend-status = COND #( WHEN has_error = abap_true THEN zmhp_cl_int_constants=>booking_status-error_des ELSE zmhp_cl_int_constants=>booking_status-initial ).
    zmhp_cl_int_status_handler=>get_instance(
                             sap_id   = CONV #( sap_id )
                             cloud_id = CONV #( cloud_id )
                             int_run  = me->int_run
...
*   Transform infotype entry to prelp
    IF infotype IS NOT INITIAL.
      zmhp_cl_int_type_cast=>pnnnn_to_wplog(
       EXPORTING
          pnnnn = infotype
          aend_id = aend_id
        IMPORTING
  ...
  METHOD pa_specific_logic.
    DATA:
      ...
      infotype_handler_class TYPE REF TO object,
      it_aend                TYPE zmhp_int_it_aend.
*    classdescr ?= cl_abap_typedescr=>describe_by_name( 'ZMHP_CL_INT_INFTY_PROC_XML' ).
    DATA(infotype_handler) = zmhp_cl_int_general_settings=>get_infotyphandler( ).

    CREATE OBJECT infotype_handler_class TYPE (infotype_handler).
...
*      CALL METHOD infotype_handler_class->(method)
        EXPORTING
          data                   = data
          begda                  = begda
          endda                  = endda
...
*          IF me->seqnr = 001 AND <action>-value IN zmhp_cl_int_constants=>non_relevant_action( ).
*            EXIT.
*          ENDIF.
*
*          ASSIGN COMPONENT zmhp_cl_int_constants=>massn OF STRUCTURE <structure> TO FIELD-SYMBOL(<value>).
...
*            IF me->object_handler->action NE zmhp_cl_int_constants=>termination.
*              me->object_handler->action = <value>.
*            ENDIF.
**      Set termination date for delimiting
*            IF me->object_handler->action = zmhp_cl_int_constants=>termination.
*              ASSIGN COMPONENT zmhp_cl_int_constants=>endda OF STRUCTURE <structure> TO <value>.
*              endda_term = <value>.
...
*        ENDIF.
      ...
      IF me->infty = zmhp_cl_int_constants=>it0050.
        ASSIGN COMPONENT zmhp_cl_int_constants=>bdegr OF STRUCTURE <structure> TO FIELD-SYMBOL(<value>).
        IF <value> IS ASSIGNED AND <value> IS INITIAL.
          <value> = '000'.
        ENDIF.
        ASSIGN COMPONENT zmhp_cl_int_constants=>grawg OF STRUCTURE <structure> TO <value>.
        IF <value> IS ASSIGNED AND <value> IS INITIAL.
          <value> = '000'.
        ENDIF.
      ENDIF.

      IF me->infty = zmhp_cl_int_constants=>it0105.
        ASSIGN COMPONENT zmhp_cl_int_constants=>subty OF STRUCTURE <structure> TO <value>.
        IF <value> IS ASSIGNED AND <value> IS INITIAL.
          EXIT.
        ENDIF.
      ENDIF.
...
*        IF me->infty NE zmhp_cl_int_constants=>it0000 AND me->object_handler->action = '04'.
*          ASSIGN COMPONENT zmhp_cl_int_constants=>endda OF STRUCTURE <structure> TO <value>.
...
*    DEVETHAU, 09.11.2021, Position generated in new method ZMHP_CL_INT_ACTIONS->PROCESS_01
*    "In case of Hire position & position relation to orgunit need to be created
*    IF me->infty                  = zmhp_cl_int_constants=>it0001 AND
*       me->object_handler->action = zmhp_cl_int_constants=>hire.
*      zmhp_cl_int_utility=>get_spras_by_molga( EXPORTING molga = me->molga
*                                               IMPORTING spras = DATA(spras)
*                                                         langu = DATA(langu) ).
*
*      DATA(infty_operation) = NEW zmhp_cl_int_it_operation( int_run = me->int_run
*                                                            molga   = me->molga ).
*
*      ASSIGN COMPONENT zmhp_cl_int_constants=>orgeh OF STRUCTURE <structure> TO <value>.
*
...
*      ASSIGN COMPONENT zmhp_cl_int_constants=>plans OF STRUCTURE <structure> TO <value>.
*      <value> = pos_id.
*    ENDIF.

*    DEVETHAU, 09.11.2021, Position generated in new method ZMHP_CL_INT_ACTIONS->PROCESS_01
      "In case of Termination the termination date must be set for each IT for delimiting
*    IF me->object_handler->action = zmhp_cl_int_constants=>termination AND
*       me->infty                  = zmhp_cl_int_constants=>it0105.
*      ASSIGN COMPONENT zmhp_cl_int_constants=>endda OF STRUCTURE <structure> TO <value>.
*      IF <value> = zmhp_cl_int_constants=>highdate.
...
*         sap_id IS INITIAL ).
*
*      DATA(oracleid) = CONV zmhp_dd_objectid( cloud_id ).
*
...
*    it_aend-action = COND #( WHEN me->infty EQ zmhp_cl_int_constants=>it0000
*                             THEN me->object_handler->action
*                             ELSE me->action ).
      "Set status for entries
      it_aend-status = COND #( WHEN has_error = abap_true
                               THEN zmhp_cl_int_constants=>booking_status-error_des
                               ELSE zmhp_cl_int_constants=>booking_status-initial ).

*    "Check for active status.
      IF me->infty = zmhp_cl_int_constants=>it0001. "OR me->infty = zmhp_cl_int_constants=>it0000.
      ...
      IF me->infty = zmhp_cl_int_constants=>it0001.
...
*   Before persisting infotype in staging table transfer old values from current infotype.
    zmhp_cl_int_infty_delta=>transfer_infty_data_v2(
      EXPORTING
        infty   = me->infty
...
*   Create IT_AEND entries with respective operation
    IF it_aend-infty IS INITIAL.
      EXIT.
    ENDIF.
    zmhp_cl_int_status_handler=>get_instance( sap_id      = CONV #( sap_id )
                                              cloud_id    = CONV #( cloud_id )
                                              cloud_pernr = CONV #( me->object_handler->cloud_pernr )
                                              int_run     = me->int_run
...
*    IF ( me->infty = zmhp_cl_int_constants=>it0000 AND me->seqnr = 000 )                                  AND
*       ( action = zmhp_cl_int_constants=>hire      OR  action = zmhp_cl_int_constants=>hire_date_change ) AND
*       it_aend-endda                               NE zmhp_cl_int_constants=>highdate.
*
*      it_aend-begda = it_aend-endda + 1.
*      it_aend-endda = zmhp_cl_int_constants=>highdate.
*      it_aend-action = zmhp_cl_int_constants=>termination.
*      me->object_handler->action = zmhp_cl_int_constants=>termination.
*
*      zmhp_cl_int_status_handler=>get_instance( sap_id      = CONV #( sap_id )
*                                                cloud_id    = CONV #( cloud_id )
*                                                cloud_pernr = CONV #( me->object_handler->cloud_pernr )
...
*   Transform infotype entry to prelp
    IF infotype IS NOT INITIAL.
      zmhp_cl_int_type_cast=>pnnnn_to_prelp(
       EXPORTING
          pnnnn = infotype
          aend_id = aend_id
        IMPORTING
  ...
  METHOD process_conversions.
    LOOP AT me->fields ASSIGNING FIELD-SYMBOL(<field>).
      ...
      IF sy-subrc IS INITIAL AND <conversion> IS ASSIGNED.
        ...
        CALL METHOD (conversion_handler)=>(<conversion>-conversion_method)
          EXPORTING
            value_in  = CONV zmhp_dd_value( <field>-value )
          RECEIVING
  ...
  METHOD process_folder_customizing.
    zmhp_cl_int_xml=>get_xpath_folder_with_begda(
      EXPORTING
...
*         Default enddate if not given
        ...
        IF sy-subrc IS INITIAL.
          IF <add_folder>-begda = 0 OR <add_folder>-begda IS INITIAL.
            READ TABLE fallback_values WITH KEY field = zmhp_cl_int_constants=>begda ASSIGNING FIELD-SYMBOL(<fallback_value>).
          ...
          IF <add_folder>-endda = 0 OR <add_folder>-endda IS INITIAL.
            READ TABLE fallback_values WITH KEY infty = customizing-infty field = zmhp_cl_int_constants=>endda ASSIGNING <fallback_value>.
            IF <fallback_value> IS ASSIGNED.
...
*            value_out = <field>-value.
 ...
 METHOD pt_2006_logic.
   DATA: ora_bal   TYPE zmhp_int_ora_bal.

   ASSIGN data TO FIELD-SYMBOL(<structure>).

   "get pernr and oracle entry id
   ASSIGN COMPONENT zmhp_cl_int_constants=>pernr OF STRUCTURE <structure> TO FIELD-SYMBOL(<pernr>).
 ...
 METHOD pt_200x_logic.
   DATA: prelp_old   TYPE prelp,
         art01       TYPE art01,
         it          TYPE REF TO data,
         sew_prelp   TYPE zmhp_prelp,
         it_aend_old TYPE zmhp_int_it_aend.
...
*JMB20210929 insert end
   ...
   IF <infotype> IS INITIAL.
     ...
     DATA(it_aend_entries) = NEW zmhp_cl_int_it_aend( )->get( pernr = pernr ).
     ...
     LOOP AT it_aend_entries ASSIGNING FIELD-SYMBOL(<it_aend_del>).
       ...
       CHECK <it_aend_del>-operation EQ zmhp_cl_int_constants=>infty_ins.

       MOVE-CORRESPONDING <it_aend_del> TO prelp_old.

       zmhp_cl_int_type_cast=>prelp_to_pnnnn( EXPORTING prelp = CONV #( prelp_old )
       ...
       DELETE zmhp_int_it_aend FROM <it_aend_del>.
       EXIT.
     ENDLOOP.
   ...
   ELSE.
     ...
     it_aend_old-operation = zmhp_cl_int_constants=>infty_del.
     ...
     it_aend-aend_id = zmhp_cl_int_status_handler=>generate_guid( ).
   ENDIF.
   ...
   it_aend-operation = zmhp_cl_int_constants=>infty_ins.
...
*
   ...
   IF <alldf> EQ abap_true OR
     ...
     zmhp_cl_int_type_cast=>pnnnn_to_prelp( EXPORTING pnnnn   = <structure>
...
METHOD pt_specific_logic.
  DATA: it_aend_old TYPE zmhp_int_it_aend,
  ...
  DATA(it_aend) = CORRESPONDING zmhp_int_it_aend( prelp_orig ).
  it_aend-aend_id = aend_id.
  ...
  CASE me->infty.
    ...
    WHEN zmhp_cl_int_constants=>it2011.
      ASSIGN COMPONENT zmhp_cl_int_constants=>user2 OF STRUCTURE <structure> TO FIELD-SYMBOL(<user2>).
      ...
      IF <user2> IS ASSIGNED    AND
        ...
        LOOP AT teven ASSIGNING FIELD-SYMBOL(<teven>).
          ...
          MOVE-CORRESPONDING prelp_old TO it_aend_old.
          it_aend_old-infty   = me->infty.
          it_aend_old-aend_id = zmhp_cl_int_status_handler=>generate_guid( ).
    ...
    WHEN zmhp_cl_int_constants=>it2001 OR
         zmhp_cl_int_constants=>it2002.
