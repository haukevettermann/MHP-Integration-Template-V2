...
*&---------------------------------------------------------------------*
  ...
  DATA(om_aend_post) = NEW zmhp_cl_om_aend_post( alv   = pa_alv
                                                    batch = sy-batch
                                                    simu  = p_test ).
*                                                    dates = VALUE rsdsselopt_t( ( sign = 'I' option = 'BT' low = pn-begda high = pn-endda ) ) ).
  ...
  om_aend_post->status_handler = NEW zmhp_cl_int_status_handler( sap_id_rng = sap_id_range cloud_id_rng = cloud_id_range
...
*                                     intrun = intrun_range ).
  ...
  "start post
  "Change
*  IF zmhp_cl_int_utility=>check_for_om_error( type = p_etype ) = abap_false.
