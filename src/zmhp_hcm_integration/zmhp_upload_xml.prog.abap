...
DATA:
  ...
  data            TYPE zmhp_cl_int_xml=>tt_data,
...
CALL FUNCTION 'FILE_GET_NAME'
  ...
  EXCEPTIONS
    file_not_found   = 1
    OTHERS           = 2.
zmhp_cl_int_xml=>upload_gui_file(
  EXPORTING
    path    = path
  IMPORTING
    length  = DATA(length)
    data    = data
    message = message ).
zmhp_cl_int_xml=>convert_xml_to_xstring(
  EXPORTING
    length  = length
    data    = data
  IMPORTING
    ...
    message = message ).
*CALL FUNCTION 'ZMHP_INT_EXTRACT_RECEIVER'
*  EXPORTING
*    xml_string = xstring
*    file_name  = 'Test_T'
** IMPORTING
