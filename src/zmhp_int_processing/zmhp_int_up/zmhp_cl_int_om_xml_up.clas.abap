class ZMHP_CL_INT_OM_XML_UP definition
  public
  final
  create public .
...
public section.
  ...
  types:
    t_omaendup TYPE TABLE OF zmhp_int_om_aeup WITH KEY
...
CLASS ZMHP_CL_INT_OM_XML_UP IMPLEMENTATION.
...
*   First process first level
    ...
    LOOP AT me->go_infotype->folders ASSIGNING FIELD-SYMBOL(<folder>).
      ...
      IF <folder>-has_elements = abap_true.
        DATA(fields) = VALUE zmhp_cl_int_infty_proc_xml_up=>t_fields( FOR field IN me->go_infotype->fields WHERE ( folder CS <folder>-folder  AND
  ...
  METHOD PROCESS_CHANGED_DATA.
    me->go_infotype = NEW zmhp_cl_int_infty_proc_xml_up( changed_data_om = me->changed_data
  ...
  METHOD READ_CUSTOMIZING.
    ...
    me->go_customizing = zmhp_cl_int_customizing_xml=>get_instance(
      EXPORTING
        object_type = zmhp_cl_int_constants=>orgunit
        molga       = me->molga ).
    ...
    IF ok IS INITIAL.
      ...
      message-message_v2 = zmhp_cl_int_constants=>orgunit.
    ELSE.
