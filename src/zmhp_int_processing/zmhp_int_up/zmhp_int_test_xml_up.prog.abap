...
*&
*&---------------------------------------------------------------------*
REPORT zmhp_int_test_xml_up.
DATA:
  is_aendup TYPE zmhp_int_it_aeup,
  s0000     TYPE p0000,
  s0002     TYPE p0002,
  s0001     TYPE p0001,
...
*MODIFY zmhp_int_it_aeup FROM is_aendup.
*cl_hr_pnnnn_type_cast=>pnnnn_to_prelp(
*  EXPORTING
...
SELECT SINGLE * FROM zmhp_int_objects INTO @DATA(ls_object) WHERE object = @zmhp_cl_int_constants=>person.
SELECT * FROM zmhp_int_it_aeup INTO TABLE @DATA(changed_data) WHERE status = @zmhp_cl_int_constants=>booking_status-initial AND int_run = '005056AE579A1EDBA2F7BE6D59883113'.
...
LOOP AT pernr ASSIGNING FIELD-SYMBOL(<pernr>).
  DATA(changed_data_pernr) = VALUE zmhp_cl_int_employee_xml_up=>t_itaendup( FOR change IN changed_data WHERE ( pernr = <pernr> ) ( change ) ).
  ...
  xml->add_node_object_employee(
    EXPORTING
      object_id_cloud = CONV #( zmhp_cl_int_employee_xml_up=>read_oracle_id(
                                                                                 EXPORTING
                                                                                   pernr     = <pernr>
                                                                                   date      = sy-datum
                                                                                 IMPORTING
  ...
  DATA(employee) = NEW zmhp_cl_int_employee_xml_up( changed_data = changed_data_pernr pernr = <pernr> xml_node = xml_node xml_document = new_xml ).
  employee->read_molga(
*    IMPORTING
*      message =
  ).
