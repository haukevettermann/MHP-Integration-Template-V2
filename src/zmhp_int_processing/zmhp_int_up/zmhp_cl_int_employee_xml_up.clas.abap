class ZMHP_CL_INT_EMPLOYEE_XML_UP definition
  public
  final
  create public .
...
public section.
  ...
  types:
    t_itaendup TYPE TABLE OF zmhp_int_it_aeup WITH KEY
  ...
  data GO_INFOTYPE type ref to ZMHP_CL_INT_INFTY_PROC_XML_UP .
  ...
  class-methods READ_ORACLE_ID
    ...
    returning
      value(ORACLE_ID) type ZMHP_DD_OBJECTID .
  methods READ_MOLGA
    exporting
      !MESSAGE type BAPIRET1 .
...
CLASS ZMHP_CL_INT_EMPLOYEE_XML_UP IMPLEMENTATION.
...
*JMB20211026 insert end
          zmhp_cl_int_xml_up=>get_direct_child(
            EXPORTING
      ...
      IF <folder>-has_elements = abap_true.
        ...
        DATA(fields) = VALUE zmhp_cl_int_infty_proc_xml_up=>t_fields( FOR field IN me->go_infotype->fields WHERE ( folder EQ folder          AND "JMB20210913 D -folder CS <folder>-folder AND
        ...
        LOOP AT fields ASSIGNING FIELD-SYMBOL(<field>).
          ...
          ENDIF.
          zmhp_cl_int_xml_up=>add_element(
            EXPORTING
              name    = CONV #( element )
...
METHOD process_changed_data.
  me->go_infotype = NEW zmhp_cl_int_infty_proc_xml_up( changed_data_it = me->changed_data
...
METHOD read_customizing.
  DATA(ok) = abap_true.
  DATA: tmp_fields TYPE zmhp_cl_int_customizing_xml=>t_customizing_fields.
  me->go_customizing = zmhp_cl_int_customizing_xml=>get_instance( EXPORTING object_type  = zmhp_cl_int_constants=>person
                                                                            molga        = me->molga
...
*
  me->molga = zmhp_cl_int_utility=>get_molga( pernr = me->pernr
                                              begda = sy-datum
                                              endda = sy-datum ).
