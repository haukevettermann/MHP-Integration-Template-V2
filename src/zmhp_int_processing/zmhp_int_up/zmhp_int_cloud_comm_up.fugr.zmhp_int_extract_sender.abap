FUNCTION ZMHP_INT_EXTRACT_SENDER.
*"----------------------------------------------------------------------
...
*
  DATA(p_obj) = VALUE rsdsselopt_t( ( sign = 'I' option = 'EQ' low = zmhp_cl_int_constants=>person )
                                    ( sign = 'I' option = 'EQ' low = zmhp_cl_int_constants=>absence )
                                    ( sign = 'I' option = 'EQ' low = zmhp_cl_int_constants=>quota ) ).

  CASE object_type.
    WHEN zmhp_cl_int_constants=>absence.
      sel_infty = VALUE #( ( sign = 'I' option = 'EQ' low = '2001' ) ).
    WHEN zmhp_cl_int_constants=>quota.
      sel_infty = VALUE #( ( sign = 'I' option = 'EQ' low = '2006' ) ).
    WHEN zmhp_cl_int_constants=>person.
...
*JMB20211111 insert end
  ...
  IF object_type IN p_obj.
    SELECT SINGLE * FROM zmhp_int_objects INTO @DATA(ls_object) WHERE object = @object_type.
    SELECT * FROM zmhp_int_it_aeup INTO TABLE @DATA(changed_data) WHERE status EQ '01' AND
...
*    cl_abap_browser=>show_xml( EXPORTING xml_string = string ).
  ...
  ELSEIF object_type = 'D'.
    SELECT SINGLE * FROM zmhp_int_objects INTO @DATA(ls_object_om) WHERE object = 'O'.
    SELECT * FROM zmhp_int_om_aeup INTO TABLE @DATA(changed_data_om) WHERE status = '01'.
    ...
    DATA(xml_om) = NEW zmhp_cl_int_xml_up( ).
    ...
    LOOP AT changed_data_om_tmp ASSIGNING FIELD-SYMBOL(<change_om>).
      DATA(changed_data_om_sap_id) = VALUE zmhp_cl_int_om_xml_up=>t_omaendup( FOR change_om IN changed_data_om WHERE ( sap_id = <change_om>-sap_id ) ( change_om ) ).

      xml_om->add_node_object_om( EXPORTING object_id_cloud = CONV #( zmhp_cl_int_om_xml_up=>read_oracle_id( EXPORTING objid     = <change_om>-sap_id
      ...
      DATA(om) = NEW zmhp_cl_int_om_xml_up( changed_data = changed_data_om_sap_id objid = <change_om>-sap_id xml_node = xml_node_om xml_document = new_xml_om ).
    ...
    IF xml_string IS NOT INITIAL.
      MODIFY zmhp_int_om_aeup FROM TABLE changed_data_om_tmp.
      COMMIT WORK.
    ENDIF.
...
*
  ELSEIF object_type = 'W'.
    ...
    DATA: file_list        TYPE TABLE OF zmhp_file_list,
    ...
    DATA(logical_filename) = CONV filename-fileintern( 'ZHCM_ORACLE_EXTRACTS' ).
    ...
    IF file_list IS NOT INITIAL.
      ...
      LOOP AT file_list_sorted ASSIGNING <file_sorted>.
        ...
        IF sy-subrc IS INITIAL.
          ...
          "Archive file
          DATA(logical_archive) = CONV filename-fileintern( 'ZHCM_ORACLE_EXTRACTS_ARCHIVE' ).
          CALL FUNCTION 'FILE_GET_NAME'
            EXPORTING
