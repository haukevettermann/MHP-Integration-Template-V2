class ZMHP_CL_INT_INFTY_PROC_XML_UP definition
  public
  final
  create public .
...
public section.
  ...
  types:
    ...
    TYPES prev_folder TYPE zmhp_dd_folder.
  ...
  types:
    ...
    TYPES folder TYPE zmhp_dd_folder.
    TYPES element TYPE zmhp_dd_element.
    TYPES begda TYPE dats.
    TYPES endda TYPE dats.
  ...
  data FIELD_CUSTOMIZING type ZMHP_CL_INT_CUSTOMIZING_XML=>T_CUSTOMIZING_FIELDS .
  data FOLDERS type T_FOLDERS .
  data FIELDS type T_FIELDS .
  data TIME_SLICES type HRPERIODS_TAB .
  data INFTY type INFTY .
  data CHANGED_DATA_IT type ZMHP_CL_INT_EMPLOYEE_XML_UP=>T_ITAENDUP .
  data CHANGED_DATA_OM type ZMHP_CL_INT_OM_XML_UP=>T_OMAENDUP .
  data GO_CUSTOMIZING type ref to ZMHP_CL_INT_CUSTOMIZING_XML .
  ...
  methods CONSTRUCTOR
    importing
      ...
      !CHANGED_DATA_IT type ZMHP_CL_INT_EMPLOYEE_XML_UP=>T_ITAENDUP optional
      !CHANGED_DATA_OM type ZMHP_CL_INT_OM_XML_UP=>T_OMAENDUP optional .
  methods GET_FIELDS
    exporting
      !MESSAGE type BAPIRET1 .
  methods GET_FOLDERS
...
private section.
  ...
  methods GET_FIELDS_2006
    importing
      !STRUCTURE type ANY
    returning
      value(FIELDS) type ZMHP_CL_INT_INFTY_PROC_XML=>T_FIELDS .
...
CLASS ZMHP_CL_INT_INFTY_PROC_XML_UP IMPLEMENTATION.
...
*JMB20211026 insert end
  ...
  DATA(fields) = VALUE /iwbep/t_mgw_name_value_pair( ( name = '/AbsStartTime'      value = 'BEGUZ'     )
                                                     ...
                                                     ( name = '/Operation'         value = zmhp_cl_forms_utils=>delete    ) ).
  ...
  LOOP AT fields ASSIGNING FIELD-SYMBOL(<fields>).
    ...
    CASE <fields>-value.
      ...
      WHEN 'ABTYP'.
        ...
        DATA(molga) = zmhp_cl_forms_utils=>get_molga( pernr = <pernr> ).

        SELECT SINGLE oracle_awart FROM zmhp_int_awa_map WHERE molga EQ @molga
                                                           AND infty EQ '2001'
                                                           AND awart EQ @<subty>
                                                           AND begda <= @<endda>
                                                           AND endda >= @<begda>
      ...
      WHEN 'SSID'.
        ...
        DATA(oracle_pernr) = zmhp_cl_forms_utils=>get_oraclepernr( EXPORTING pernr = <pernr> ).
      ...
      WHEN 'EMPLY'.
        ...
        molga = zmhp_cl_int_utility=>get_molga( pernr = <pernr>
                                                begda = <begda>
                                                endda = <endda> ).

        zmhp_cl_mig_utils=>get_mapping_fields( EXPORTING molga          = VALUE #( ( sign = 'I' option = 'EQ' low = molga ) )
        ...
        zmhp_cl_mig_utils=>get_mapping_values( EXPORTING molga          = VALUE #( ( sign = 'I' option = 'EQ' low = molga ) )
                                                         infty          = zmhp_cl_mig_utils=>it0001
                                                         sap_field      = zmhp_cl_mig_utils=>bukrs
                                                         oracle_field   = zmhp_cl_mig_utils=>legalemployername
                                                         export         = abap_true
        ...
        DATA(value_tmp) = CONV zmhp_dd_value( bukrs ).

        mapping_handler->process_mapping( EXPORTING import         = abap_false
                                                        export         = abap_true
                                                        infty          = zmhp_cl_mig_utils=>it0001
      ...
      WHEN zmhp_cl_forms_utils=>delete.
        new_field-value   = zmhp_cl_forms_utils=>merge.
...
ENDMETHOD.
  ...
  METHOD determine_time_slices.
    READ TABLE me->fields WITH KEY field = zmhp_cl_int_constants=>begda ASSIGNING FIELD-SYMBOL(<begda>).
...
METHOD get_fields.
  DATA: lr_structdescr TYPE REF TO cl_abap_structdescr,
        ...
        fields_infty   TYPE zmhp_cl_int_infty_proc_xml=>t_fields,
  ...
  LOOP AT me->changed_data_it ASSIGNING FIELD-SYMBOL(<changed_data_it>).
    DATA(fields) = VALUE zmhp_cl_int_infty_proc_xml=>t_fields( ( infty     = '0001'
                                                                 field_sap = 'MOLGA'
...
*   Create dynamic structure
    ...
    ASSIGN COMPONENT zmhp_cl_int_constants=>begda OF STRUCTURE <changed_data_it> TO FIELD-SYMBOL(<begda>).
    ASSIGN COMPONENT zmhp_cl_int_constants=>endda OF STRUCTURE <changed_data_it> TO FIELD-SYMBOL(<endda>).
...
*
        DATA(field_oracle) = CONV zmhp_dd_field( <field>-element ).
        REPLACE ALL OCCURRENCES OF '/' IN field_oracle WITH space.
        TRANSLATE field_oracle TO UPPER CASE.

        DATA(value_tmp) = CONV zmhp_dd_value( <value> ).

        APPEND LINES OF VALUE zmhp_cl_int_infty_proc_xml=>t_fields( ( infty     = infty
                                                                      field_sap = <field>-field
                                                                      value     = value_tmp     )  ) TO fields.

        mapping_handler->process_mapping( EXPORTING infty          = <field>-infty
                                                        field_sap      = <field>-field
...
*   Create dynamic structure
    ...
    LOOP AT me->field_customizing ASSIGNING FIELD-SYMBOL(<field_om>) WHERE infty = <changed_data_om>-infty.
      ...
      ASSIGN COMPONENT zmhp_cl_int_constants=>begda OF STRUCTURE <changed_data_om> TO FIELD-SYMBOL(<begda_om>).
      ASSIGN COMPONENT zmhp_cl_int_constants=>endda OF STRUCTURE <changed_data_om> TO FIELD-SYMBOL(<endda_om>).
