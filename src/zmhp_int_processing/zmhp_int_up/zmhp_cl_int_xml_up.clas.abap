class ZMHP_CL_INT_XML_UP definition
  public
  final
  create public .
...
public section.
  ...
  class-methods ADD_NODE_OBJECT_EMPLOYEE
    importing
      ...
      !AEND_UP type ZMHP_INT_IT_AEUP optional
    exporting
  ...
  class-methods ADD_NODE_OBJECT_OM
    importing
      ...
      !XPATH_OBJECT type STRING
      !XML_DOCUMENT type ref to IF_IXML_DOCUMENT
      !AEND_UP type ZMHP_INT_OM_AEUP optional
    exporting
      !XML_NODE type ref to IF_IXML_NODE
    changing
      !XML_DATA_NODE type ref to IF_IXML_NODE .
...
CLASS ZMHP_CL_INT_XML_UP IMPLEMENTATION.
...
*   Add begda
    IF begda IS NOT INITIAL.
      DATA(node_begda) = xml_in->create_element( name = zmhp_cl_int_constants=>startdate ).
      node_begda->set_value( value = CONV #( zmhp_cl_int_conversion=>convert_date_oracle_om( value_in = CONV #( begda ) ) ) ).
      node_out->append_child( new_child = node_begda ).
    ENDIF.
*   Add endda
    IF endda IS NOT INITIAL.
      DATA(node_endda) = xml_in->create_element( name = zmhp_cl_int_constants=>enddate ).
      node_endda->set_value( value = CONV #( zmhp_cl_int_conversion=>convert_date_oracle_om( value_in = CONV #( endda ) ) ) ).
...
*
    ...
    DATA(oracle_pernr) = zmhp_cl_forms_utils=>get_oraclepernr( CONV #( object_id_sap ) ).
...
*   Set Start Date in case of pernr update.
    IF aend_up IS NOT INITIAL.
      xml_element = xml_document->create_element( name = zmhp_cl_int_constants=>startdate ).
      xml_element->set_value( value = CONV #( zmhp_cl_int_conversion=>convert_date_oracle( value_in = CONV #( aend_up-begda ) ) ) ).
      xml_node->append_child( new_child = xml_element ).
    ENDIF.
  ENDMETHOD.
...
*   Create and append org name
    ...
    SELECT SINGLE dep_name FROM hrp9401 INTO @DATA(stext) WHERE objid = @object_id_sap.
...
*   Set Start Date in case of pernr update.
    IF aend_up IS NOT INITIAL.
      xml_element = xml_document->create_element( name = zmhp_cl_int_constants=>startdate ).
      xml_element->set_value( value = CONV #( zmhp_cl_int_conversion=>convert_date_oracle_om( value_in = CONV #( aend_up-begda ) ) ) ).
      xml_node->append_child( new_child = xml_element ).
    ENDIF.
  ENDMETHOD.
...
METHOD get_direct_child.
  ...
  WHILE child IS BOUND.
    IF child->get_name( ) = name.
      ...
      WHILE child IS BOUND.
        IF child->get_name( ) = zmhp_cl_int_constants=>startdate.
          begda = child->get_value( ).
        ENDIF.
        IF child->get_name( ) = zmhp_cl_int_constants=>enddate.
