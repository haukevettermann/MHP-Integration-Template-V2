class ZMHP_CL_INT_OM_AEND definition
  public
  create public .
...
public section.
  ...
  data AEND_NEW type ZMHP_TT_OM_AEND .
  data AEND_ORIG type ZMHP_TT_OM_AEND .
  data AEND_NOCHNG type ZMHP_TT_OM_AEND .
  ...
  methods GET
    ...
    returning
      value(OM_AEND) type ZMHP_TT_OM_AEND .
  ...
  methods PNNNN_TO_OM_AEND
    importing
      !PNNNN type ANY
    returning
      value(OM_AEND) type ZMHP_INT_OM_AEND .
...
CLASS ZMHP_CL_INT_OM_AEND IMPLEMENTATION.
  ...
  METHOD get.
    ...
    SELECT * FROM zmhp_int_om_aend INTO TABLE om_aend WHERE sap_id   IN objid  AND
...
*!!!OBSOLETE: Do not use -> moved to ZMHP_CL_INT_TYPE_CAST!!!
*
**    DATA(infotype) = zmhp_cl_int_infty_proc_xml=>get_instance( ).
...
*
*    MODIFY zmhp_int_om_aend FROM TABLE om_aend.
*
*    is_ok = abap_true.
*    IF sy-subrc NE 0.
*      is_ok = abap_false.
