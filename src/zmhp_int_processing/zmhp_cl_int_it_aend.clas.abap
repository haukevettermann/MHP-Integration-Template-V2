class ZMHP_CL_INT_IT_AEND definition
  public
  create public .

public section.

  constants IT_AEND_MC type CHAR20 value 'ZMHP_MC_INT_IT_AEND' ##NO_TEXT.
  data MSG_CONT type ref to /IWBEP/IF_MESSAGE_CONTAINER .
  constants STATUS_INITIAL type ZMHP_DD_STATUS value '01' ##NO_TEXT.
  constants STATUS_ERROR type ZMHP_DD_STATUS value '03' ##NO_TEXT.
  constants STATUS_SUCCESS type ZMHP_DD_STATUS value '02' ##NO_TEXT.
  ...
  methods SAVE_ENTRIES
    importing
      !IT_AEND type ZMHP_TT_IT_AEND optional
      !OM_AEND type ZMHP_TT_OM_AEND optional
    returning
  ...
  methods PNNNN_TO_IT_AEND
    importing
      !PNNNN type ANY
    returning
      value(IT_AEND) type ZMHP_INT_IT_AEND .
  ...
  methods POST_IT_AEND
    ...
    exporting
      ...
      !IT_AEND_LOCKED type ZMHP_TT_IT_AEND
    changing
      !IT_AEND type ZMHP_TT_IT_AEND
...
CLASS ZMHP_CL_INT_IT_AEND IMPLEMENTATION.


METHOD check_time_event.

  DATA: begda_ext   TYPE zmhp_dd_date_ext,
*        ltime_ext   TYPE zmhp_dd_time_ext,
        error_v     TYPE char19,
...
*
  IF event IS NOT INITIAL.
    ...
    IF <time> IS ASSIGNED.
*      ltime_ext = zmhp_cl_int_utility=>get_external_time( time = <time> ).
...
*      CONCATENATE begda_ext ltime_ext INTO error_v SEPARATED BY space.
    ...
    IF <satza> IS INITIAL.
      ...
      return = VALUE bapiret1( type = zmhp_cl_int_constants=>error
                               ...
                               number = zmhp_cl_int_constants=>msg_no-m36
                               message_v1 = error_v
    ...
    IF <user2> IS INITIAL.
      ...
      return = VALUE bapiret1( type = zmhp_cl_int_constants=>error
                               id = zmhp_cl_int_constants=>msg_class_int
                               number = zmhp_cl_int_constants=>msg_no-m37
                               message_v1 = error_v
                               message_v2 = it_aend-pernr
                               message_v3 = it_aend-cloud_id
                               message_v4 = it_aend-int_run
...
ENDMETHOD.
  ...
  METHOD get.
    ...
    SELECT * FROM zmhp_int_it_aend INTO TABLE it_aend WHERE pernr   IN pernr  AND
  ...
  METHOD get_peras.
    ...
    SELECT DISTINCT pernr FROM zmhp_int_it_aend INTO TABLE @DATA(lt_pernr) WHERE begda LE @endda AND
                                                                                 endda GE @begda.
...
*!!!OBSOLETE: Do not use -> moved to ZMHP_CL_INT_TYPE_CAST!!!
*
...
*        ltime_ext        TYPE zmhp_dd_time_ext,
  ...
  IF <struc> IS ASSIGNED.
    ...
    ASSIGN COMPONENT zmhp_cl_int_constants=>user2 OF STRUCTURE <struc> TO FIELD-SYMBOL(<user2>).
    ...
    "new entry or modify
    IF it_aend-operation = zmhp_cl_int_constants=>infty_operation-pa_insert OR
       it_aend-operation = zmhp_cl_int_constants=>infty_operation-pa_modify.
...
*         returncode              = returncode
      ...
      IF sy-subrc <> 0.
        ...
        return = VALUE bapiret1( type = zmhp_cl_int_constants=>error
                                 id = zmhp_cl_int_constants=>msg_class_int
                                 number = zmhp_cl_int_constants=>msg_no-m38
      ...
      "Delte only
    ELSEIF it_aend-operation = zmhp_cl_int_constants=>infty_operation-pa_delete.
...
*          ltime_ext = zmhp_cl_int_utility=>get_external_time( time = <ltime> ).
...
*          CONCATENATE begda_ext ltime_ext INTO error_v SEPARATED BY space.
        ...
        return = VALUE bapiret1( type = zmhp_cl_int_constants=>error
                                id = zmhp_cl_int_constants=>msg_class_int
                                number = zmhp_cl_int_constants=>msg_no-m39
                                message_v1 = error_v
                                message_v2 = it_aend-pernr
                                message_v3 = it_aend-cloud_id
                                message_v4 = it_aend-int_run
...
*    me->enhance_entries( CHANGING it_aend = it_aend ).
  ...
  METHOD save_entries.
* !!!OBSOLETE: Do not use -> moved to ZMHP_CL_INT_STATUS_HANDLER -> PERSTIST_DATA!!!
*
*    IF it_aend IS SUPPLIED AND it_aend IS NOT INITIAL.
*      MODIFY zmhp_int_it_aend FROM TABLE it_aend.
*
*      is_ok = abap_true.
*      IF sy-subrc NE 0.
*        is_ok = abap_false.
