*&---------------------------------------------------------------------*
*& Report ZMHP_RP_IT_AEND_POST
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT zmhp_rp_it_aend_post_pt.
...
**---------------------------------------------------------------------*
  ...
  SELECT-OPTIONS so_stat FOR zmhp_int_it_aend-status.
  SELECT-OPTIONS so_intr FOR zmhp_int_it_aend-int_run.
  SELECT-OPTIONS so_cldid FOR zmhp_int_it_aend-cloud_id.
...
START-OF-SELECTION.
  ...
  IF pnppernr[] IS INITIAL.
    pnppernr[] = NEW zmhp_cl_int_it_aend( )->get_peras( begda = pn-begda endda = pn-endda ).
  ENDIF.

  "no pernr was found in ZMHP_INT_IT_AEND
  IF pnppernr[] IS INITIAL.
    STOP.
  ENDIF.
...
END-OF-SELECTION.
  ...
  DATA(cl_it_aend_post_pt) = NEW zmhp_cl_it_aend_post_pt( alv   = pa_alv
                                                          batch = sy-batch
                                                          simu  = pa_simu
...
*JMB20211004 insert end
  ...
  cl_it_aend_post_pt->status_handler = NEW zmhp_cl_int_status_handler( sap_id_rng   = gr_pernr
                                                                       infty_rng    = gr_inftys
