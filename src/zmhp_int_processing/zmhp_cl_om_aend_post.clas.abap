class ZMHP_CL_OM_AEND_POST definition
  public
  create public .
...
public section.
  ...
  data OM_AEND_ERROR type ZMHP_TT_OM_AEND .
  data OM_AEND_POST type ZMHP_TT_OM_AEND .
  data OM_AEND_NOCHANGE type ZMHP_TT_OM_AEND .
  ...
  methods POST_OM_AEND
    importing
      !SIMU type BOOLEAN
    exporting
      !OM_AEND_POST type ZMHP_TT_OM_AEND
      !OM_AEND_ERROR type ZMHP_TT_OM_AEND
      !OM_AEND_NOCHANGE type ZMHP_TT_OM_AEND
    changing
      !OM_AEND type ZMHP_TT_OM_AEND
...
CLASS ZMHP_CL_OM_AEND_POST IMPLEMENTATION.
...
*       batch EQ abap_false.
  ...
  METHOD delimit_hrp1000.
    ...
    lr_structdescr ?= cl_abap_typedescr=>describe_by_name( 'P' && infty ).
    ...
    ASSIGN COMPONENT zmhp_cl_int_constants=>objid OF STRUCTURE <hrp_old> TO FIELD-SYMBOL(<objid>).
    ASSIGN COMPONENT zmhp_cl_int_constants=>begda OF STRUCTURE <hrp_old> TO FIELD-SYMBOL(<begda>).
    ASSIGN COMPONENT zmhp_cl_int_constants=>endda OF STRUCTURE <hrp_old> TO FIELD-SYMBOL(<endda>).
...
*                                        IMPORTING message = return-message ).
      ...
      ELSE.
        ...
        return = VALUE bapiret1( type = zmhp_cl_int_constants=>success
                             id = zmhp_cl_int_constants=>msg_class_int
                             number = zmhp_cl_int_constants=>msg_no-m15
                             message_v1 = <objid>
                             message_v2 = <begda>
                             message_v3 = <endda>
  ...
  METHOD get_om_aend_data.
    ...
    om_aend = NEW zmhp_cl_int_om_aend( )->get( objid  = objid
                                               intrun = intrun
                                               dates  = dates
                                               status = status ).
...
*      IF infty = zmhp_cl_int_constants=>it1000.
...
*    ENDIF.
    ...
    IF simu = abap_true.
      ...
      return = VALUE bapiret1( type = zmhp_cl_int_constants=>success
                                 id = zmhp_cl_int_constants=>msg_class_int
                                 number = zmhp_cl_int_constants=>msg_no-m27
                                 message_v1 = sap_id
                                 message_v2 = infty
                                 message_v3 = date
*                                 message_v4 = date
    ...
    ELSE.
      ...
      IF it_change = abap_true.
        ...
        IF infty NE zmhp_cl_int_constants=>it1000 AND infty NE zmhp_cl_int_constants=>it1001.
        ...
        EXPORTING
          ...
          IF <hrp_old> IS INITIAL AND action = zmhp_cl_int_constants=>infty_operation-om_delimit.
            return = VALUE bapiret1( type = zmhp_cl_int_constants=>warning
                                     id = zmhp_cl_int_constants=>msg_class_int
                                     number = zmhp_cl_int_constants=>msg_no-m29
...
*                                     message_v4 = <om_aend>-sap_id
          ...
          ASSIGN COMPONENT zmhp_cl_int_constants=>begda OF STRUCTURE <hrp_old> TO FIELD-SYMBOL(<begda_old>).
...
*                                        IMPORTING message = return-message ).
  ...
  METHOD object_creation.
    DATA: it        TYPE REF TO data,
          ...
          om_aendup TYPE zmhp_int_om_aeup.
...
*    <langu> = spras.
    zmhp_cl_int_utility=>get_spras_by_molga( EXPORTING molga = om_aend-molga
...
*    CALL FUNCTION 'RHOM_ALL_BUFFER_INIT'.
    IF simu = abap_true.
      DATA(begda_ext) = zmhp_cl_int_utility=>get_external_date( date = om_aend-begda ).
      DATA(endda_ext) = zmhp_cl_int_utility=>get_external_date( date = om_aend-endda ).
      CONCATENATE begda_ext '-' endda_ext INTO DATA(date) SEPARATED BY space.
      return = VALUE bapiret1( type = zmhp_cl_int_constants=>success
                                 id = zmhp_cl_int_constants=>msg_class_int
                                 number = zmhp_cl_int_constants=>msg_no-m25
                                 message_v1 = om_aend-infty
    ...
    ELSE.
      TRY.
          CALL FUNCTION 'RHOM_CREATE_OBJECTS_FROM_TAB'
            EXPORTING
              plvar               = '01'
              otype               = om_aend-otype "zmhp_cl_int_constants=>position
              ostat               = '1'
              begda               = om_aend-begda
              endda               = om_aend-endda
              buffer_mode         = 'X'
...
*            CALL FUNCTION 'RHOM_WRITE_BUFFER_TO_DB'.
            ...
            IF simu NE abap_true.
              ...
              om_aendup-status = zmhp_cl_int_constants=>booking_status-initial.
...
*         OUTPUT        = wplog-
    ...
    lr_structdescr ?= cl_abap_typedescr=>describe_by_name( 'P' && om_aend-infty ).
    lr_tabledescr ?= cl_abap_tabledescr=>create( p_line_type = lr_structdescr ).
    CREATE DATA lr_table TYPE HANDLE lr_tabledescr.
...
*          hrpdata TYPE STANDARD TABLE,
          ...
          om_aend_tmp     TYPE zmhp_tt_om_aend,
          om_aend_del     TYPE zmhp_tt_om_aend,
    ...
    FIELD-SYMBOLS: <struk>       TYPE any,
                   <infotype>    TYPE any,
                   <hrp_data>    TYPE STANDARD TABLE,
                   <om_aend_tmp> TYPE zmhp_int_om_aend,
                   <hrp_old>     TYPE any,
                   <hrp_last>    TYPE any.
    ...
    LOOP AT me->status_handler->om_aend ASSIGNING FIELD-SYMBOL(<om_aend>). "WHERE status = '01' OR status = '07'.
      IF me->book_manager_relation = abap_true.
        IF <om_aend>-subty NE zmhp_cl_int_constants=>relations-manager.
          CONTINUE.
        ENDIF.
      ELSE.
        IF <om_aend>-subty = zmhp_cl_int_constants=>relations-manager.
          count = count + 1.
      ...
      me->status_handler->check_for_error(
         EXPORTING
            ...
            type = zmhp_cl_int_constants=>orgunit
...
*        ENDIF.
      ...
      DATA(infty_operation) = NEW zmhp_cl_int_it_operation( int_run = <om_aend>-int_run molga = <om_aend>-molga objid = <om_aend>-sap_id ).


*      DATA(error_handler) = NEW zmhp_cl_int_error_handler( sap_id = CONV #( <om_aend>-sap_id ) cloud_id = CONV #( <om_aend>-cloud_id )
...
*        IF <om_aend>-action NE zmhp_cl_int_constants=>hire.
        om_aend_tmp = VALUE zmhp_tt_om_aend( FOR ls_om_aend IN me->status_handler->om_aend WHERE ( sap_id EQ <om_aend>-sap_id  AND cloud_id = <om_aend>-cloud_id ) ( ls_om_aend ) ).
*        ELSE.
*          om_aend_tmp = VALUE zmhp_tt_om_aend( FOR ls_om_aend IN om_aend WHERE ( int_run = <om_aend>-int_run AND
...
**          zmhp_cl_utils=>add_message_to_handler( EXPORTING iv_sap_id  = sap_id
**                                                           is_return = return
...
*
**          error_handler->add_message_pa( EXPORTING sap_id = <om_aend>-sap_id int_run = <om_aend>-int_run return = return begda = <om_aend>-begda endda = <om_aend>-endda ).
**        zmhp_cl_utils=>add_message_to_handler( EXPORTING iv_sap_id  = <om_aend>-sap_id
**                                                         is_return = return
**                                               CHANGING  MESSAGE_HANDLER = MESSAGE_HANDLER ).
*            CLEAR: sap_id, return.
*
...
*          ENDIF.
      ...
      IF <om_aend>-sap_id IS INITIAL AND <om_aend>-infty NE zmhp_cl_int_constants=>it1000.
        zmhp_cl_int_utility=>get_objid_by_cloudid( EXPORTING cloud_id = CONV #( <om_aend>-cloud_id ) begda = <om_aend>-begda endda = <om_aend>-endda IMPORTING objid = <om_aend>-sap_id ).
      ENDIF.

      IF ( <om_aend>-sap_id IS NOT INITIAL AND <om_aend>-infty NE zmhp_cl_int_constants=>it1000 )
            OR ( <om_aend>-sap_id IS INITIAL AND <om_aend>-infty = zmhp_cl_int_constants=>it1000 )
              OR ( <om_aend>-sap_id IS NOT INITIAL AND <om_aend>-infty = zmhp_cl_int_constants=>it1000 ).
...
*              SELECT SINGLE spras FROM t001 WHERE land1 = @country INTO @DATA(spras).
            ...
            <langu> = langu.
          ENDIF.
          DATA(infty_delta) = NEW zmhp_cl_int_infty_delta( infty = <om_aend>-infty ).
          ...
          IF it_change = abap_true.
            <om_aend>-operation = zmhp_cl_int_constants=>infty_operation-om_modify.
          ENDIF.
          IF it_create = abap_true.
            <om_aend>-operation = zmhp_cl_int_constants=>infty_operation-om_insert.
          ENDIF.
          "Add messages
*        IF return_tab IS NOT INITIAL.
*
*        ENDIF.
          IF <om_aend>-operation = zmhp_cl_int_constants=>infty_operation-om_delimit AND it_create = abap_true.
            CLEAR it_create.
          ENDIF.

          "If inactivation ignore no change and process
          IF it_change = abap_false AND it_create = abap_false AND <om_aend>-infty = zmhp_cl_int_constants=>it1000.
          ...
          IF it_change = abap_false AND it_create = abap_false.
            no_change = abap_true.
            begda_ext = zmhp_cl_int_utility=>get_external_date( date = <om_aend>-begda ).
            endda_ext = zmhp_cl_int_utility=>get_external_date( date = <om_aend>-endda ).
            return = VALUE bapiret1( type = zmhp_cl_int_constants=>success
                                     id = zmhp_cl_int_constants=>msg_class_int
                                     number = zmhp_cl_int_constants=>msg_no-m23
                                     message_v1 = <om_aend>-infty
          ...
          ELSE.
            ...
            IF <om_aend>-infty = zmhp_cl_int_constants=>it1001.
              ASSIGN COMPONENT zmhp_cl_int_constants=>sobid OF STRUCTURE <struk> TO FIELD-SYMBOL(<sobid>).
              ASSIGN COMPONENT zmhp_cl_int_constants=>sclas OF STRUCTURE <struk> TO FIELD-SYMBOL(<sclas>).
              ASSIGN COMPONENT zmhp_cl_int_constants=>objid OF STRUCTURE <struk> TO FIELD-SYMBOL(<objid>).
*            IF <sclas> = zmhp_cl_int_constants=>orgunit.
*              IF sap_id_buffer IS NOT INITIAL.
*                <sobid> = sap_id_buffer.
*              ENDIF.
*            ELSE.
...
*            ENDIF.
              ...
              IF <sobid> IS NOT INITIAL.
                "Build parent object
*              DATA(parent) = zmhp_cl_int_utility=>build_object( begda = <om_aend>-begda endda = <om_aend>-endda objid = COND hrobjid( WHEN <sclas> = zmhp_cl_int_constants=>orgunit THEN CONV #( <sobid> )
*                                                                     WHEN <sclas> = zmhp_cl_int_constants=>costcenter THEN CONV #( <objid> ) )
*                                                 otype = COND otype( WHEN <sclas> = zmhp_cl_int_constants=>orgunit AND <om_aend>-otype = zmhp_cl_int_constants=>position THEN zmhp_cl_int_constants=>position
*                                                                     WHEN <sclas> = zmhp_cl_int_constants=>costcenter AND <om_aend>-otype = zmhp_cl_int_constants=>orgunit THEN zmhp_cl_int_constants=>orgunit
*                                                                     WHEN <sclas> = zmhp_cl_int_constants=>orgunit AND <om_aend>-otype = zmhp_cl_int_constants=>orgunit THEN zmhp_cl_int_constants=>orgunit ) ).
*              "Build child object
*              DATA(child) = zmhp_cl_int_utility=>build_object( begda = <om_aend>-begda endda = <om_aend>-endda objid = CONV #( <sobid> )
*                                                 otype = <sclas> bukrs = <om_aend>-legal_entity ).

                IF <om_aend>-subty = zmhp_cl_int_constants=>relations-manager.
                  IF <sobid> = <objid>.
                    zmhp_cl_int_utility=>get_pernr_by_cloudid( EXPORTING cloud_id = CONV #( <om_aend>-relat_obj ) begda = <om_aend>-begda endda = <om_aend>-endda IMPORTING pernr = DATA(manager_pernr) ).
                    zmhp_cl_int_utility=>get_plans_it0001( EXPORTING pernr = manager_pernr begda = <om_aend>-begda endda = <om_aend>-endda IMPORTING plans = DATA(plans) ).
                    <sobid> = plans.
                    CLEAR: plans, manager_pernr.
                  ENDIF.
                ENDIF.

                DATA(parent) = zmhp_cl_int_utility=>build_object( begda = <om_aend>-begda endda = <om_aend>-endda objid = <objid>
                                                   otype = <om_aend>-otype ).
              ...
              ELSE.
                ...
                return = VALUE bapiret1( type = zmhp_cl_int_constants=>success
                                           id = zmhp_cl_int_constants=>msg_class_int
                                           number = zmhp_cl_int_constants=>msg_no-m13
...
*                                  message_v4 =
              ...
              ELSE.
                ...
                return = VALUE bapiret1( type = zmhp_cl_int_constants=>success
                                           id = zmhp_cl_int_constants=>msg_class_int
                                           number = zmhp_cl_int_constants=>msg_no-m12
                                           message_v1 = <om_aend>-sap_id
                                           message_v2 = begda_ext
                                           message_v3 = endda_ext
*                                  message_v4 =
...
*          me->status_handler->add_log_message( aend_id = <om_aend>-aend_id bapiret1 = return hrpad_return = return_tab ).
            ...
            ELSE.
              ...
              IF <om_aend>-infty = zmhp_cl_int_constants=>it1000 AND it_create = abap_true.
                "HRP1000 Creation
                IF <hrp_old> IS INITIAL.
                  ...
                  ELSE.
                    ...
                    return = VALUE bapiret1( type = zmhp_cl_int_constants=>success
                                               id = zmhp_cl_int_constants=>msg_class_int
                                               number = zmhp_cl_int_constants=>msg_no-m8
                                               message_v1 = <om_aend>-sap_id
                                               message_v2 = begda_ext
                                               message_v3 = endda_ext
*                                  message_v4 =
              ...
              ELSE.
                ...
                IF <om_aend>-department_status = 'I'.
                  ...
                  IF <hrp_last> IS INITIAL.
                    return = VALUE bapiret1( type = zmhp_cl_int_constants=>success
                    id = zmhp_cl_int_constants=>msg_class_int
                    number = zmhp_cl_int_constants=>msg_no-m28
                    message_v1 = <om_aend>-sap_id
                    message_v2 = spras
*                                             message_v3 = <om_aend>-endda
*                                  message_v4 =
                  ...
                  ELSE.
                    ...
                    IF relat_exists = abap_true.
                      return = VALUE bapiret1( type = zmhp_cl_int_constants=>error
                      id = zmhp_cl_int_constants=>msg_class_int
                      number = zmhp_cl_int_constants=>msg_no-m30
                      message_v1 = <om_aend>-sap_id
...
*                          return = zmhp_cl_int_utility=>map_sy_msg( msgid = sy-msgid msgty = sy-msgty msgno = sy-msgno msgv1 = sy-msgv1 msgv2 = sy-msgv2 msgv3 = sy-msgv3 msgv4 = sy-msgv4 ).
                ...
                IF return IS INITIAL.
                  ...
                  return = VALUE bapiret1( type = zmhp_cl_int_constants=>success
                                           id = zmhp_cl_int_constants=>msg_class_int
                                           number = zmhp_cl_int_constants=>msg_no-m9
                                           message_v1 = <om_aend>-infty
                                           message_v2 = begda_ext
                                           message_v3 = endda_ext
                                           message_v4 = <om_aend>-sap_id
          ...
          IF return-type NE zmhp_cl_int_constants=>success.
...
*                                               CHANGING  MESSAGE_HANDLER = MESSAGE_HANDLER ).
            ...
            "set error status
            <om_aend>-status = zmhp_cl_int_constants=>booking_status-error.
            IF <om_aend>-sap_id IS INITIAL.
              MODIFY om_aend_tmp FROM <om_aend> TRANSPORTING status WHERE status NE zmhp_cl_int_constants=>booking_status-error.
              APPEND LINES OF om_aend_tmp TO om_aend_error.
...
*              count = count + 1.
          ...
          IF max EQ count.
            ...
            ENDIF.
            "set success status
*          <om_aend>-status = zmhp_cl_int_constants=>booking_status-success.
*
*          MODIFY om_aend_tmp FROM <om_aend> TRANSPORTING status WHERE status NE zmhp_cl_int_constants=>booking_status-success.
*
...
*          count = count + 1.
      ...
      ELSE.
        ...
        return = VALUE bapiret1( type = zmhp_cl_int_constants=>error
                         id = zmhp_cl_int_constants=>msg_class_int
                         number = zmhp_cl_int_constants=>msg_no-m34
...
*      ENDIF.
  ...
  METHOD prepare_protocol.
    ...
    IF message_handler IS BOUND.
      ...
      message_handler->if_hrpay00_pal_services~create_fcat( EXPORTING i_structure_name = 'ZMHP_INT_OM_AEND'
...
*    ENDIF.
  ...
  METHOD start_post.
    ...
    "call post
*    DATA(om_aend_obj) = NEW zmhp_cl_int_om_aend( ).
    me->post_om_aend(
      EXPORTING
        simu            = simu
      IMPORTING
    ...
    "store success status ZMHP_INT_IT_AEND
*    DATA(is_ok) = om_aend_obj->save_entries( om_aend = om_aend_post ).
*
*    "store error status ZMHP_INT_IT_AEND
...
*    ENDIF.
    ...
    DATA(is_ok) = me->status_handler->persist_data( simu = simu source = zmhp_cl_int_constants=>poster ).
  ...
  METHOD update_status.
    ...
    LOOP AT me->om_aend_post ASSIGNING FIELD-SYMBOL(<om_aend_post>).
      me->status_handler->set_status( aend_id = <om_aend_post>-aend_id status = zmhp_cl_int_constants=>booking_status-success ).
    ENDLOOP.
    "Update Error
    LOOP AT me->om_aend_error ASSIGNING FIELD-SYMBOL(<om_aend_error>).
      me->status_handler->set_status( aend_id = <om_aend_error>-aend_id status = zmhp_cl_int_constants=>booking_status-error ).
    ENDLOOP.
    "Update No Change
    LOOP AT me->om_aend_nochange ASSIGNING FIELD-SYMBOL(<om_aend_nochange>).
      me->status_handler->set_status( aend_id = <om_aend_nochange>-aend_id status = zmhp_cl_int_constants=>booking_status-nochange ).
    ENDLOOP.
    ...
    IF simu = abap_true.
      ...
      LOOP AT me->status_handler->simu_om_aend ASSIGNING FIELD-SYMBOL(<simu_om_aend>).
        <simu_om_aend>-status = zmhp_cl_int_constants=>booking_status-simulated.
      ENDLOOP.
    ENDIF.
  ENDMETHOD.
ENDCLASS.
