class ZMHP_CL_INT_IT_OPERATION definition
  public
  final
  create public .
...
public section.
  ...
  methods READ_PAXXXX_DCIF
    importing
      ...
      !IV_ENDDA type SY-DATUM optional
      !IV_PERNR type P_PERNR optional
      !EV_MESSAGE type BAPI_MSG optional
      !ER_MESSAGE_LIST type ref to CL_HRPA_MESSAGE_LIST optional
      !IV_DATA type ANY optional
      !IV_ENTITY type CHAR50 optional
    exporting
  ...
  methods PERFORM_HIRE
    importing
      ...
      !IT_AEND_TAB_REL type ZMHP_TT_IT_AEND optional
      !SIMU type BOOLE_D
    exporting
      !IS_OK type BOOLE_D
      !RETURN_TAB type HRPAD_RETURN_TAB
...
CLASS ZMHP_CL_INT_IT_OPERATION IMPLEMENTATION.
...
*    IF infty = zmhp_cl_int_constants=>it0000 AND action NE zmhp_cl_int_constants=>termination.
    ...
    IF <record_0000> IS ASSIGNED AND <record_0000> IS NOT INITIAL.
      ...
      IF <record_0000>-massn = zmhp_cl_int_constants=>termination.
...
* Implement suitable error handling here
          ...
          ELSE.
            parent-otype = zmhp_cl_int_constants=>orgunit.
            parent-objid = orgeh.
            parent-plvar = '01'.
            ...
            IF return-type NE zmhp_cl_int_constants=>error AND simu NE abap_true.
              "Success create relation
              CALL FUNCTION 'RHOM_WRITE_BUFFER_TO_DB'.
              COMMIT WORK AND WAIT.
*              SUBMIT rhinte20 AND RETURN
*                WITH pchplvar = zmhp_cl_int_constants=>plvar
*                WITH pchotype = zmhp_cl_int_constants=>position
...
*        IF <fs_endda_old> GT delimit_date.
        ...
        EXPORTING
          ...
          operation = zmhp_cl_int_constants=>infty_operation-pa_delete
          simu = simu
          IMPORTING
            return = return
          ).
...
*            ).
        ...
        IF return IS NOT INITIAL."-type = zmhp_cl_int_constants=>error.
...
*        IF <fs_endda_old> GT delimit_date.
        ...
        EXPORTING
          ...
          operation = zmhp_cl_int_constants=>infty_operation-pa_modify
          simu = simu
          IMPORTING
            return = return
          ).
...
*    ASSIGN record TO <record_new>.
    ...
    DATA(molga) = zmhp_cl_int_utility=>get_molga( pernr = pernr begda = begda endda = endda ).

**JMB20210617 delete start
*    SELECT * FROM zmhp_int_infotyp INTO TABLE @DATA(it_cust) WHERE object = @zmhp_cl_int_constants=>person AND molga = @molga.
*    DATA(infotypes) = it_cust.
*    SELECT * FROM zmhp_int_infotyp INTO TABLE @it_cust WHERE object = @zmhp_cl_int_constants=>person AND molga = '*'.
...
*JMB20210617 delete end, insert start
    ...
    SELECT * FROM zmhp_int_infotyp INTO TABLE @DATA(infotypes) WHERE object EQ @zmhp_cl_int_constants=>person AND
                                                                     molga  IN @molga_r.
*JMB20210617 insert end
    ...
    LOOP AT infotypes ASSIGNING FIELD-SYMBOL(<infotype>).
      lr_structdescr ?= cl_abap_typedescr=>describe_by_name( CONV string( 'P' && <infotype>-infty ) ).
...
*          <record_new> = CORRESPONDING #( <record_old> ).
          ...
          ASSIGN COMPONENT zmhp_cl_int_constants=>begda OF STRUCTURE <record_old> TO FIELD-SYMBOL(<begda_old>).
          ASSIGN COMPONENT zmhp_cl_int_constants=>endda OF STRUCTURE <record_old> TO FIELD-SYMBOL(<endda_old>).
          ASSIGN COMPONENT zmhp_cl_int_constants=>begda OF STRUCTURE <record_new> TO FIELD-SYMBOL(<begda_new>).
*      ASSIGN COMPONENT 'STAT2' OF STRUCTURE <record_new> TO FIELD-SYMBOL(<stat_new>).
*      ASSIGN COMPONENT zmhp_cl_int_constants=>massn OF STRUCTURE <record_new> TO FIELD-SYMBOL(<massn_new>).
*      <massn_new> = zmhp_cl_int_constants=>termination.
*      <stat_new> = '0'.
          DATA dat TYPE begda.
          dat = <begda_new>.
          <begda_new> = begda.
...
*
          IF <infotype>-infty EQ zmhp_cl_int_constants=>it0000.
            me->read_paxxxx( EXPORTING begda = <begda_old>
                                       endda = <begda_old>
                                       infty = zmhp_cl_int_constants=>it0302
                                       pernr = pernr
                                       simu  = simu
                             IMPORTING return_tab = return_tab
                                       record_tab = tab_0302 ).

            LOOP AT tab_0302 ASSIGNING FIELD-SYMBOL(<0302_entry>) WHERE massn = zmhp_cl_int_constants=>hire.
...
*JMB20210617 end insert
          ...
          IF <infotype>-infty EQ zmhp_cl_int_constants=>it0000.
            ...
            me->read_paxxxx( EXPORTING begda = <begda_old>
                                       endda = <begda_old>
                                       infty = CONV #( zmhp_cl_int_constants=>it0001 )
                                       pernr = pernr
                                       simu  = simu
                             IMPORTING return_tab = return_tab
                                       record_tab = tab_0001 ).
            ...
            IF sy-subrc IS INITIAL.
              ...
              WITH test EQ simu.
              IF sy-subrc IS INITIAL.
*              ASSIGN COMPONENT zmhp_cl_int_constants=>plans OF STRUCTURE <record_new> TO FIELD-SYMBOL(<plans>).
                SELECT SINGLE * FROM t528b INTO @DATA(t528b) WHERE begda = @<begda_old> AND endda = @<endda_old> AND plans = @p0001-plans AND otype = @zmhp_cl_int_constants=>position.
                IF sy-subrc IS INITIAL.
            ...
            ENDIF.
          ENDIF.
          IF <infotype>-infty EQ zmhp_cl_int_constants=>it0050 OR <infotype>-infty EQ zmhp_cl_int_constants=>it0105.
            action = zmhp_cl_int_constants=>infty_operation-pa_copy.
          ELSE.
            action = zmhp_cl_int_constants=>infty_operation-pa_modify.
          ENDIF.
...
*                                       number    = <record_new>-seqnr
                                         record    = <record_new>
                                         operation = action "zmhp_cl_int_constants=>infty_operation-pa_modify
                                         simu      = simu
                               IMPORTING return    = return ).
          ...
          IF return-type NE zmhp_cl_int_constants=>error.
            ...
            ret = VALUE hrpad_return( type       = zmhp_cl_int_constants=>success
                                      id         = zmhp_cl_int_constants=>msg_class_int
                                      number     = zmhp_cl_int_constants=>msg_no-m22
                                      message_v1 = pernr
                                      message_v2 = <infotype>-infty
*                                   message_v3 = zmhp_cl_int_utility=>read_massn_txt( massn = CONV #( zmhp_cl_int_constants=>hire ) sprsl = spras )
*                                   message_v4 = delimit_date
                                     ).
          ELSE.
...
*    ENDIF.
    ...
    DATA(global_hr) = 'GLOB'.
    SET PARAMETER ID 'GLOB' FIELD global_hr.
    IF infty NE zmhp_cl_int_constants=>it1000.
      global_hr = ''.
      SET PARAMETER ID 'GLOB' FIELD global_hr.
    ENDIF.
...
*          READ TABLE container_tab INTO container INDEX lv_count - 1.
      ...
      ENDIF.
    ENDIF.
    CALL FUNCTION 'ZMHP_FUB_GLOBHR_GET_ERROR_TXT'
      EXPORTING
...
*    CREATE DATA lr_table_old TYPE HANDLE lr_tabledescr.
*    ASSIGN lr_table_old->* TO <record_old_tab>.
*    IF infty NE zmhp_cl_int_constants=>it0001.
    CALL FUNCTION 'HR_INFOTYPE_OPERATION'
      EXPORTING
        infty         = infty
*       recordnumber  = <it_aend>-seqnr
...
*      EXPORTING
*        act_fcode               = action
**        act_plvar               = CONV #( zmhp_cl_int_constants=>plvar )
**       ACT_ISTAT               = '1'
*        vbegda                  = begda
*        vendda                  = endda
**       DEFIBEGDA               =
...
*    CALL FUNCTION 'RHOM_ALL_BUFFER_INIT'.
    ...
    ELSE.
      ...
      IF me->otype = zmhp_cl_int_constants=>orgunit.

        lr_structdescr ?= cl_abap_typedescr=>describe_by_name( CONV string( 'P' && zmhp_cl_int_constants=>it1000 ) ).
        CREATE DATA hrp_old_it TYPE HANDLE lr_structdescr.
        ASSIGN hrp_old_it->* TO <hrp_old>.
        ...
        EXPORTING
          begda = begda
          endda = endda
          infty = CONV #( zmhp_cl_int_constants=>it1000 )
          langu = CONV #( spras )
          otype = me->otype
          sap_id = child-objid
          subty = ''
      ...
      IF subty = zmhp_cl_int_constants=>relations-manager.
        is_manager = abap_true.
      ENDIF.
      IF <hrp_old> IS ASSIGNED.
      CLEAR: <hrp_old>.
      lr_structdescr ?= cl_abap_typedescr=>describe_by_name( CONV string( 'P' && zmhp_cl_int_constants=>it1001 ) ).
      CREATE DATA hrp_old_it TYPE HANDLE lr_structdescr.
      ASSIGN hrp_old_it->* TO <hrp_old>.
      ...
      ASSIGN COMPONENT zmhp_cl_int_constants=>sobid OF STRUCTURE <hrp_old> TO FIELD-SYMBOL(<sobid_old>).
...
METHOD MAINTAIN_TEVEN_HC.
  ...
  IF sy-subrc <> 0.
    return = VALUE #( type       = zmhp_cl_int_constants=>error
                      id         = zmhp_cl_int_constants=>msg_class_int
                      number     = zmhp_cl_int_constants=>msg_no-m32
                      message_v1 = zmhp_cl_int_constants=>it2011
                      message_v2 = pernr
                      message_v3 = begda ).
  ENDIF.
...
*          return_tab     TYPE hrpad_return_tab,
          ...
          p0000          TYPE p0000,
          p0001          TYPE p0001,
          it_aendup      TYPE zmhp_int_it_aeup,
          record_old     TYPE REF TO data,
          ret            LIKE LINE OF return_tab,
          return         TYPE bapiret1.
    ...
    ELSE.
      ...
      lr_structdescr ?= cl_abap_typedescr=>describe_by_name( CONV string( 'P' && zmhp_cl_int_constants=>it0001 ) ).
      CREATE DATA record_old TYPE HANDLE lr_structdescr.
      ASSIGN record_old->* TO <record_old>.
      lr_tabledescr ?= cl_abap_tabledescr=>create( p_line_type = lr_structdescr ).
      CREATE DATA lr_table_old TYPE HANDLE lr_tabledescr.
...
*        CLEAR: ret, return.
        IF <fs_record_old> IS ASSIGNED.
          ASSIGN COMPONENT zmhp_cl_int_constants=>endda OF STRUCTURE <fs_record_old> TO FIELD-SYMBOL(<fs_endda_old>).
          ASSIGN COMPONENT zmhp_cl_int_constants=>begda OF STRUCTURE <fs_record_old> TO FIELD-SYMBOL(<fs_begda_old>).
          ASSIGN COMPONENT zmhp_cl_int_constants=>plans OF STRUCTURE <fs_record_old> TO FIELD-SYMBOL(<fs_pos_old>).
*        IF <fs_endda_old> GT delimit_date.
          <fs_pos_old> = zmhp_cl_int_constants=>default_pos.
          "Delete old record
          me->maintain_paxxxx(
          EXPORTING
            begda = <fs_begda_old>
...
*    IF is_ok = abap_false.
*      DATA(error_handler) = NEW zmhp_cl_int_error_handler( cloud_id = CONV #( it_aend-cloud_id ) aend_id = CONV #( it_aend-aend_id )
...
*          return_tab     TYPE hrpad_return_tab,
    ...
    IF it_aend_tab_rel IS INITIAL.
      it_aend_tmp = VALUE zmhp_tt_it_aend( FOR ls_it_aend IN it_aend_tab WHERE ( int_run = me->int_run AND
...
*      prelp-endda = zmhp_cl_int_constants=>highdate.
      IF aend_tmp-infty = '0000'.
        ...
        IF aend_tmp-action = zmhp_cl_int_constants=>hire.
          DATA(begda) = aend_tmp-begda.
          ASSIGN COMPONENT zmhp_cl_int_constants=>massn OF STRUCTURE p0000 TO FIELD-SYMBOL(<massn>).
          ASSIGN COMPONENT zmhp_cl_int_constants=>massg OF STRUCTURE p0000 TO FIELD-SYMBOL(<massg>).
        ENDIF.
      ENDIF.
      ...
      TRY.
          CALL FUNCTION 'HR_PAD_HIRE_EMPLOYEE'
            EXPORTING
              employeenumber = pernr_n
              hiringdate     = begda
              actiontype     = zmhp_cl_int_constants=>hire
*             reasonforaction = <massg> " muss noch angepasst werden!
              pnnnn_tab      = prelp_tab
              nocommit       = 'X' "No commit, only write to buffer, database entries only written when all entries processed correctly
            IMPORTING
    ...
    IF is_ok = abap_true.
      ...
      IF <bapipakey>-employeeno IS NOT INITIAL.
        ...
        IF simu NE abap_true.
          DATA(it_aend_up) = NEW zmhp_cl_int_it_aendup( ).
          ...
          it_aendup-int_run = zmhp_cl_int_utility=>create_guid( ).
          ...
          it_aendup-status = zmhp_cl_int_constants=>booking_status-initial.
  ...
  METHOD perform_reverse_termination.
    ...
    ASSIGN record TO <record_new>.
    "Only perform reverse termination block if current record being processed is IT0000 and action not equal termination
    IF infty = zmhp_cl_int_constants=>it0000 AND action NE zmhp_cl_int_constants=>termination AND action NE zmhp_cl_int_constants=>rehire.
      CLEAR: is_rev_term.
      ...
      IF is_rev_term = abap_true.
        ...
        READ TABLE return_tab INTO DATA(return) WITH KEY type = zmhp_cl_int_constants=>error.
...
*              return = return_maint
*            ).
*          IF return_maint IS NOT INITIAL."-type = zmhp_cl_int_constants=>error.
...
*                                  record_tab = tab_0001 ).
  ...
  METHOD read_hrpxxxx.
    DATA: lr_structdescr TYPE REF TO cl_abap_structdescr,
          lr_tabledescr  TYPE REF TO cl_abap_tabledescr,
          relat_tmp TYPE relat,
          rsign_tmp TYPE rsign,
          lr_table       TYPE REF TO data.
    FIELD-SYMBOLS:  <hrp_old_tab> TYPE STANDARD TABLE.
...
*        READ TABLE <hrp_old_tab> ASSIGNING <hrp_old> WITH KEY langu = langu.
          LOOP AT <hrp_old_tab> ASSIGNING <hrp_old>.
            ...
            ENDIF.
          ENDLOOP.
        ELSEIF infty = zmhp_cl_int_constants=>it1001.
          IF <hrp_old> IS ASSIGNED.
            UNASSIGN <hrp_old>.
          ENDIF.
          IF rsign_tmp IS NOT INITIAL AND relat_tmp IS NOT INITIAL.
*        READ TABLE <hrp_old_tab> ASSIGNING <hrp_old> WITH KEY langu = langu.
            LOOP AT <hrp_old_tab> ASSIGNING <hrp_old>.
              ...
              IF <relat> = relat_tmp AND <rsign> = rsign_tmp.
...
*     IMPORTING
      ...
      EXCEPTIONS
        ...
        OTHERS          = 3.
    IF sy-subrc <> 0.
      return = zmhp_cl_int_utility=>map_sy_msg( msgid = sy-msgid msgty = sy-msgty msgno = sy-msgno
...
*          lv_begda                    TYPE begda,
*          lv_endda                    TYPE endda,
*          lc_hcm_globalhr_person      TYPE REF TO zmhp_cl_hcm_globalhr_person,
*          lt_prelp                    TYPE hrpad_prelp_tab,
*          lr_infty_reader             TYPE REF TO if_hrpa_read_infotype,
*          lr_message_list             TYPE REF TO cl_hrpa_message_list,
*          lr_masterdata_bl            TYPE REF TO if_hrpa_masterdata_bl,
