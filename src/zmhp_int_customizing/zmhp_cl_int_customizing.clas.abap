class ZMHP_CL_INT_CUSTOMIZING definition
  public
  final
  create public .
...
public section.
  ...
  types:
    t_fields TYPE TABLE OF zmhp_int_fields .
  types:
  ...
  methods READ_REL_FIELDS
    exporting
      !REL_FIELDS type ZMHP_TT_REL_FLD
      !IS_OK type BOOLE_D .
...
CLASS ZMHP_CL_INT_CUSTOMIZING IMPLEMENTATION.
  ...
  METHOD get_components.
    ...
    ELSE.
      lv_structure = 'zmhp_dummy_fields'.
    ENDIF.
  ...
  METHOD read_customizing.
    DATA(infty) = iv_infty.
    SELECT * FROM zmhp_int_fields INTO TABLE @me->gt_customizing WHERE molga = @iv_molga
  ...
  METHOD read_rel_fields.
    SELECT * FROM zmhp_int_rel_fld INTO TABLE @rel_fields WHERE infty = @me->infty
                                                                AND molga = @me->molga.
    IF sy-subrc = 0.
      is_ok = abap_true.
    ELSE.
