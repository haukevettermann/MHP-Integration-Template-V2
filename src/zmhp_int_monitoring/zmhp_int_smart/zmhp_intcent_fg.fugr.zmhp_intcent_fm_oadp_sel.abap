FUNCTION ZMHP_INTCENT_FM_OADP_SEL.
*"----------------------------------------------------------------------
*"*"Lokale Schnittstelle:
*"  IMPORTING
*"     REFERENCE(USER) TYPE  SYUNAME
*"     REFERENCE(BEGDA) TYPE  OBJEC-BEGDA
*"     REFERENCE(ENDDA) TYPE  OBJEC-ENDDA
*"     REFERENCE(EVPATH) TYPE  GDSTR-WEGID
*"     REFERENCE(EVALPATH) TYPE  GDSTR-WEGID
*"     REFERENCE(DEPTH) TYPE  GDSTR-DEPTH
*"     REFERENCE(PLVAR) LIKE  OBJEC-PLVAR
*"  TABLES
*"      PATHROOTS STRUCTURE  HRROOTOB
*"      ROOT_OBJECTS STRUCTURE  HRROOTOB
*"      ROOT_OBJEC STRUCTURE  OBJEC
*"      ROOT_STRUC STRUCTURE  STRUC
*"      PARAMVALUES STRUCTURE  HRWPC_S_OADP_PARAMVALUE
...
* read sachx table, furthermore check for super user
        ...
        SELECT * FROM zmhp_int_superus INTO TABLE @DATA(lt_superus) WHERE uname = @ls_result_struc-objid.
