class ZMHP_CL_SMART_BI_IMPL_EXT_RULE definition
  public
  final
  create public .
...
CLASS ZMHP_CL_SMART_BI_IMPL_EXT_RULE IMPLEMENTATION.
  ...
  METHOD /mhp/smart_if_rule_ext~apply_customizingrule_on_table.
    ...
    IF is_rule-rule_function CP 'ZMHP_STATUS_TEXT*'.
...
* status texte vervollständigen
      LOOP AT ct_table ASSIGNING <ls_table>.
        ...
        ENDIF.
      ENDLOOP.
    ELSEIF is_rule-rule_function CP 'ZMHP_MSGSTATUS_TEXT*'.
      CALL FUNCTION 'DD_DOMVALUES_GET'
        EXPORTING
...
* status texte vervollständigen
      LOOP AT ct_table ASSIGNING <ls_table>.
        ...
        ENDIF.
      ENDLOOP.
    ELSEIF is_rule-rule_function CP 'ZMHP_MSGTYPE_TEXT*'.
      CALL FUNCTION 'DD_DOMVALUES_GET'
        EXPORTING
          domname        = 'CACSMSGTYPE'
          text           = 'X'
...
* status texte vervollständigen
      LOOP AT ct_table ASSIGNING <ls_table> WHERE ('msg_type is not initial').
        ...
        ENDIF.
      ENDLOOP.
    ELSEIF is_rule-rule_function CP 'ZMHP_OBJECTTYPE_TEXT*'.
* status texte vervollständigen
      LOOP AT ct_table ASSIGNING <ls_table> WHERE ('otype is not initial').
        ...
        ENDIF.
      ENDLOOP.
    ELSEIF is_rule-rule_function CP 'ZMHP_OBJECT_DETAILS*'.
