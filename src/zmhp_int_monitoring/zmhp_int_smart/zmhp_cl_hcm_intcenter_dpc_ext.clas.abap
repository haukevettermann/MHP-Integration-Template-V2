class ZMHP_CL_HCM_INTCENTER_DPC_EXT definition
  public
  inheriting from ZMHP_CL_HCM_INTCENTER_DPC
  create public .
...
private section.
  ...
  methods ENHANCE_ENTITY_FIELDCHANGE
    changing
      !CT_ENTITY type ZMHP_CL_HCM_INTCENTER_MPC=>TT_FIELDCHANGE optional
      !CS_ENTITY type ZMHP_CL_HCM_INTCENTER_MPC=>TS_FIELDCHANGE optional .
  methods ENHANCE_ENTITY_IT_AEND
    changing
      !CT_ENTITY type ZMHP_CL_HCM_INTCENTER_MPC=>TT_INFOTYPECHANGE optional
      !CS_ENTITY type ZMHP_CL_HCM_INTCENTER_MPC=>TS_INFOTYPECHANGE optional .
  methods ENHANCE_ENTITY_MESSAGE
    changing
      !CT_ENTITY type ZMHP_CL_HCM_INTCENTER_MPC=>TT_MESSAGE optional
      !CS_ENTITY type ZMHP_CL_HCM_INTCENTER_MPC=>TS_MESSAGE optional .
  methods ENHANCE_ENTITY_OM_AEND
    changing
      !CT_ENTITY type ZMHP_CL_HCM_INTCENTER_MPC=>TT_ORGCHANGE optional
      !CS_ENTITY type ZMHP_CL_HCM_INTCENTER_MPC=>TS_ORGCHANGE optional .
  ...
  methods ENHANCE_ENTITY_INTRUN
    changing
      !CS_ENTITY type ZMHP_CL_HCM_INTCENTER_MPC=>TS_INTEGRATIONRUN optional .
ENDCLASS.



CLASS ZMHP_CL_HCM_INTCENTER_DPC_EXT IMPLEMENTATION.
  ...
  METHOD /iwbep/if_mgw_appl_srv_runtime~changeset_process.
    DATA:
      ls_entity_changes TYPE zmhp_cl_hcm_intcenter_mpc=>ts_fieldchange,
  ...
  METHOD /iwbep/if_mgw_appl_srv_runtime~execute_action.
    DATA:
      ls_int_run_entity TYPE zmhp_cl_hcm_intcenter_mpc=>ts_integrationrun,
...
* daten lesen
        SELECT SINGLE int_run, sap_id, cloud_id, otype, stream_direction, MIN( timestamp ) AS timestamp, SUM( counter_msg ) AS counter_msg,
          SUM( counter_msg_e ) AS counter_msg_e FROM zmhp_int_center_le INTO CORRESPONDING FIELDS OF @ls_int_run_entity WHERE int_run = @ls_int_run_entity-int_run
...
* delete old messages
          ...
          IF ls_int_run_entity-otype = 'P'.
            SELECT * FROM zmhp_int_it_aend INTO TABLE @DATA(it_aend_tab) WHERE int_run = @ls_int_run_entity-int_run AND cloud_pernr = @ls_int_run_entity-cloud_id.
*                                                                    AND pernr = ls_int_run_entity-sap_id
            ...
            ENDLOOP.
          ELSE.
            SELECT * FROM zmhp_int_om_aend INTO TABLE @DATA(om_aend_tab) WHERE int_run = @ls_int_run_entity-int_run AND cloud_id = @ls_int_run_entity-cloud_id.
*                                                                    AND pernr = ls_int_run_entity-sap_id
          ...
          ENDIF.
          IF ls_int_run_entity-otype = 'P'.
            UPDATE zmhp_int_it_aend SET status = @lv_new_status, aedtm = @sy-datlo, uname = @sy-uname WHERE int_run = @ls_int_run_entity-int_run
              AND pernr = @ls_int_run_entity-sap_id AND cloud_pernr = @ls_int_run_entity-cloud_id.
          ELSEIF ls_int_run_entity-otype = 'O'.
            UPDATE zmhp_int_om_aend SET status = @lv_new_status, aedtm = @sy-datlo, uname = @sy-uname WHERE int_run = @ls_int_run_entity-int_run
          ...
          ENDIF.
          zmhp_cl_int_utility=>start_int_run( int_run = ls_int_run_entity-int_run cloud_id = CONV #( cloud_id )
  ...
  METHOD /iwbep/if_mgw_appl_srv_runtime~get_stream.
    DATA:
      ls_image    TYPE zmhp_cl_hcm_intcenter_mpc=>ts_image,
...
**ENDTRY.
  ...
  METHOD enhance_entity_fieldchange.
    ...
    LOOP AT ct_entity ASSIGNING FIELD-SYMBOL(<ls_entity>).
      ...
      TRY.
          SELECT SINGLE otype INTO @DATA(lv_otype) FROM zmhp_int_v_cun WHERE aend_id = @<ls_entity>-aend_id.
          IF lv_otype = 'P'.
      ...
      <ls_entity>-imageurl = |/sap/opu/odataZMHP_HCM_INTCENTER_SRV/ImageSet(ObjectType='F',ObjectID='')/$value|.
    ENDLOOP.
  ENDMETHOD.
  ...
  METHOD enhance_entity_intrun.
    ...
    cs_entity-imageurl = |/sap/opu/odataZMHP_HCM_INTCENTER_SRV/ImageSet(ObjectType='{ cs_entity-otype }',ObjectID='{ cs_entity-sap_id }')/$value|.

* user selektieren
    TYPES: ty_objid_range TYPE RANGE OF objid.
    /mhp/smart_cl_repo_util=>get_orgview_assignments( EXPORTING iv_orgsel = 'ZMHP_INTCENT_US'
    ...
    SELECT SINGLE SUM( counter_chg ) AS counter_chg, SUM( counter_chg_e ) AS counter_chg_e
      FROM zmhp_int_center_un INTO CORRESPONDING FIELDS OF @cs_entity WHERE int_run = @cs_entity-int_run
    ...
    SELECT SINGLE SUM( counter_msg ) AS counter FROM zmhp_int_center_le INTO @DATA(lv_sum_status_03)
    ...
    SELECT SINGLE SUM( counter_msg ) AS counter FROM zmhp_int_center_le INTO @DATA(lv_sum_status_01)
      WHERE int_run = @cs_entity-int_run AND sap_id = @cs_entity-sap_id AND cloud_id = @cs_entity-cloud_id
      AND status = '01' AND sachb_sbgrpcode IN @lt_objsel.

    SELECT SINGLE SUM( counter_msg ) AS counter FROM zmhp_int_center_le INTO @DATA(lv_sum_status_07)
      WHERE int_run = @cs_entity-int_run AND sap_id = @cs_entity-sap_id AND cloud_id = @cs_entity-cloud_id
...
* no errors and some initial
  ...
  METHOD enhance_entity_it_aend.
    ...
    IF <ls_changing_entity> IS ASSIGNED.
      ...
      cs_entity-imageurl = |/sap/opu/odataZMHP_HCM_INTCENTER_SRV/ImageSet(ObjectType='I',ObjectID='{ cs_entity-infty }')/$value|.
...
* user selektieren
      ...
      SELECT SINGLE SUM( counter_msg ) AS counter_msg, SUM( counter_msg_e ) AS counter_msg_e FROM zmhp_int_center_le
      ...
      IF cs_entity-status = '03'.
        SELECT COUNT( * ) FROM zmhp_int_msg_f INTO @DATA(lv_editable_count) WHERE aend_id = @cs_entity-aend_id
  ...
  METHOD enhance_entity_message.
    ...
    CALL FUNCTION 'DD_DOMVALUES_GET'
      EXPORTING
        domname        = 'ZMHP_DD_MSG_STATUS_DOM'
        text           = 'X'
        langu          = sy-langu
      TABLES
        dd07v_tab      = lt_idd07v
    ...
    LOOP AT ct_entity ASSIGNING FIELD-SYMBOL(<ls_entity>).
      ...
      SELECT SINGLE molga FROM zmhp_int_center_le INTO @DATA(lv_molga) WHERE aend_id = @<ls_entity>-aend_id AND msg_id = @<ls_entity>-msg_guid.
      SELECT SINGLE msgtext FROM zmhp_int_notes INTO @<ls_entity>-solution_description WHERE molga = @lv_molga AND
        msgid = @<ls_entity>-id AND msgno = @note_nr AND msgty = @<ls_entity>-type.
      IF <ls_entity>-solution_description IS INITIAL.
        SELECT SINGLE msgtext FROM zmhp_int_notes INTO @<ls_entity>-solution_description WHERE molga = '*' AND
          msgid = @<ls_entity>-id AND msgno = @note_nr AND msgty = @<ls_entity>-type.
      ENDIF.
    ...
    IF <ls_changing_entity> IS ASSIGNED.
      ...
      SELECT SINGLE int_run, SUM( counter_msg ) AS counter FROM zmhp_int_center_le INTO @DATA(ls_sum)
      ...
      cs_entity-imageurl = |/sap/opu/odataZMHP_HCM_INTCENTER_SRV/ImageSet(ObjectType='M',ObjectID='{ cs_entity-type }')/$value|.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD enhance_entity_om_aend.
    ...
    IF <ls_changing_entity> IS ASSIGNED.
      ...
      cs_entity-imageurl = |/sap/opu/odataZMHP_HCM_INTCENTER_SRV/ImageSet(ObjectType='O',ObjectID='{ cs_entity-sap_id }')/$value|.
...
* daten lesen
    SELECT SINGLE * FROM zmhp_int_msg_f INTO CORRESPONDING FIELDS OF @er_entity WHERE aend_id = @ls_full_key-aend_id
...
* daten lesen
    SELECT * FROM zmhp_int_msg_f INTO CORRESPONDING FIELDS OF TABLE @et_entityset WHERE aend_id = @ls_full_key-aend_id.
...
* tech context anwenden
  ...
  METHOD fieldchangeset_update_entity.
    ...
    DATA(ls_fields) = CORRESPONDING zmhp_int_msg_f( er_entity ).

* daten lesen
    SELECT SINGLE int_run, pernr AS sap_id, cloud_id, molga FROM zmhp_int_it_aend INTO @DATA(ls_int_run)
      WHERE aend_id = @ls_fields-aend_id.
    IF sy-subrc <> 0.
      SELECT SINGLE int_run, sap_id, cloud_id, molga FROM zmhp_int_om_aend INTO @ls_int_run
        WHERE aend_id = @ls_fields-aend_id.
    ENDIF.

    IF ls_int_run IS NOT INITIAL.
      DATA(lo_instance) = zmhp_cl_int_status_handler=>get_instance( EXPORTING cloud_id = CONV #( ls_int_run-cloud_id )
      ...
      lo_instance->persist_data( source = zmhp_cl_int_constants=>cockpit ).
    ENDIF.
  ENDMETHOD.
...
* daten lesen
    SELECT SINGLE * FROM zmhp_int_it_aend INTO CORRESPONDING FIELDS OF @er_entity WHERE int_run = @ls_full_key-int_run
...
* daten lesen
    IF ls_full_key-aend_id IS INITIAL.
      SELECT * FROM zmhp_int_it_aend INTO CORRESPONDING FIELDS OF TABLE @et_entityset WHERE int_run = @ls_full_key-int_run
...
* daten lesen
    SELECT SINGLE int_run, sap_id, cloud_id, otype, stream_direction, MIN( timestamp ) AS timestamp, SUM( counter_msg ) AS counter_msg,
      SUM( counter_msg_e ) AS counter_msg_e FROM zmhp_int_center_le INTO CORRESPONDING FIELDS OF @er_entity WHERE int_run = @er_entity-int_run
  ...
  METHOD integrationrunse_get_entityset.
    ...
    IF sy-uname = 'DEKUCCHR'.
      SELECT  int_run, sap_id, cloud_id, otype, stream_direction,
        SUM( counter_msg ) AS counter FROM zmhp_int_center_le INTO CORRESPONDING FIELDS OF TABLE @et_entityset
        GROUP BY int_run, sap_id, cloud_id, otype, stream_direction.
    ENDIF.
  ENDMETHOD.
...
* daten lesen
    SELECT SINGLE * FROM zmhp_int_msg_l INTO CORRESPONDING FIELDS OF @er_entity WHERE aend_id = @ls_full_key-aend_id
...
* daten lesen
    IF ls_full_key-field_id IS NOT INITIAL.
      SELECT 'I' AS sign, 'EQ' AS option, msg_guid AS low FROM zmhp_int_msg_f INTO TABLE @lt_range_guid32
        WHERE aend_id = @ls_full_key-aend_id AND field_id = @ls_full_key-field_id.
      IF lt_range_guid32 IS NOT INITIAL.
        SELECT * FROM zmhp_int_msg_l INTO CORRESPONDING FIELDS OF TABLE @et_entityset WHERE msg_guid IN @lt_range_guid32.
      ENDIF.
    ELSEIF ls_full_key-msg_guid IS NOT INITIAL.
      SELECT * FROM zmhp_int_msg_l INTO CORRESPONDING FIELDS OF TABLE @et_entityset WHERE msg_guid = @ls_full_key-msg_guid.
    ELSEIF ls_full_key-aend_id IS NOT INITIAL.
      SELECT * FROM zmhp_int_msg_l INTO CORRESPONDING FIELDS OF TABLE @et_entityset WHERE aend_id = @ls_full_key-aend_id.
    ELSEIF ls_full_key-int_run IS NOT INITIAL AND ( ls_full_key-sap_id IS NOT INITIAL OR ls_full_key-cloud_id IS NOT INITIAL ).
      SELECT 'I' AS sign, 'EQ' AS option, aend_id AS low FROM zmhp_int_center_un INTO TABLE @lt_range_guid32
        WHERE int_run = @ls_full_key-int_run AND sap_id = @ls_full_key-sap_id AND cloud_id = @ls_full_key-cloud_id.
      IF lt_range_guid32 IS NOT INITIAL.
        SELECT * FROM zmhp_int_msg_l INTO CORRESPONDING FIELDS OF TABLE @et_entityset WHERE aend_id IN @lt_range_guid32.
      ENDIF.
    ENDIF.
...
* daten lesen
    SELECT SINGLE * FROM zmhp_int_om_aend INTO CORRESPONDING FIELDS OF @er_entity WHERE int_run = @ls_full_key-int_run
...
* daten lesen
    IF ls_full_key-aend_id IS INITIAL.
      SELECT * FROM zmhp_int_om_aend INTO CORRESPONDING FIELDS OF TABLE @et_entityset WHERE int_run = @ls_full_key-int_run
