class ZMHP_CL_INT_ALV definition
  public
  final
  create public .
...
public section.
  ...
  types TS_STATUS type ref to ZMHP_CL_INT_STATUS_HANDLER .
  ...
  data ERROR_ENTRIES type ZMHP_CL_INT_STATICS=>T_IT_AEND .
  data SUCCESSFUL_ENTRIES_PA type ZMHP_CL_INT_STATICS=>T_IT_AEND .
  data SUCCESSFUL_ENTRIES_OM type ZMHP_CL_INT_STATICS=>T_OM_AEND .
  data ERROR_ENTRIES_PA type ZMHP_CL_INT_STATICS=>T_IT_AEND .
  data ERROR_ENTRIES_OM type ZMHP_CL_INT_STATICS=>T_OM_AEND .
...
CLASS ZMHP_CL_INT_ALV IMPLEMENTATION.
...
* Create ALV and set for display
    me->successful_entries_pa = VALUE zmhp_cl_int_statics=>t_it_aend( FOR it IN zmhp_cl_int_statics=>it_aend WHERE ( status NE zmhp_cl_int_constants=>booking_status-error_des ) ( it ) ).
    IF me->successful_entries_pa IS NOT INITIAL.
      ...
      alv_grid_1->set_table_for_first_display(
        ...
        EXCEPTIONS
          ...
          program_error = 2
          too_many_lines = 3 ).
      me->error_entries_pa = VALUE zmhp_cl_int_statics=>t_it_aend( FOR it IN zmhp_cl_int_statics=>it_aend WHERE ( status EQ zmhp_cl_int_constants=>booking_status-error_des ) ( it ) ).
      ...
      alv_grid_2->set_table_for_first_display(
        ...
        EXCEPTIONS
          ...
          too_many_lines = 3 ).
    ELSE.
      me->successful_entries_om = VALUE zmhp_cl_int_statics=>t_om_aend( FOR it_om IN zmhp_cl_int_statics=>om_aend WHERE ( status NE zmhp_cl_int_constants=>booking_status-error_des ) ( it_om ) ).
      layout-zebra = abap_true.
      layout-grid_title = 'Erfolgreich Deserialisiert'.
      alv_grid_1->set_table_for_first_display(
        EXPORTING
          i_structure_name = 'ZMHP_INT_OM_AEND'
          is_layout = layout
        CHANGING
        ...
        EXCEPTIONS
          ...
          program_error = 2
          too_many_lines = 3 ).
      me->error_entries_om = VALUE zmhp_cl_int_statics=>t_om_aend( FOR it_om IN zmhp_cl_int_statics=>om_aend WHERE ( status EQ zmhp_cl_int_constants=>booking_status-error_des ) ( it_om ) ).
      layout-grid_title = 'Fehler bei Deserialisierung'.
      alv_grid_2->set_table_for_first_display(
        EXPORTING
          i_structure_name = 'ZMHP_INT_OM_AEND'
        ...
        EXCEPTIONS
          ...
          too_many_lines = 3 ).
    ENDIF.
    CLEAR: zmhp_cl_int_statics=>messages.
...
*    SET HANDLER gref_event_receiver->handle_user_command FOR lo_alv_grid_2.
  ...
  METHOD build_log.
    ...
    IF zmhp_cl_int_statics=>messages IS INITIAL AND zmhp_cl_int_statics=>it_aend IS INITIAL AND zmhp_cl_int_statics=>om_aend IS INITIAL.
      WRITE 'Keine Änderung'.
    ELSE.
      NEW-LINE.
      IF zmhp_cl_int_statics=>it_aend IS NOT INITIAL.
        WRITE: 0'PERNR', 10'CLOUD ID', 26'MOLGA', 31'BEGDA', 40'ENDDA', 49'MASSN', 53'INFTY', 59'SUBTY', 65'DATA1'.
        LOOP AT zmhp_cl_int_statics=>it_aend ASSIGNING FIELD-SYMBOL(<it_aend>).
          NEW-LINE.
        ...
        ENDLOOP.
      ENDIF.
      IF zmhp_cl_int_statics=>om_aend IS NOT INITIAL.
        WRITE: 0'PERNR', 10'CLOUD ID', 26'MOLGA', 31'BEGDA', 40'ENDDA', 49'INFTY', 55'SUBTY', 61'VDATA'.
        LOOP AT zmhp_cl_int_statics=>om_aend ASSIGNING FIELD-SYMBOL(<om_aend>).
          NEW-LINE.
        ...
        ENDLOOP.
      ENDIF.
      IF zmhp_cl_int_statics=>messages IS NOT INITIAL.
        WRITE: 0'SIMULATION', 4'TYPE', 8'ID', 40'VAR 1', 60'VAR 2', 80'VAR 3', 100'VAR 4'.
        LOOP AT zmhp_cl_int_statics=>messages ASSIGNING FIELD-SYMBOL(<messages>).
          NEW-LINE.
...
* Create infotype table
      ...
      cl_hr_pnnnn_type_cast=>prelp_to_pnnnn_tab(
        ...
        IMPORTING
          pnnnn_tab = <table> ).
    ELSE.
      DATA(selected_entry_om) = COND zmhp_int_om_aend(
