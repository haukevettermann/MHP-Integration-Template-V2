class ZMHP_CL_REM_CHANGES definition
  public
  create public .
...
public section.
  ...
  types:
    REM_MAP_TT type table of zmhp_int_rem_map .
  types:
    REM_UPD_TT type table of zmhp_int_rem_upd .
  types:
  ...
  data INT_REM_UPD type ZMHP_TT_REM_UPD .
  ...
  methods CREATE_ENTRIES
    importing
      ...
      !MOLGA type MOLGA
      !PERNR type PERNR_D
      !INT_REM_MAP type ZMHP_CL_REM_CHANGES=>REM_MAP_TT
      !PERIOD type RSDSSELOPT_T
    exporting
      !INT_REM_UPD type ZMHP_CL_REM_CHANGES=>REM_UPD_TT .
  methods DETECT_CHANGES
    importing
      ...
      !PAYXX_RESULT type ANY TABLE
    exporting
      !REM_UPD_CHANGES type ZMHP_CL_REM_CHANGES=>REM_UPD_TT .
  ...
  methods SAVE_ENTRIES
    importing
      !REM_UPD type ZMHP_CL_REM_CHANGES=>REM_UPD_TT
    returning
...
private section.
  ...
  methods COMPARE_TABLES
    importing
      !REM_UPD_OLD type ZMHP_CL_REM_CHANGES=>REM_UPD_TT
      !REM_UPD_NEW type ZMHP_CL_REM_CHANGES=>REM_UPD_TT
    exporting
      !REM_UPD_DEL type ZMHP_CL_REM_CHANGES=>REM_UPD_TT
      !REM_UPD_ADD type ZMHP_CL_REM_CHANGES=>REM_UPD_TT
      !REM_UPD_MOD type ZMHP_CL_REM_CHANGES=>REM_UPD_TT
      !REM_UPD_NO_CHANGES type ZMHP_CL_REM_CHANGES=>REM_UPD_TT
      !NO_CHANGES type FLAG .
ENDCLASS.



CLASS ZMHP_CL_REM_CHANGES IMPLEMENTATION.


  METHOD compare_tables.

    DATA: rem_upd_line TYPE zmhp_int_rem_upd.
...
*          <rem_upd_new>-aedtm NE rem_upd_old_line-aedtm OR
  ...
  METHOD create_entries.
    DATA: rem_upd_t TYPE STANDARD TABLE OF zmhp_int_rem_upd,
          rem_upd_l TYPE zmhp_int_rem_upd.
...
*    ENDLOOP.
  ...
  METHOD detect_changes.
    ...
    DATA: change_date        TYPE dats,
          rem_upd_t          TYPE STANDARD TABLE OF zmhp_int_rem_upd,
          rem_upd_del        TYPE STANDARD TABLE OF zmhp_int_rem_upd,
          rem_upd_add        TYPE STANDARD TABLE OF zmhp_int_rem_upd,
          rem_upd_mod        TYPE STANDARD TABLE OF zmhp_int_rem_upd,
          no_changes         TYPE flag,
          rem_upd_tmp        TYPE STANDARD TABLE OF zmhp_int_rem_upd,
          rem_upd_no_changes TYPE STANDARD TABLE OF zmhp_int_rem_upd.


    SELECT * FROM zmhp_int_rem_map WHERE begda <= @sy-datum
                                   AND   endda >= @sy-datum
    ...
    SELECT * FROM zmhp_int_rem_upd WHERE molga     = @molga
                                   AND   pernr     = @pernr
...
*    *    SELECT SINGLE strukname FROM zmhp_int_rem_map WHERE molga = @molga INTO @strukname.
*    SELECT strukname FROM zmhp_int_rem_map WHERE molga = @molga INTO TABLE @DATA(struknames).
*
*
...
*    ENDIF.
  ...
  METHOD prepare_protocol.
    ...
    IF message_handler IS BOUND.
      ...
      message_handler->if_hrpay00_pal_services~create_fcat( EXPORTING i_structure_name = 'ZMHP_INT_REM_UPD'
...
*      ENDIF.
  ...
  METHOD read_cluster.
    ...
    DATA: periods         TYPE zmhp_cl_rem_changes=>periods_t,
          pernr           TYPE pernr_d,
          molga           TYPE molga,
          rem_upd         TYPE TABLE OF zmhp_int_rem_upd,
          rem_upd_changes TYPE TABLE OF zmhp_int_rem_upd.
    ...
    LOOP AT ir_pernr ASSIGNING FIELD-SYMBOL(<pernr>).
      ...
      molga = zmhp_cl_forms_utils=>get_molga( CONV #( <pernr>-low ) ).
  ...
  METHOD read_cluster_force.
    ...
    DATA: periods     TYPE zmhp_cl_forms_utils=>periods_t,
          pernr       TYPE pernr_d,
          int_rem_upd TYPE TABLE OF zmhp_int_rem_upd,
          is_ok       TYPE flag.
    ...
    LOOP AT ir_pernr ASSIGNING FIELD-SYMBOL(<pernr>).
      ...
      DATA(molga) = zmhp_cl_forms_utils=>get_molga( CONV #( <pernr>-low ) ).
  ...
  METHOD save_entries.
    ...
    IF rem_upd IS NOT INITIAL AND me->simu NE 'X'.
      MODIFY zmhp_int_rem_upd FROM TABLE rem_upd.
*      INSERT zmhp_int_rem_upd FROM TABLE rem_upd. "DBSQL_DUPLICATE_KEY_ERROR tritt auf -> aendtm zu key machen?
      COMMIT WORK.
      IF sy-subrc NE 0.
        is_ok = abap_false.
