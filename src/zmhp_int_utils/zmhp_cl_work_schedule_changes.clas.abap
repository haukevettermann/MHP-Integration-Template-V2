class ZMHP_CL_WORK_SCHEDULE_CHANGES definition
  public
  final
  create public .
...
public section.
  ...
  class-methods CREATE_SHIFT_EVENT_NODE
    importing
      !WORK_SCHEDULE_TIMES type ZMHP_WORK_SCHEDULES_TIMES
  ...
  methods GET_WORK_SCHEDULES_TIMES
    ...
    returning
      value(WORK_SCHEDULE_TIMES) type ZMHP_TT_WORK_SCHEDULES_TIMES .
...
private section.
  ...
  data INT_WS_MAP_TAB type ZMHP_TT_INT_WS_MAP .
  ...
  data WORK_SCHEDULE_TIMES type ZMHP_TT_WORK_SCHEDULES_TIMES .
  ...
  methods GET_CUSTOMIZING
    importing
      ...
      !MOLGA type MOLGA
    returning
      value(INT_WS_MAP) type ZMHP_INT_WS_MAP .
  methods BUILD_XML
    importing
      !WORK_SCHEDULE_TIMES type ZMHP_TT_WORK_SCHEDULES_TIMES
    returning
...
CLASS ZMHP_CL_WORK_SCHEDULE_CHANGES IMPLEMENTATION.
...
METHOD build_xml.
  ...
  DATA: event_node              TYPE REF TO if_ixml_node,
        ...
        work_schedule_times_tmp TYPE zmhp_tt_work_schedules_times,
        event_number            TYPE string,
        evt_num                 TYPE string,
        request_number          TYPE string,
  ...
  DATA(oraclepernr) = zmhp_cl_forms_utils=>get_oraclepernr( tmp_pernr ).

  event_node = xml_document->create_element( name = 'Event' ).

  request_node  = zmhp_cl_work_schedule_changes=>create_request_node( EXPORTING work_schedule_times = work_schedule_times_tmp[ 1 ]
  ...
  LOOP AT work_schedule_times_tmp ASSIGNING FIELD-SYMBOL(<work_schedule_times_tmp>).
    ...
    IF <work_schedule_times_tmp>-pernr NE tmp_pernr.
      ...
      oraclepernr = zmhp_cl_forms_utils=>get_oraclepernr( tmp_pernr ).
      ...
      request_node = zmhp_cl_work_schedule_changes=>create_request_node( EXPORTING work_schedule_times = <work_schedule_times_tmp>
                                                                                   xml_document        = xml_document
                                                                         IMPORTING event_number        = event_number
                                                                                   request_number      = request_number ).
...
*
      IF begda_tmp IS NOT INITIAL AND
        ...
        DATA(oracle_endda) = zmhp_cl_forms_utils=>convert_date_oracle( endda_tmp ).
...
*
      ...
      DATA(oracle_begda) = zmhp_cl_forms_utils=>convert_date_oracle( begda_tmp ).
...
*JMB20211014 insert end
    ...
    shift_event_node = zmhp_cl_work_schedule_changes=>create_shift_event_node( EXPORTING work_schedule_times = <work_schedule_times_tmp>
                                                                                         xml_document        = xml_document
                                                                                         request_number      = request_number
...
*
    AT LAST.
      ...
      oracle_endda = zmhp_cl_forms_utils=>convert_date_oracle( endda_tmp ).
      end_date_node->set_value( value = CONV #( oracle_endda ) ).
      shift_node->append_child( new_child = end_date_node ).
    ENDAT.
...
METHOD create_and_store_xml_file.
  ...
  xml_string = zmhp_cl_int_xml_up=>create_string_from_xstring( xstring = me->xstring ).

  IF me->simu IS INITIAL.

    CALL FUNCTION 'RH_AUTHORITY_CHECK_OFF'.

    DATA(logical_filename) = CONV filename-fileintern('ZMHP_HCM_ORACLE_EXTRACTS').
...
**      DATA(oracle_begda) = zmhp_cl_forms_utils=>convert_date_oracle( CONV #( <line>-low ) ).
**      DATA(oracle_endda) = zmhp_cl_forms_utils=>convert_date_oracle( CONV #( <line>-high ) ).
...
*    filename = filename && '.xml'.
  ...
  METHOD create_request_node.
    ...
    DATA(timestamp) = zmhp_cl_work_schedule_changes=>create_request_ts( ).
    xml_single_node->set_value( value = timestamp ).
    rval->append_child( new_child = xml_single_node ) .

    xml_single_node = xml_document->create_element( name = 'CloudPernr' ).
    DATA(cloud_pernr) = zmhp_cl_forms_utils=>get_oraclepernr( work_schedule_times-pernr ).
...
*    DATA(oracle_begda) = zmhp_cl_forms_utils=>convert_date_oracle( work_schedule_times-begda ).
*    xml_single_node->set_value( value = CONV #( oracle_begda ) ).
*    rval->append_child( new_child = xml_single_node ).
*
*    xml_single_node = xml_document->create_element( name = 'EndDate' ).
*    DATA(oracle_endda) = zmhp_cl_forms_utils=>convert_date_oracle( work_schedule_times-endda ).
...
*JMB20211015 deletion end
    ...
    xml_single_node->set_value( value = CONV #( zmhp_cl_forms_utils=>convert_date_oracle( work_schedule_times-endda ) ) ).
    rval->append_child( new_child = xml_single_node ).

    xml_single_node = xml_document->create_element( name = 'StartTime' ).
    DATA(shift_start_time) = zmhp_cl_work_schedule_changes=>create_shift_time( date = work_schedule_times-begda
    ...
    DATA(shift_end_time) = zmhp_cl_work_schedule_changes=>create_shift_time( date = work_schedule_times-endda
...
*    2018-07-02T18:00:00
  ...
  METHOD get_customizing.
    ...
    FIELD-SYMBOLS: <int_ws_map_tab> TYPE zmhp_int_ws_map.

    IF int_ws_map_tab IS INITIAL.
      SELECT * FROM zmhp_int_ws_map INTO TABLE int_ws_map_tab.
*    ELSE.
    ENDIF.
...
* read infotype 0000
    CALL METHOD zmhp_cl_infotype_utils=>read_infotype_records
      EXPORTING
        pernr         = pernr
        infty         = '0000'
        begda         = begda
        ...
        IF <work_schedule_times> IS ASSIGNED AND <work_schedule_times> IS NOT INITIAL.
          ...
          IF <dayint>-beguz IS INITIAL AND <dayint>-enduz IS INITIAL.
            ...
            IF dayint_40 IS INITIAL.
              ...
              zmhp_cl_work_schedule_changes=>add_time( EXPORTING
...
ENDMETHOD.
  ...
  METHOD prepare_protocol.
    ...
    IF message_handler IS BOUND.
      ...
      message_handler->if_hrpay00_pal_services~create_fcat( EXPORTING i_structure_name = 'ZMHP_WORK_SCHEDULE_TIMES'
...
*
*      "Define structure of successful and failed IT_AEND entries ALV table
*      message_handler->if_hrpay00_pal_services~create_fcat( EXPORTING i_structure_name = 'ZMHP_WORK_SCHEDULES_TIMES'
...
METHOD submit_ws_rpt.
  ...
  CASE pnnnn_new-infty.
    WHEN '0007'.
      ...
      SUBMIT zmhp_rp_work_schedules_changes WITH pnppernr IN pernr
