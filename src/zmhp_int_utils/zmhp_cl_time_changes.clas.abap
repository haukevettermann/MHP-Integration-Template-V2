class ZMHP_CL_TIME_CHANGES definition
  public
  create public .
...
public section.
  ...
  types:
    BAL_UPD_TT type table of ZMHP_INT_BAL_UPD .
  types:
    RES_MAP_TT type table of ZMHP_INT_RES_MAP .
  types:
    BAL_UPT_TT type table of ZMHP_INT_BAL_UPD .
  types:
    PERIODS_T type table of PERIOD .
  types:
    TIME_PAY_RESULTS_T type table of ZMHP_TIME_PAY_RESULTS .
  types:
  ...
  data INT_BAL_UPD type ZMHP_TT_BAL_UPD .
  ...
  class-methods BUILD_XML
    importing
      !INT_BAL_UPD type ZMHP_TT_BAL_UPD
    exporting
  ...
  class-methods CREATE_CHILD_NODE
    importing
      ...
      !ORACLEPERNR type ZMHP_DD_OBJECTNUMBER
      !MOLGA type MOLGA
      !PERNR type PERNR_D
      !INT_BAL_UPD type ZMHP_INT_BAL_UPD
    returning
  ...
  class-methods DETERMINE_ZTART_NODE_NAME
    importing
      !ZTART type PT_ZTART
    returning
      value(NODE_NAME) type ZMHP_DD_ZTART_NODENAME .
  ...
  methods COMPARE_TABLES
    importing
      !BAL_UPD_OLD type ZMHP_CL_TIME_CHANGES=>BAL_UPD_TT
      !BAL_UPD_NEW type ZMHP_CL_TIME_CHANGES=>BAL_UPD_TT
    exporting
      !BAL_UPD_DEL type ZMHP_CL_TIME_CHANGES=>BAL_UPD_TT
      !BAL_UPD_ADD type ZMHP_CL_TIME_CHANGES=>BAL_UPD_TT
      !BAL_UPD_MOD type ZMHP_CL_TIME_CHANGES=>BAL_UPD_TT
      !BAL_UPD_NO_CHANGES type ZMHP_CL_TIME_CHANGES=>BAL_UPD_TT
  ...
  methods CREATE_ENTRIES
    importing
      !INT_RES_MAP type ZMHP_CL_TIME_CHANGES=>RES_MAP_TT
      !PERNR type PERNR_D
      !CLUSTER_B2 type HRF_TIM_B2
    exporting
      !BAL_UPD type ZMHP_CL_TIME_CHANGES=>BAL_UPD_TT .
  methods DETECT_CHANGES
    importing
      ...
      !BEGDA type BEGDA
    exporting
      !BAL_UPD_CHANGES type ZMHP_CL_TIME_CHANGES=>BAL_UPD_TT .
  methods GET
    importing
      !DATES type RSDSSELOPT_T
    ...
    returning
      value(FO_AEUP) type ZMHP_TT_FO_AEUP .
  methods POST_FO_AEUP
    importing
      !SIMU type BOOLEAN
    exporting
      !FO_AEUP_POST type ZMHP_TT_FO_AEUP
      !FO_AEUP_ERROR type ZMHP_TT_FO_AEUP
      !FO_AEUP_LOCKED type ZMHP_TT_FO_AEUP
    changing
      !FO_AEUP type ZMHP_TT_FO_AEUP
      !MESSAGE_HANDLER type ref to CL_HRPAY00_MESSAGE_HANDLER .
  methods PREPARE_PROTOCOL .
  methods PREPARE_TABLES
    importing
      !STATUS type ZMHP_DD_STATUS
    changing
      !BAL_UPD_OLD type ZMHP_CL_TIME_CHANGES=>BAL_UPD_TT
      !BAL_UPD_NEW type ZMHP_CL_TIME_CHANGES=>BAL_UPD_TT .
...
CLASS ZMHP_CL_TIME_CHANGES IMPLEMENTATION.


  METHOD build_xml.
    DATA: xml_document_node TYPE REF TO if_ixml_node,
          int_bal_upd_tmp   TYPE TABLE OF zmhp_int_bal_upd,
          child_node        TYPE REF TO if_ixml_node.


    DATA(xml) = NEW zmhp_cl_int_xml_up( ).
    ...
    DATA(oraclepernr) = zmhp_cl_forms_utils=>get_oraclepernr( pernr ).
...
*    DATA(parent_node) = xml_document->create_element( name = 'PayrollInterfaceInboundRecord' ).
    DATA(parent_node) = zmhp_cl_time_changes=>create_parent_node( name         = 'PayrollInterfaceInboundRecord'
    ...
    LOOP AT int_bal_upd_tmp ASSIGNING FIELD-SYMBOL(<int_bal_upd_tmp>).
      ...
      IF <int_bal_upd_tmp>-pernr NE tmp_pernr. " pernr changes
        ...
        oraclepernr = zmhp_cl_forms_utils=>get_oraclepernr( <int_bal_upd_tmp>-pernr ).
        ...
        parent_node    = zmhp_cl_time_changes=>create_parent_node( name         = 'PayrollInterfaceInboundRecord'
                                                                   xml_document = xml_document
                                                                   oraclepernr  = oraclepernr
                                                                   molga        = molga
                                                                   pernr        = tmp_pernr
      ...
      child_node = zmhp_cl_time_changes=>create_child_node( name         = 'PayrollInterfaceInboundRecordInfo'
                                                            xml_document = xml_document
                                                            oraclepernr  = oraclepernr
                                                            molga        = molga
                                                            pernr        = tmp_pernr
  ...
  METHOD COMPARE_TABLES.
    ...
    DATA: bal_upd_line TYPE zmhp_int_bal_upd.
...
*           <bal_upd_new>-aedtm  NE bal_upd_old_line-aedtm  OR
  ...
  METHOD create_child_node.
    ...
    DATA(country_iso) = zmhp_cl_forms_utils=>get_country_by_molga( molga ).
    ...
    DATA(oracle_inbd) = zmhp_cl_time_changes=>get_oracle_inbound_record( ztart   = int_bal_upd-ztart
    ...
    DATA(source_system_id) = zmhp_cl_time_changes=>generate_source_system_id( pernr   = pernr
                                                                              begda   = begda
                                                                              endda   = endda
    ...
    DATA(oracle_endda) = zmhp_cl_forms_utils=>convert_date_oracle( date = int_bal_upd-endda ).
    ...
    DATA(oracle_begda) = zmhp_cl_forms_utils=>convert_date_oracle( date = int_bal_upd-begda ).
    xml_single_node->set_value( value = CONV #( oracle_begda ) ).
    rval->append_child( new_child = xml_single_node ).

    DATA(node_name) = zmhp_cl_time_changes=>determine_ztart_node_name( int_bal_upd-ztart ).
  ...
  METHOD create_entries.
    DATA: bal_upd_t TYPE TABLE OF zmhp_int_bal_upd,
          bal_upd_l TYPE zmhp_int_bal_upd.
    ...
    LOOP AT int_res_map ASSIGNING FIELD-SYMBOL(<line_map>).
      ...
      LOOP AT <res_tab> ASSIGNING FIELD-SYMBOL(<tab_line>).
        ...
        IF <ztart> IS ASSIGNED.
          ...
          bal_upd_l-status    = '01'.
          bal_upd_l-ztext     = zmhp_cl_forms_utils=>get_ztart_text( ztart = <ztart> ).
  ...
  METHOD create_parent_node.
    ...
    DATA(oracle_begda) = zmhp_cl_forms_utils=>convert_date_oracle( begda ).
...
*    xml_single_node->set_value( value = 'ORA_HRY_INBD_PAYROLL').
    ...
    xml_single_node = xml_document->create_element( name = 'SourceSystemId' ).
    DATA(ssid_part) = zmhp_cl_time_changes=>get_ssid_component( ztart   = int_bal_upd-ztart
                                                                tabname = int_bal_upd-tabname ).
  ...
  METHOD DETECT_CHANGES.
    ...
    DATA: change_date        TYPE dats,
          bal_upd_t          TYPE STANDARD TABLE OF zmhp_int_bal_upd,
          bal_upd_l          TYPE zmhp_int_bal_upd,
          res_map_ztart      TYPE TABLE OF pt_ztart,
          tabname            TYPE char40,
          bal_upd_del        TYPE STANDARD TABLE OF zmhp_int_bal_upd,
          bal_upd_mod        TYPE STANDARD TABLE OF zmhp_int_bal_upd,
          bal_upd_add        TYPE STANDARD TABLE OF zmhp_int_bal_upd,
          no_changes         TYPE flag,
          bal_upd_tmp        TYPE STANDARD TABLE OF zmhp_int_bal_upd,
          bal_upd_no_changes TYPE STANDARD TABLE OF zmhp_int_bal_upd.


*    DATA(oracle_id) = zmhp_cl_int_employee_xml_up=>read_oracle_id( pernr = pernr ).

    " read customizing
    SELECT * FROM zmhp_int_res_map WHERE begda <= @sy-datum
                                   AND   endda >= @sy-datum
    ...
    SELECT * FROM zmhp_int_bal_upd WHERE molga     = @cluster_b2-molga
                                   AND   pernr     = @pernr
                                   AND   status    = '02'
...
*****      ENDLOOP.
  ...
  METHOD determine_ztart_node_name.
    ...
    SELECT SINGLE node_name FROM zmhp_int_res_map
  ...
  METHOD generate_source_system_id.
    ...
    DATA(oracle_begda) = zmhp_cl_forms_utils=>convert_date_oracle( begda ).
    DATA(oracle_endda) = zmhp_cl_forms_utils=>convert_date_oracle( endda ).
    DATA(ssid_comp)    = zmhp_cl_time_changes=>get_ssid_component( ztart   = ztart
                                                                   tabname = tabname ).
    CONCATENATE 'PER'
                 pernr
                'PAY'
...
*    PER_23860_PAY_12_2020_TS_2021/06/01_2021/06/30
  ...
  SELECT * FROM zmhp_int_fo_aeup INTO TABLE fo_aeup WHERE pernr     IN pernr  AND
  ...
  METHOD get_oracle_inbound_record.
    ...
    SELECT SINGLE oracle_ztart FROM  zmhp_int_res_map
                               WHERE ztart   = @ztart
                               AND   tabname = @tabname
                               INTO  @oracle_ztart.
  ...
  METHOD get_ssid_component.
    ...
    SELECT SINGLE ssid_comp FROM zmhp_int_res_map
                            WHERE ztart   EQ @ztart
...
*      me->status_handler->molga = <fo_aeup>-molga.  " molga nicht in zmhp_int_fo_aeup vorhanden
...
*      DATA(method) = SWITCH char9( <fo_aeup>-infty " zmhp_int_fo_aeup hat kein feld infty
...
*        WHEN '2006' THEN 'POST_OPER' ).
  ...
  METHOD prepare_protocol.
    ...
    IF message_handler IS BOUND.
      ...
      message_handler->if_hrpay00_pal_services~create_fcat( EXPORTING i_structure_name = 'ZMHP_INT_BAL_UPD'
...
***
***      "Define structure of successful and failed IT_AEND entries ALV table
***      message_handler->if_hrpay00_pal_services~create_fcat( EXPORTING i_structure_name = 'ZMHP_INT_FO_AEUP'
...
***    ENDIF.
  ...
  METHOD read_cluster_b2.
    ...
    DATA: pernr           TYPE pernr_d,
          cluster_b2      TYPE hrf_tim_b2,
          periods         TYPE zmhp_cl_forms_utils=>periods_t,
          bal_upd         TYPE TABLE OF zmhp_int_bal_upd,
          bal_upd_changes TYPE TABLE OF zmhp_int_bal_upd.


    CALL METHOD zmhp_cl_forms_utils=>get_periods(
      EXPORTING
        begda   = begda
        endda   = endda
      IMPORTING
...
*            get_zes   = ft_zes.
  ...
  METHOD read_cluster_b2_force.
    ...
    DATA: pernr       TYPE pernr_d,
          periods     TYPE zmhp_cl_forms_utils=>periods_t,
          cluster_b2  TYPE hrf_tim_b2,
          bal_upd_tmp TYPE TABLE OF zmhp_int_bal_upd.
  ...
  METHOD SAVE_ENTRIES.
    ...
    IF bal_upd IS NOT INITIAL AND me->simu NE 'X'.
      MODIFY zmhp_int_bal_upd FROM TABLE bal_upd.
*      INSERT zmhp_int_bal_upd FROM TABLE bal_upd. "DBSQL_DUPLICATE_KEY_ERROR tritt auf -> aendtm zu key machen?
      COMMIT WORK.
      IF sy-subrc NE 0.
        is_ok = abap_false.
  ...
  METHOD test_method.
    ...
    SELECT * FROM zmhp_int_bal_upd WHERE status = '02'
    ...
    CALL METHOD zmhp_cl_time_changes=>build_xml
      EXPORTING
...
*        MODIFY zmhp_int_bal_upd FROM TABLE int_bal_upd.
*        COMMIT WORK.
*        status = '02'.
    ELSE.
*      status = '03'.
...
*          periods   TYPE zmhp_cl_time_changes=>periods_t,
*          begda     TYPE dats,
*          endda     TYPE dats,
