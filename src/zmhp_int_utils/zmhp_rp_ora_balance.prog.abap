...
DATA: awr,
      ...
      o_ora_bal    TYPE REF TO zmhp_cl_rp_ora_bal,
...
START-OF-SELECTION.
  ...
  IF pnppernr[] IS INITIAL.
    pnppernr[] = NEW zmhp_cl_int_ora_bal( )->get_peras( begda = pn-begda endda = pn-endda ).
  ENDIF.
...
END-OF-SELECTION.
  ...
  DATA(ora_bal) = NEW zmhp_cl_int_ora_bal( ).
  ...
  o_ora_bal = NEW zmhp_cl_rp_ora_bal( pernr = r_pernr ).
  ...
  SET HANDLER zmhp_events=>on_adt_clk FOR salv_table->get_event( ).
