class ZMHP_CL_FORMS_CHANGES definition
  public
  create public .
...
public section.
  ...
  data INT_FO_AEUP type ZMHP_TT_FO_AEUP .
  ...
  methods CREATE_FO_AEUP_ENTRIES
    importing
      !RESULT type ZMHP_TIME_PAY_RESULTS
      !PERNR type PERNR_D
      !ORACLEPERNR type ZMHP_DD_OBJECTNUMBER
      !AEDTM type AEDTM
      !OTYPE type CHAR1
    exporting
      !FO_AEUP type ZMHP_INT_FO_AEUP .
  methods CONSTRUCTOR
    importing
      !ALV type BOOLEAN
      !BATCH type BOOLEAN
...
CLASS ZMHP_CL_FORMS_CHANGES IMPLEMENTATION.
  ...
  METHOD create_fo_aeup_entries.
    DATA: fo_aeup_line TYPE zmhp_int_fo_aeup.
...
*    LOOP AT results ASSIGNING FIELD-SYMBOL(result).
    ...
    fo_aeup_line-endda        = zmhp_cl_forms_utils=>convert_date_oracle_to_sap( result-date_to   ).
    fo_aeup_line-begda        = zmhp_cl_forms_utils=>convert_date_oracle_to_sap( result-date_from ).
  ...
  ENDMETHOD.
    ...
    METHOD detect_changes.
      ...
      DATA: pernr        TYPE pernr_d,
            ...
            aedtm        TYPE aedtm,
            period       TYPE char6,
            tmp_results  TYPE zmhp_cl_forms_utils=>time_pay_results_t,
            results      TYPE zmhp_cl_forms_utils=>time_pay_results_t,
            oracle_id    TYPE zmhp_dd_element,
            fo_aeup_line TYPE zmhp_int_fo_aeup,
            fo_aeup_mod  TYPE TABLE OF zmhp_int_fo_aeup,
            fo_aeup_tmp  TYPE TABLE OF zmhp_int_fo_aeup,
            length       TYPE i,
            status       TYPE zmhp_dd_status,
            l_endda      TYPE endda,
            l_begda      TYPE begda.
...
*          oracle_id    TYPE zmhp_dd_element,
          ...
          period       TYPE char6,
          aedtm        TYPE aedtm,
          tmp_result   TYPE zmhp_time_pay_results,
          fo_aeup      TYPE TABLE OF zmhp_int_fo_aeup,
          fo_aeup_line TYPE zmhp_int_fo_aeup,
          is_ok        TYPE flag.
...
*      oracle_id = zmhp_cl_int_employee_xml_up=>read_oracle_id( pernr = pernr ).
      oraclepernr = zmhp_cl_forms_utils=>get_oraclepernr( pernr ).
      ...
      LOOP AT periods ASSIGNING FIELD-SYMBOL(<period>).
        ...
        SELECT * FROM zmhp_int_fo_aeup WHERE pernr =  @pernr
                                       AND   begda IN @period_range
                                       AND   endda IN @period_range
                                       AND   otype =  @zmhp_cl_forms_utils=>rem_statement
                                       INTO  TABLE    @DATA(int_fo_aeup).

        period = CONV #( <period>-datuv+0(6) ).
        aedtm  = zmhp_cl_rem_changes=>get_change_date(  pernr  = pernr
                                                        begda  = <period>-datuv
                                                        endda  = <period>-datub ).
...
*             worker_id   = oracle_id
              oraclepernr = oraclepernr
              aedtm       = aedtm
              otype       = zmhp_cl_forms_utils=>rem_statement
            IMPORTING
...
*      oracle_id = zmhp_cl_int_employee_xml_up=>read_oracle_id( pernr = pernr ).
      ...
      LOOP AT periods ASSIGNING FIELD-SYMBOL(<period>).
        ...
        SELECT * FROM zmhp_int_fo_aeup WHERE pernr =  @pernr
                                       AND   begda IN @period_range
                                       AND   endda IN @period_range
                                       AND   otype =  @zmhp_cl_forms_utils=>time_statement
                                       INTO  TABLE    @DATA(int_fo_aeup).

        period = CONV #( <period>-datuv+0(6) ).
        aedtm  = zmhp_cl_time_changes=>get_change_date( pernr  = pernr
                                                        period = period ).
...
*             worker_id   = oracle_pernr
              oraclepernr = oraclepernr
              aedtm       = aedtm
              otype       = zmhp_cl_forms_utils=>time_statement
            IMPORTING
              fo_aeup     = fo_aeup_line. " create initial records
...
*          ENDLOOP.
  ...
  METHOD get_rem_statements.
    ...
    DATA: tmp_result   TYPE zmhp_time_pay_results,
          fo_aeup      TYPE TABLE OF zmhp_int_fo_aeup,
          fo_aeup_line TYPE zmhp_int_fo_aeup,
          is_ok        TYPE flag.
    ...
    LOOP AT ir_pernr ASSIGNING FIELD-SYMBOL(<pernr>).
      ...
      LOOP AT periods ASSIGNING FIELD-SYMBOL(<period>).
        ...
        DATA(aedtm)  = zmhp_cl_rem_changes=>get_change_date( pernr = pernr
                                                             begda = <period>-datuv
                                                             endda = <period>-datub ).
        CALL METHOD zmhp_cl_forms_utils=>get_rem_pdf_data
          EXPORTING
  ...
  METHOD get_time_statements.
    ...
    LOOP AT ir_pernr ASSIGNING FIELD-SYMBOL(<pernr>).
      ...
      LOOP AT periods ASSIGNING FIELD-SYMBOL(<period>).
        ...
        DATA(aedtm) = zmhp_cl_time_changes=>get_change_date( pernr  = pernr
                                                             period = period ).
        CALL METHOD zmhp_cl_forms_utils=>get_time_pdf_data
          EXPORTING
            begda       = <period>-datuv
...
*    ENDIF.
  ...
  METHOD save_entries.
    ...
    IF fo_aeup IS NOT INITIAL AND me->simu NE 'X'.
      ...
      MODIFY zmhp_int_fo_aeup FROM TABLE fo_aeup.
      COMMIT WORK.
