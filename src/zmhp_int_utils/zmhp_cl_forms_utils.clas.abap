class ZMHP_CL_FORMS_UTILS definition
  public
  create public .
...
public section.
  ...
  class-methods BUILD_XML
    importing
      !RESULTS type ZMHP_CL_FORMS_UTILS=>TIME_PAY_RESULTS_T
    returning
  ...
  class-methods GENERATE_DOCUMENT_NAME
    importing
      !PERNR type ZMHP_INT_FO_AEUP-PERNR
      !BEGDA type ZMHP_INT_FO_AEUP-BEGDA
      !OTYPE type CHAR1
    returning
  ...
  class-methods GET_ORACLEPERNR
    importing
      !PERNR type PERNR_D
    returning
      value(ORACLEPERNR) type ZMHP_DD_OBJECTNUMBER .
  ...
  class-methods GET_REM_DATA
    importing
      !INT_FO_AEUP type ZMHP_INT_FO_AEUP
    returning
  ...
  class-methods GET_REM_PDF_DATA
    ...
    exporting
      !STATUS type SUBRC
      !PAYRESULT type ZMHP_TIME_PAY_RESULTS .
  class-methods GET_SEQNR_BY_PERNR
    importing
      ...
      !ENDDA type DATS
    exporting
      !SEQNRS type ZMHP_CL_FORMS_UTILS=>SEQNR_T .
  ...
  class-methods GET_TIME_PDF_DATA
    importing
      ...
      !PERNR type PERNR_D
    exporting
      !TIMERESULT type ZMHP_TIME_PAY_RESULTS
      !STATUS type SUBRC .
...
CLASS ZMHP_CL_FORMS_UTILS IMPLEMENTATION.


  METHOD build_xml.
    DATA: l_results         TYPE zmhp_cl_forms_utils=>time_pay_results_t,
          xml_document_node TYPE REF TO if_ixml_node,
          single_result     TYPE zmhp_time_pay_results.
...
* read infotype 0001 entry which is valid at iv_endda
  CALL METHOD zmhp_cl_forms_utils=>read_infotype_record
    EXPORTING
      pernr         = pernr
      infty         = '0001'
      begda         = begda
  ...
  ls_pmehf-molga = zmhp_cl_forms_utils=>get_molga( pernr ).
...
*
***      """" begin of test data
***      DATA: change_data_tim  TYPE STANDARD TABLE OF zmhp_int_fo_aeup,
***            change_data_line TYPE zmhp_int_fo_aeup,
***            change_data_rem  TYPE STANDARD TABLE OF zmhp_int_fo_aeup.
...
***      change_data_line-cloud_id = zmhp_cl_int_employee_xml_up=>read_oracle_id( pernr = change_data_line-pernr ).
***      APPEND change_data_line TO change_data_tim.
***      APPEND change_data_line TO change_data_rem.
***      """" end of test data
*
*      DATA: temp_results      TYPE zmhp_cl_forms_utils=>time_pay_results_t,
*            results           TYPE zmhp_cl_forms_utils=>time_pay_results_t,
*            records_to_delete TYPE TABLE OF zmhp_int_fo_aeup,
*            content           TYPE stringtab.
*
*      IF object_type = 'T'.
*        zip_file = 'Time_Statements.zip'.
*
*        SELECT * FROM zmhp_int_fo_aeup INTO TABLE @DATA(change_data_tim) WHERE otype = 'T'.
*
*        IF sy-subrc NE 0.
*          RETURN.
*        ENDIF.
...
*          CALL METHOD zmhp_cl_forms_utils=>get_time_pdf_data
*            EXPORTING
...
*
*        SELECT * FROM zmhp_int_fo_aeup INTO TABLE @DATA(change_data_rem) WHERE otype = 'R'.
*
*        IF sy-subrc NE 0.
*          RETURN.
*        ENDIF.
...
*
*          CALL METHOD zmhp_cl_forms_utils=>get_rem_pdf_data
*            EXPORTING
...
*          mv_pernr = iv_pernr.
  ...
  METHOD get_periods.
    ...
    DATA: nperiods   TYPE i,
          ...
          l_begda    TYPE dats,
          l_endda    TYPE dats,
          lt_periods TYPE zmhp_cl_forms_utils=>periods_t,
  ...
  METHOD get_rem_data.
    ...
    DATA(oraclepernr) = zmhp_cl_forms_utils=>get_oraclepernr( int_fo_aeup-pernr ).
    DATA(molga)       = zmhp_cl_forms_utils=>get_molga( int_fo_aeup-pernr ).
    DATA(date_from)   = zmhp_cl_forms_utils=>convert_date_oracle( int_fo_aeup-begda ).
    DATA(date_to)     = zmhp_cl_forms_utils=>convert_date_oracle( int_fo_aeup-endda ).

    document_code = int_fo_aeup-begda+0(6).
    document_name = zmhp_cl_forms_utils=>generate_document_name( pernr = int_fo_aeup-pernr
                                                                 begda = int_fo_aeup-begda
                                                                 otype = int_fo_aeup-otype ).

    file_name     = zmhp_cl_forms_utils=>generate_file_name( pernr = int_fo_aeup-pernr
                                                             begda = int_fo_aeup-begda
                                                             otype  = int_fo_aeup-otype ).
    ...
    CONCATENATE zmhp_cl_forms_utils=>merge
                'DocumentsOfRecord'
                oraclepernr
                documenttype
                ''
...
METHOD get_rem_pdf_data.
  ...
  DATA: molga          TYPE molga,
        ...
        l_payresult    TYPE zmhp_time_pay_results,
        xstring        TYPE xstring,
        lt_rgdir       TYPE STANDARD TABLE OF pc261,
        lt_seqnr       TYPE zmhp_cl_forms_utils=>seqnr_t,
  ...
  TRY.
      molga = zmhp_cl_forms_utils=>get_molga( pernr ).
      country_iso = zmhp_cl_forms_utils=>get_country_by_molga( molga ).

      IF lt_rgdir IS INITIAL.
        lt_rgdir  = zmhp_cl_forms_utils=>get_raw_payrollresults_list( pernr ).
      ENDIF.
      ...
      IF lt_rgdir IS NOT INITIAL.
        ...
        IF lt_seqnr IS NOT INITIAL.
          LOOP AT lt_seqnr ASSIGNING FIELD-SYMBOL(<single_seqnr>).
            ...
            l_payresult-action        = zmhp_cl_forms_utils=>delete.
            l_payresult-type          = zmhp_cl_forms_utils=>glb_payslip.
            ...
            l_payresult-otype         = zmhp_cl_forms_utils=>rem_statement.
...
*
  DATA(operation) = COND string( WHEN del EQ abap_true
                                 THEN zmhp_cl_forms_utils=>delete
                                 ELSE zmhp_cl_forms_utils=>merge ).
...
ENDMETHOD.
  ...
  METHOD get_time_pdf_data.
    DATA: form_name    TYPE hrf_infonet,
          ...
          l_endda      TYPE dats,
          periods      TYPE zmhp_cl_forms_utils=>periods_t,
          l_timeresult TYPE zmhp_time_pay_results,
          l_period     TYPE char6,
          country_iso  TYPE char2,
...
*      EXIT.
    ...
    l_timeresult-action        = zmhp_cl_forms_utils=>delete.
    l_timeresult-type          = zmhp_cl_forms_utils=>glb_timestatement.
    ...
    l_timeresult-document_code = generate_time_document_code( pernr  = pernr
                                                              period = begda ).
    l_timeresult-otype         = zmhp_cl_forms_utils=>time_statement.
...
*    DATA: is_aendup    TYPE zmhp_int_it_aeup,
*          s0000        TYPE p0000,
*          s0002        TYPE p0002,
*          s0001        TYPE p0001,
*          filename     TYPE string,
*          message      TYPE bapiret1,
*          changed_data TYPE TABLE OF zmhp_int_it_aeup,
*          xml_string   TYPE string.
*
*    SELECT SINGLE * FROM zmhp_int_objects INTO @DATA(ls_object) WHERE object = 'P'.
*    SELECT * FROM zmhp_int_it_aeup INTO TABLE @DATA(changed_data) WHERE status = '01'
*                                                                  AND   infty = '2001'.
...
*    ENDCASE.
    ...
    DATA: is_aendup  TYPE zmhp_int_it_aeup,
          up_success TYPE TABLE OF zmhp_int_it_aeup,
          s0000      TYPE p0000,
          s0002      TYPE p0002,
          s0001      TYPE p0001,
          filename   TYPE string,
    ...
    SELECT SINGLE * FROM zmhp_int_objects INTO @DATA(ls_object) WHERE object = 'P'.
    SELECT * FROM zmhp_int_it_aeup INTO TABLE @DATA(changed_data) UP TO 5 ROWS
                                                                  WHERE status = '02'
                                                                  AND infty NE '2001'
    ...
    LOOP AT changed_data_tmp ASSIGNING FIELD-SYMBOL(<change>).
      ...
      DATA(changed_data_pernr) = VALUE zmhp_cl_int_employee_xml_up=>t_itaendup( FOR     change IN changed_data
                                                                                WHERE ( pernr = <change>-pernr ) ( change ) ).

      xml->add_node_object_employee( EXPORTING object_id_cloud = CONV #( zmhp_cl_int_employee_xml_up=>read_oracle_id( EXPORTING pernr   = <change>-pernr
                                                                                                                                date    = <change>-begda
      ...
      LOOP AT changed_data_pernr ASSIGNING FIELD-SYMBOL(<data_pernr>).
        ...
        DATA(data_pernr) = VALUE zmhp_cl_int_employee_xml_up=>t_itaendup( ( <data_pernr> ) ).
...
*JMB20210907 end insert
        ...
        DATA(employee) = NEW zmhp_cl_int_employee_xml_up( changed_data = data_pernr
...
*      MODIFY zmhp_int_it_aeup FROM TABLE changed_data_tmp. "JMB20211005 D
*      MODIFY zmhp_int_it_aeup FROM TABLE up_success. "JMB20211005 I
*      COMMIT WORK.
    ENDIF.
