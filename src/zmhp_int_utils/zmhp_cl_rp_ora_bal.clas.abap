class ZMHP_CL_RP_ORA_BAL definition
  public
  create public .
...
public section.
  ...
  data BAL_COMP type ZMHP_CL_RP_ORA_BAL=>T_ORA_BAL_COMP .

  methods COMBINE_ENTRIES
    importing
      !P2006 type PTT_P2006
      !ORA_BAL type ZMHP_TT_ORA_BAL
    exporting
      !ORA_BAL_COMP type ZMHP_CL_RP_ORA_BAL=>T_ORA_BAL_COMP .
  methods CREATE_CORRV_ENTRIES .
  methods GET_CORRV_ENTRIES
    importing
      !SEL_ROWS type SALV_T_ROW
      !BAL_COMP type ZMHP_CL_RP_ORA_BAL=>T_ORA_BAL_COMP .
...
CLASS ZMHP_CL_RP_ORA_BAL IMPLEMENTATION.
...
METHOD combine_entries.
  DATA: ora_bal_c TYPE zmhp_cl_rp_ora_bal=>ora_bal_comp.
...
METHOD create_corrv_entries.
  DATA: p2006   TYPE p2006,
        it_aeup TYPE TABLE OF zmhp_int_it_aeup.
  ...
  MODIFY zmhp_int_it_aeup FROM TABLE it_aeup.
...
METHOD get_open_bookings.
  ...
  SELECT pernr FROM zmhp_int_it_aend INTO TABLE @DATA(pernr_aend) WHERE pernr  IN @pernr AND
                                                                        infty  IN @infty AND
                                                                        status IN @status.

  SELECT pernr FROM zmhp_int_it_aeup INTO TABLE @DATA(pernr_aeup) WHERE pernr  IN @pernr AND
                                                                        infty  IN @infty AND
                                                                        status IN @status.
...
METHOD prepare_alv.
  ...
  TRY.
    ...
    CATCH cx_salv_data_error.
      MESSAGE e041(zmhp_hcm_integration).
