...
public section.
  ...
  constants IT0033 type INFTY value '0033' ##NO_TEXT.
  ...
  constants HFMCODE type ZMHP_DD_FIELD value 'HFMCODE' ##NO_TEXT.
  ...
  constants DEFAULT_BUSINESS_UNIT type STRING value 'WMS' ##NO_TEXT.
  ...
  constants IT0016 type INFTY value '0016' ##NO_TEXT.
...
*JMB20210928 insert end
  ...
  METHOD update_begin_date.
    DATA: pernr_old TYPE rsdsselopt_t,
          ...
          p0006_tmp TYPE p0006.

    "delete all assignments older than first action
    CHECK p0000 IS NOT INITIAL.
    SORT p0001 BY pernr begda ASCENDING.
    SORT p0002 BY pernr begda ASCENDING.
    SORT p0006 BY pernr begda ASCENDING.

    LOOP AT p0000 ASSIGNING FIELD-SYMBOL(<p0000>). "WHERE endda GE sy-datum.
      CHECK <p0000>-pernr NOT IN pernr_old OR
            pernr_old     IS INITIAL.

      "delete all records that are older than earliest hire entry
      IF <p0000>-endda = zmhp_cl_int_constants=>highdate.
        DELETE p0001 WHERE pernr EQ <p0000>-pernr AND
                           endda LT <p0000>-endda.
      ELSE.
        DELETE p0001 WHERE pernr EQ <p0000>-pernr AND
                           endda LE <p0000>-endda.
      ENDIF.

      DELETE p0002 WHERE pernr EQ <p0000>-pernr AND
                         endda LE sy-datum.
*                         begda GE <p0000>-endda.

      DELETE p0006 WHERE pernr EQ <p0000>-pernr AND
                   endda LT <p0000>-endda.
...
*JMB20211030 deletion end
      ...
      LOOP AT p0006 ASSIGNING FIELD-SYMBOL(<p0006>) WHERE pernr EQ <p0000>-pernr.
        <p0002>-begda = <p0000>-begda.
        EXIT.
      ENDLOOP.
