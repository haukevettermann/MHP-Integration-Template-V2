class ZMHP_CL_MIG_OM_DATA definition
  public
  create public .
...
private section.
  ...
  data MOLGA_COUNTRY_CODES type TABLE OF t500l .
  ...
  data ORGID_COUNTRY_CODE type ZMHP_TT_MIG_ORGID_CC .
  ...
  data PA9400_PERNR type ZMHP_P9400_TAB .
  ...
  methods MAP_HR_PERIODS_CA
    ...
    changing
      !OM_DATA_ENTRY type ZMHP_S_MIG_OM_DATA
    returning
      value(OM_DATA) type ZMHP_TT_MIG_OM_DATA .
  methods MAP_HR_PERIODS
    importing
      value(HR_PERIODS) type HRPERIODS_TAB
    exporting
      !OM_DATA_FUT type ZMHP_TT_MIG_OM_DATA
    changing
      !OM_DATA_ENTRY type ZMHP_S_MIG_OM_DATA
    returning
      value(OM_DATA) type ZMHP_TT_MIG_OM_DATA .
...
CLASS ZMHP_CL_MIG_OM_DATA IMPLEMENTATION.
...
METHOD create_data.
  ...
  DATA lv_plans_realo         TYPE realo.
    DATA lv_leader_type         TYPE otype.
  DATA lv_leader_id_realo     TYPE realo.
  DATA lv_pernr_vg_numc       TYPE pernr_d.
  DATA lt_p0001_vg            TYPE TABLE OF p0001.
  DATA ls_p0001_vg            TYPE p0001.
...
*DELETE ADJACENT DUPLICATES FROM data_tmp COMPARING department_code.
...
METHOD download_files.
  ...
  CASE al11.
    WHEN abap_true.
      ...
      CASE sy-mandt.
        WHEN zmhp_cl_int_constants=>cofu_mandant-italy.
...
*      APPEND LINES OF VALUE p1008_tab( FOR p1008 IN p1008_tab WHERE ( persa IS NOT INITIAL AND btrtl IS NOT INITIAL ) ( p1008 ) ) TO hrp1008_br.
*      APPEND LINES OF VALUE p1008_tab( FOR p1008 IN p1008_tab WHERE ( bukrs IS NOT INITIAL ) ( p1008 ) ) TO hrp1008_com.
*    ELSE.
    ...
    DATA(struc_i1) = VALUE struc_t( FOR <struc_i1> IN struc WHERE ( otype EQ 'I1'  ) ( <struc_i1> ) ).
    SORT struc_i1 BY pup ASCENDING.
    DATA(struc_i1s) = VALUE #( struc_i1[ 1 ] OPTIONAL ).
    IF struc_i1s IS NOT INITIAL.
      APPEND INITIAL LINE TO hrp1008_br ASSIGNING FIELD-SYMBOL(<hrp1008_br>).
      <hrp1008_br>-objid = <objec>-objid.
      <hrp1008_br>-begda = struc_i1s-vbegda.
      <hrp1008_br>-endda = struc_i1s-vendda.
      <hrp1008_br>-btrtl = struc_i1s-objid+4(4).
      <hrp1008_br>-persa = struc_i1s-objid+0(4).
    ENDIF.
*    APPEND LINES OF VALUE p1008_tab( FOR <struc_br> IN struc_i1 ( objid = <objec>-objid
*                                                                  begda = <struc_br>-vbegda
*                                                                  endda = <struc_br>-vendda
*                                                                  btrtl = <struc_br>-objid+4(4)
*                                                                  persa = <struc_br>-objid+0(4) ) ) TO hrp1008_br.

    DATA(struc_ic) = VALUE struc_t( FOR <struc_ic> IN struc WHERE ( otype EQ 'IC'  ) ( <struc_ic> ) ).
    SORT struc_ic BY pup ASCENDING.
    DATA(struc_ics) = VALUE #( struc_ic[ 1 ] OPTIONAL ).
    IF struc_ics IS NOT INITIAL.
      APPEND INITIAL LINE TO hrp1008_com ASSIGNING FIELD-SYMBOL(<hrp1008_com>).
      <hrp1008_com>-objid = <objec>-objid.
      <hrp1008_com>-begda = struc_ics-vbegda.
      <hrp1008_com>-endda = struc_ics-vendda.
      <hrp1008_com>-bukrs = struc_ics-objid.
    ENDIF.
*      "collect all personnel areas
...
*                                                                                         btrtl = <struc_br>-objid+4(4)
*                                                                                         persa = <struc_br>-objid+0(4) ) ) TO hrp1008_br.
*      IF lines( hrp1008_br ) > 2.
*        SORT hrp1008_br BY begda ASCENDING.
*      ENDIF.
*
...
*      IF lines( hrp1008_com ) > 2.
*        SORT hrp1008_com BY begda ASCENDING.
*      ENDIF.
*    ENDIF.

    APPEND LINES OF VALUE t_t001( FOR bukrs IN hrp1008_com ( bukrs = bukrs-bukrs ) ) TO me->bukrs_txt.
    SORT bukrs_txt.
    DELETE ADJACENT DUPLICATES FROM bukrs_txt.
    CLEAR: struc.
  ENDLOOP.
...
METHOD get_bukrs_txt.
  ...
  SORT bukrs.
  DELETE ADJACENT DUPLICATES FROM bukrs.
  CLEAR bukrs_txt.
...
METHOD get_country_to_org_unit.
  ...
  DATA: org_unit_id           TYPE pd_objid_r,
        ...
        main_org_country_code TYPE zmhp_tt_mig_orgid_cc,
        country_code          TYPE intca,
        country_codes         TYPE rsdsselopt_t.

  LOOP AT objec ASSIGNING FIELD-SYMBOL(<objec>).
    DATA(objid) = <objec>-objid.
    CALL FUNCTION 'ZMHP_HROMFB_GET_ABTEILUNG_NEW'
      EXPORTING
        plvar          = <objec>-plvar
        otype          = <objec>-otype
        objid          = <objec>-objid
...
METHOD get_mapping_fields.
  ...
  SELECT * FROM zmhp_int_mapp_fi INTO TABLE @mapping_fields_bukrs WHERE molga        IN @molga   AND
                                                                        infty        = '1008'    AND
                                                                        field_sap    = 'BUKRS'   AND
...
METHOD get_mapping_values.
  ...
  SELECT * FROM zmhp_int_mapping INTO TABLE @mapping_values_bukrs WHERE molga        IN @molga   AND
                                                                        infty        = '1008'    AND
                                                                        field_sap    = 'BUKRS'   AND
                                                                        field_oracle = 'COMPANY' AND
                                                                        export       = @abap_true.
...
METHOD get_om_data.
  ...
  SELECT objid, otype, rsign, relat, begda, endda, sobid INTO CORRESPONDING FIELDS OF TABLE @hrp1001_up FROM hrp1001 WHERE objid IN @objid AND
                                                                                                      otype EQ 'O'    AND
                                                                                                      rsign EQ 'A'    AND
                                                                                                      relat EQ '002'  AND
                                                                                                      sclas EQ 'O'    AND
...
*JMB20210819
  ...
  zmhp_cl_mig_utils=>check_assign_supervisor( IMPORTING manager_pernr = DATA(manager_pernr)
...
METHOD map_hr_periods.
  DATA: value      TYPE zmhp_dd_value,
        plvar      TYPE plvar,
        manager_id TYPE realo,
        team_id    TYPE pa0002-pernr,
        tlpernr    TYPE pa0002-pernr,
        alpernr    TYPE pa0002-pernr,
        bzlb(04)   TYPE c,
        zv_sobid   TYPE hrp1001-sobid,
        zs_sobid   TYPE hrp1001-sobid,
        ls_p0001   TYPE pa0001,
        ls_csks TYPE csks.

  "get mapping fields for locationcode
  zmhp_cl_mig_utils=>get_mapping_fields( EXPORTING molga        = CONV #( VALUE #( ( sign = 'I' option = 'EQ' low = '*' ) ) )
                                                   infty        = zmhp_cl_mig_utils=>it0001
...
*JMB20210706 end insert
  ...
  LOOP AT hr_periods ASSIGNING FIELD-SYMBOL(<hr_periods>).
    om_data_entry-effective_start_date = zmhp_cl_mig_utils=>convert_date( <hr_periods>-begda ).
    om_data_entry-effective_end_date   = zmhp_cl_mig_utils=>convert_date( <hr_periods>-endda ).
      ...
      CASE sy-mandt.
        WHEN '901'.
          ...
          IF manager_id IS NOT INITIAL.
            ...
            ENDLOOP.
          ENDIF.
          IF om_data_entry-manager  IS NOT INITIAL.
            om_data_entry-dep_type = 'Department'.
          ENDIF.
          CLEAR: manager_id.
          EXIT.
        ...
        WHEN '210'.
          ...
          CALL FUNCTION 'Z_HR_GET_VORGESETZTER'
            EXPORTING
              pernr   = <pa0001>-orgeh
              stichtg = sy-datum
*             VORGES  = 'TEAM'
...
* Implement suitable error handling here
          ...
          IF bzlb IS NOT INITIAL AND team_id IS NOT INITIAL.
            om_data_entry-dep_type = 'Team'.
          ELSE.
            om_data_entry-dep_type = 'Department'.
          ENDIF.
    ...
    LOOP AT hrp1001_up_tmp ASSIGNING FIELD-SYMBOL(<hrp1001_up>) WHERE begda LE <hr_periods>-endda AND
                                                                      endda GE <hr_periods>-begda.
      IF <hrp1001_up>-rsign = 'A' AND <hrp1001_up>-relat = '002'.
        om_data_entry-parent_integration_key = <hrp1001_up>-sobid.
      ELSEIF <hrp1001_up>-rsign = 'B' AND <hrp1001_up>-relat = '002'.
        CLEAR: hrp1001_up_tmp.
      ENDIF.
...
*DEBIEAKSA 20220630 bspw. oberste Org.einheit hat kein Parent -> muss aber trotzdem mitgeliefert werden -> daher zusätzliche Abfrage auf RSIGN und RELAT
    IF om_data_entry-parent_integration_key IS NOT INITIAL OR hrp1001_up_tmp IS INITIAL.

*JMB20210819 end insert

**JMB20210706 start insert - check mandant and departments for CoFu countries and set Department_SET
*DEBIEAKSA 20220630 auskommentiert, da SEW
*      CASE sy-mandt.
*        WHEN '005'. "France
...
*DEBIEAKSA 20220726 Logik für Kostenstelle, Buchungskreis, Personalbereich, Personalteilbereich über den Inhaber der Stelle.
...
*        ENDIF.
        ...
        IF ls_csks IS NOT INITIAL.
          om_data_entry-company = ls_csks-bukrs.
        ENDIF.


        IF om_data_entry-company IS NOT INITIAL.
          "get bukrs text
          READ TABLE bukrs_txt INTO DATA(butxt) WITH KEY bukrs = om_data_entry-company.
...
*        IF <hrp1008_br>-persa IS NOT INITIAL AND <hrp1008_br>-btrtl IS NOT INITIAL.
*          om_data_entry-location = <hrp1008_br>-persa && <hrp1008_br>-btrtl.
*        ELSE.
*          om_data_entry-location = '01000101'.
*        ENDIF.
...
*
      IF om_data_entry-company EQ '0028' AND
         sy-mandt              EQ zmhp_cl_int_constants=>cofu_mandant-germany.
        om_data_entry-department_set = 'DE_PS' && set.
      ENDIF.
*JMB20211031 insert end

      IF om_data_entry-integration_key NE '00000003' AND om_data_entry-parent_integration_key IS INITIAL.
        CLEAR: om_data_entry.
      ELSE.
...
METHOD map_om_data.
  ...
  DATA: om_data          TYPE zmhp_tt_mig_om_data,
        om_data_imp      TYPE zmhp_tt_mig_om_data,
        om_data_ca_tmp   TYPE zmhp_tt_mig_om_data,
        om_data_fut      TYPE zmhp_tt_mig_om_data,
        om_entry         TYPE zmhp_s_mig_om_data,
        om_data_conv_tmp TYPE truxs_t_text_data,
        value_tmp        TYPE zmhp_dd_value,
        t001             TYPE TABLE OF t001.
...
*    READ TABLE me->hrp1008_com ASSIGNING FIELD-SYMBOL(<bukrs>) WITH KEY objid = <objec>-objid
*                                                                     begda = <objec>-begda
*                                                                     endda = <objec>-endda.
*   Read t001 with bukrs.
*    SELECT SINGLE land1, butxt FROM t001 INTO ( @om_entry-country, @om_entry-company_name ) WHERE bukrs = @<bukrs>-bukrs.
*    "set country of main org unit
*    om_entry-country = t001-land1.
    CONCATENATE om_entry-country set INTO DATA(department_set).
...
*JMB20211008 deletion end
    ...
    DATA(om_data_imp_cnt) = lines( om_data_imp ).
    DATA(om_data_cnt) = lines( om_data ).
    DATA(om_data_ca_cnt) = lines( om_data_ca_tmp ).
...
**JMB20210528 start insert- create Costing data
    ...
    IF lines( om_data_ca_tmp ) = 6401.
      IF 1 = 0.
      ENDIF.
    ENDIF.
*JMB20210528 end insert

    CLEAR: om_entry, orgid_cc_entry, om_data_tmp, om_data_fut.
  ENDLOOP.

  om_data_mig = create_mig_file( om_data_imp ).
  om_data_ca  = create_ca_file( om_data_ca_tmp ).
  om_data_cc  = create_cc_file( om_data_ca_tmp ).
  SORT om_data.
  DELETE ADJACENT DUPLICATES FROM om_data.
